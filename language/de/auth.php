<?php

//%%%%%%		File Name auth.php 		%%%%%

define('_AUTH_MSG_AUTH_METHOD',"Verwendung der %s Authentifizierungsmethode.");
define('_AUTH_LDAP_EXTENSION_NOT_LOAD','PHP LDAP Erweiterung nicht geladen (überprüfen Sie die PHP Konfigurationsdatei php.ini)');
define('_AUTH_LDAP_SERVER_NOT_FOUND',"Keine Verbindung zum Server möglich");
define('_AUTH_LDAP_USER_NOT_FOUND',"Mitglied %s nicht im Verzeichnisserver (%s) in %s gefunden");
define('_AUTH_LDAP_CANT_READ_ENTRY',"Konnte Eintrag %s nicht lesen.");
define('_AUTH_LDAP_XOOPS_USER_NOTFOUND',"Sorry no corresponding user information has been found in the ImpressCMS database for connection: %s <br />" .
		"Please verify your user datas or set on the automatic provisionning");
define('_AUTH_LDAP_START_TLS_FAILED',"Fehler beim Öffnen der TLS-Verbindung");

