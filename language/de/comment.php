<?php

define('_CM_TITLE','Titel');
define('_CM_MESSAGE','Nachricht');
define('_CM_DOSMILEY','Smiley-Icons aktivieren');
define('_CM_DOHTML','HTML-Tags aktivieren');
define('_CM_DOAUTOWRAP','Zeilenumbrüche aktivieren');
define('_CM_DOXCODE','iCMS Codes aktivieren');
define('_CM_REFRESH','Neu laden');
define('_CM_PENDING','Warten auf');
define('_CM_HIDDEN','Versteckt');
define('_CM_ACTIVE','Aktiv');
define('_CM_STATUS','Status');
define('_CM_POSTCOMMENT','Kommentar schreiben');
define('_CM_REPLIES','Antworten');
define('_CM_PARENT','Übergeordnet');
define('_CM_TOP','Oben');
define('_CM_ONLINE','Online!');
define('_CM_POSTED','Geschrieben am'); // Posted date
define('_CM_UPDATED', 'Aktualisiert');
define('_CM_THREAD','Thread');
define('_CM_POSTER','Benutzer');
define('_CM_JOINED','Angemeldet am');
define('_CM_POSTS','Beiträge');
define('_CM_FROM','Aus');
define('_CM_COMDELETED', 'Kommentar(e) gelöscht.');
define('_CM_COMDELETENG', 'Kommentar konnte nicht gelöscht werden.');
define('_CM_DELETESELECT' , 'Wollen Sie alle Kommentare löschen?');
define('_CM_DELETEONE' , 'Nein, nur diesen Kommentar löschen');
define('_CM_DELETEALL', 'Ja, alle löschen');
define('_CM_THANKSPOST', 'Vielen Dank für Ihre Nachricht!');
define('_CM_NOTICE', "Die hier veröffentlichten Artikel und Kommentare stehen uneingeschränkt im alleinigen Verantwortungsbereich des jeweiligen Autors.");
define('_CM_COMRULES','Regeln für Kommentare');
define('_CM_COMAPPROVEALL','Kommentare werden sofort und immer genehmigt');
define('_CM_COMAPPROVEUSER','Nur angemeldete Benutzer dürfen Kommentare schreiben');
define('_CM_COMAPPROVEADMIN','Alle Kommentare müssen vom Administrator geprüft und genehmigt werden');
define('_CM_COMANONPOST','Gästen erlauben Kommentare zu schreiben?');
define('_CM_COMNOCOM','Disable comments');
define('_CM_RE','Re');
############# added since 1.2 #############
define('_CM_COULDNOTCHANGEPIDTOID', 'Could not change comment parent ID from <b>%s</b> to <b>%s</b>. (ID: %s)');
define('_CM_COULDNOTCHANGEROOTID', 'Could not change comment root ID from <b>%s</b> to <b>%s</b>.');
define('_CM_COULDNOTCHANGEPAID', 'Could not change comment parent ID from <b>%s</b> to <b>%s</b>.');
define('_CM_COULDNOTUPDATEPOSTS', 'Could not update user posts.');
