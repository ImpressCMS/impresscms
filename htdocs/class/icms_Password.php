    ...
    */
    private function icms_passwordExpired($uname)
    {
        ...
        if($uname !== '')
        {
            ...
            {
                $sql = $db->query("SELECT loginname, pass_expired FROM ".$db->prefix('users')." WHERE
                    loginname = '".@htmlspecialchars($uname, ENT_QUOTES, _CHARSET)."'");
                list($loginname, $pass_expired) = $db->fetchRow($sql);
            }
            elseif($table->fieldExists('login_name'))
            {
                $sql = $db->query("SELECT login_name, pass_expired FROM ".$db->prefix('users')." WHERE
                    login_name = '".@htmlspecialchars($uname, ENT_QUOTES, _CHARSET)."'");
                list($login_name, $pass_expired) = $db->fetchRow($sql);
            }
            else
            {
