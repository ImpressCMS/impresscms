...
// ***** Private Functions *****
	...
	*/
	private function icms_encryptPassword($pass, $salt, $enc_type, $reset)
	{
		...
		if($enc_type == 0) {$pass_hash = md5($pass);} // no salt used for compatibility with external scripts such as ipb/phpbb etc.
		elseif($enc_type == 1) {$pass_hash = hash('sha256', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 2) {$pass_hash = hash('sha384', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 3) {$pass_hash = hash('sha512', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 4) {$pass_hash = hash('ripemd128', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 5) {$pass_hash = hash('ripemd160', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 6) {$pass_hash = hash('whirlpool', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 7) {$pass_hash = hash('haval128,4', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 8) {$pass_hash = hash('haval160,4', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 9) {$pass_hash = hash('haval192,4', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 10) {$pass_hash = hash('haval224,4', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 11) {$pass_hash = hash('haval256,4', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 12) {$pass_hash = hash('haval128,5', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 13) {$pass_hash = hash('haval160,5', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 14) {$pass_hash = hash('haval192,5', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 15) {$pass_hash = hash('haval224,5', $salt.md5($pass).$this->mainSalt);}
		elseif($enc_type == 16) {$pass_hash = hash('haval256,5', $salt.md5($pass).$this->mainSalt);}
		unset($mainSalt, $pass);
		return $pass_hash;
	}
