...
* @version	$Id: xoopscomments.php 11500 2011-12-17 22:09:00Z skenow $
*/
...
class XoopsComments extends XoopsObject
{
   ...
   */
	function printNavBar($item_id, $mode="flat", $order=1)
	{
		...
		echo "<form method='get' action='".$_SERVER['SCRIPT_NAME']."'><table width='100%' border='0' cellspacing='1' cellpadding='2'><tr><td class='bg1' align='center'><select name='mode'><option value='nocomments'";
		if ( $mode == "nocomments" ) {
   ...
   */
	function showThreadPost($order, $mode, $adminview=0, $color_num=1)
	{
		...
		if ( $poster ) {
			...
			$rank = $poster->rank();
			if ( $rank['image'] != "" ) {
				$rank['image'] = "<img src='".XOOPS_UPLOAD_URL."/system/userrank/".$rank['image']."' alt='' />";
			}
