...
class IcmsFormRichFileElement extends icms_form_elements_Tray {
	...
	function IcmsFormRichFileElement($form_caption, $key, $object) {
		...
		if($object->getVar('url') != '' ){
			...
			$this->addElement( new icms_form_elements_Lebel( '', _CO_ICMS_CURRENT_FILE."<a href='" . str_replace('{ICMS_URL}', ICMS_URL ,$object->getVar('url')) . "' target='_blank' >". $caption."</a><br/><br/>" ) );
			//$this->addElement( new icms_form_elements_Lebel( '', "<br/><a href = '".SMARTOBJECT_URL."admin/file.php?op=del&amp;fileid=".$object->id()."'>"._CO_ICMS_DELETE_FILE."</a>"));
		}
		...
		if($object->isNew()){
			...
			$this->addElement( new icms_form_elements_Lebel( '', '<br/><br/><small>'._CO_ICMS_URL_FILE_DSC.'</small>'));
		...
		if(!$object->isNew()){
			$this->addElement( new icms_form_elements_Lebel( '','<br/>'._CO_ICMS_CHANGE_FILE));
			$this->addElement(new IcmsFormFileUploadElement($object, $key));
			$this->addElement( new icms_form_elements_Lebel( '', '<br/><br/><small>'._CO_ICMS_URL_FILE_DSC.'</small>'));
			$this->addElement( new icms_form_elements_Lebel( '','<br/>'._CO_ICMS_URL_FILE));
			$this->addElement(new IcmsFormTextElement($object, 'url_'.$key));
