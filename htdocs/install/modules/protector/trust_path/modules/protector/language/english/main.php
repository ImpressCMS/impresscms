<?php
define('_MD_PROTECTOR_YOUAREBADIP','You are registered as BAD_IP by Protector.<br />') ;
define('_MD_PROTECTOR_FMT_JAILINFO','This restriction will be expired on %s') ;
define('_MD_PROTECTOR_FMT_JAILTIME','Y-m-j H:i:s') ;
define('_MD_PROTECTOR_BANDWIDTHLIMITED','This site is very busy now. try later.') ;
define('_MD_PROTECTOR_TURNJAVASCRIPTON','Turn JavaScript ON') ;
define('_MD_PROTECTOR_DENYBYRBL','Your post was not accepted because your IP is registered in RBL') ;
define('_MD_PROTECTOR_DENYBYMULTIBYTE','Protector rejects your post, because your posted string does not contain any multibyte characters') ;
define('_MD_PROTECTOR_FMT_REGISTER_MORATORIUM','Post it again %s minutes later. (for Anti-SPAMMING, sorry)') ;
define('_MD_PROTECTOR_MANIPUNOTIFYSBJ','Notify from Protector') ;
define('_MD_PROTECTOR_FMT_MANIPUNOTIFYBODY',"%s\n\nThe root of your site has been modified.\n If you didn't make this modification, check it carefully against ftp worms etc.") ;

?>