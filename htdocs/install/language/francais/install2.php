<?php
//cpascalweb
define('_INSTALL_WEBMASTER', 'Webmasters');
define('_INSTALL_WEBMASTERD', 'Webmasters de ce site');
define('_INSTALL_REGUSERS', 'Utilisateurs enregistr&eacute;s');
define('_INSTALL_REGUSERSD', 'Groupe des utilisateurs enregistr&eacute;s');
define('_INSTALL_ANONUSERS', 'Utilisateurs anonymes');
define('_INSTALL_ANONUSERSD', 'Groupe des utilisateurs anonymes');

define('_INSTALL_L165', 'Le site est actuellement ferm&eacute; pour maintenance. Merci de revenir plus tard.');


define('_INSTALL_ANON', 'Anonyme');

define('_INSTALL_DISCLMR', "Bien que les administrateurs et mod&eacute;rateurs de ce site s'efforceront de supprimer ou modifier tout sujet, articles et autres &agrave; caract&egrave;re r&eacute;pr&eacute;hensible aussi rapidement que possible, il est impossible de passer en revue tous les messages. Vous admettez donc que tous les posts effectu&eacute;s sur ce site expriment les point de vues et les opinions de leurs auteurs et non pas des administrateurs, ou mod&eacute;rateurs, ou webmasters (except&eacute; les messages post&eacute;s par eux-m&ecirc;me) et, par cons&eacute;quent, ils ne seront pas tenu responsable.

Vous acceptez de ne poster aucun post abusif, obsc&egrave;ne, vulgaire, diffamatoire, haineux, mena&ccedil;ant, sexuel r&eacute;pr&eacute;hensible ou tout autre sujet qui violerait les lois applicables en france. Attention, en cas de non respect de ces r&egrave;gles vous serez imm&eacute;diatement interdit de connexion a ce site (et votre fournisseur de service pourra &ecirc;tre inform&eacute;). L'adresse IP de chaque message est enregistr&eacute;e afin d'aider &agrave; faire respecter ces conditions. En outre, la cr&eacute;ation de multiples comptes pour un seul utilisateur n'est pas autoris&eacute;. Vous acceptez le fait que le webmaster, l'administrateur et les mod&eacute;rateurs de ce site ont le droit absolu de supprimer, modifier, d&eacute;placer ou fermer n'importe quel compte ou sujet de discussion et autres posts &agrave; tout moment si elles le jugent r&eacute;pr&eacute;hensible. En tant qu'utilisateur, vous vous engagez &agrave; l'exactitude de toute information que vous aurez entr&eacute; ci-apr&egrave;s. Bien que ces informations ne seront pas divulgu&eacute;es &agrave; de tierce personne sans votre consentement, le webmaster, l'administrateur et les mod&eacute;rateurs ne peuvent pas &ecirc;tre tenue pour responsable de toute tentative de piratage informatique malgr&eacute; toutes les dispositions prises.

Ce site utilisent des cookies pour stocker des informations sur votre ordinateur. Ces cookies ne contiendront aucune information que vous aurez entrer ci-dessus, ils servent uniquement &agrave; am&eacute;liorer votre navigation et connexion sur ce site. L'adresse Email est uniquement utilis&eacute;e pour confirmer votre inscription (et aussi pour vous envoyer un nouveau mot de passe si vous l'oublieriez).

En cliquant ci-dessous lors de votre inscription &agrave; ce site vous acceptez ces conditions sinon refuser de vous inscrire.");

define('_INSTALL_PRIVPOLICY', "<p>Cette politique de confidentialit&eacute; fixe les modalit&eacute;s de ce site l'utilisation et la protection de toute information que vous donnez sur ce site Lorsque vous utilisez ce site Web. Ont s'engagent &agrave; veiller &agrave; ce que votre vie priv&eacute;e soit prot&eacute;g&eacute;e. Si nous vous demandons de nous fournir certaines informations permettant de vous identifier lorsque vous utilisez ce site, vous pouvez &ecirc;tre assur&eacute; que ces informations seront utilis&eacute; conform&eacute;ment &agrave; la pr&eacute;sente d&eacute;claration de confidentialit&eacute;. Nous pouvons changer cette politique de temps en temps en la mettant &agrave; jour. Vous devez v&eacute;rifier cette page de temps &agrave; autre pour veiller que vous acceptez toujours ces conditions malgr&eacute; ces changements.
</p><p>
Cette politique est effective &agrave; partir du [date].
</p>
<h2>Qu'est-ce que nous recueillons</h2>
<p>
Nous recueillons les informations suivantes:
<ul>
<li>nom et titre</li>
<li>les informations personnel (age, sexe etc..) et toutes les coordonn&eacute;es (Email, messagerie hotmail etc..)</li>
<li>les informations d&eacute;mographiques telles que code postal, pr&eacute;f&eacute;rences et int&eacute;r&ecirc;ts</li>
<li>et d'autres informations pertinentes pour vous faire de meilleurs offres suivant le sujet du site</li></ul>
</p>
<h2>Ce que nous faisons avec les informations que nous recueillons</h2>
<p>
Nous avons besoin de ces informations pour comprendre vos besoins et vous fournir un meilleur service, et en particulier pour les raisons suivantes:
<ul>
<li>La tenue des registres internes.</li>
<li>Pour am&eacute;liorer nos produits et services.</li>
<li>Pour vous envoyez des Emails promotionnels sur les nouveaux produits, des offres sp&eacute;ciales, les mise &agrave; jour du site, les nouveaut&eacute;es ou d'autres informations que nous pensons que vous trouverez int&eacute;ressant.</li>
<li>De temps en temps, pour vous contacter par Email pour des &eacute;tudes de march&eacute; ou autres.</li>
<li>Utiliser ces informations pour personnaliser le site selon vos int&eacute;r&ecirc;ts.</li></ul>
</p>
<h2>S&eacute;curit&eacute;</h2>
<p>
Nous nous sommes engag&eacute;s &agrave; faire en sorte que vos informations soit s&eacute;curis&eacute;e. Afin d'emp&ecirc;cher tout acc&egrave;s non autoris&eacute; ou la divulgation nous avons mis en place des scripts de gestion pour sauvegarder et s&eacute;curiser les informations que nous recueillons en ligne.
</p>
<h2>Comment nous utilisons les cookies</h2>
<p>
Un cookie est un petit fichier qui demande la permission d'&ecirc;tre install&eacute; sur votre disque dur. Une fois que vous &ecirc;tes d'accord, le fichier est ajout&eacute; et le cookie permet d'adapter le fonctionnement du site &agrave; vos besoins, en se souvenant les informations sur vos pr&eacute;f&eacute;rences.
</p><p>
Nous utilisons aussi un journal cookies pour identifier les pages qui sont visit&eacute;es & pour vous authentifier en tant que membre enregistr&eacute;. Cela nous aide aussi en analysant les donn&eacute;es du trafic sur les pages du site afin d'am&eacute;liorer notre site Web,de l'adapter aux besoins des utilisateurs. Nous utilisons ces informations &agrave; des fins statistiques puis les donn&eacute;es sont supprim&eacute; du syst&egrave;me. Globalement, les cookies nous aider &agrave; vous fournir un meilleur site Web, en nous permettant de surveiller les pages que vous trouverez utile et ceux qui ne le sont pas. Un cookie ne nous donne absolument pas acc&egrave;s &agrave; votre ordinateur ou &agrave; toutes autres informations vous concernant, seulement aux donn&eacute;es que vous choisissez de partager avec nous.
</p><p>
Vous pouvez choisir d'accepter ou de refuser les cookies. La plupart des navigateurs Web acceptent automatiquement les cookies, mais vous pouvez modifier les pr&eacute;f&eacute;rences de votre navigateur afin de refuser les cookies. attention ! Cela peut vous emp&ecirc;cher de profiter pleinement du site, y compris l'enregistrement et l'identification sur le site
</p>
<h2>Liens vers d'autres sites</h2>
<p>
Notre site Web contient des liens pour vous permettre de visiter d'autres sites. Cependant, une fois que vous avez utilis&eacute; ces liens vous quitter notre site, Il convient de noter que nous n'avons pas de contr�le sur les autres sites. Par cons&eacute;quent, nous ne pouvons pas &ecirc;tre responsables de la protection et la confidentialit&eacute; de toute information que vous fournissez en visitant ces sites et ces sites ne sont pas r&eacute;gies par la pr&eacute;sente d&eacute;claration de confidentialit&eacute;. Vous devez faire preuve de prudence et de regarder la d&eacute;claration de confidentialit&eacute; applicable au site Web en question.
</p>
<h2>Contr�le de vos informations personnelles</h2>
<p>
A vous de choisir de restreindre la collecte ou d'utiliser vos renseignements personnels par les moyens suivants:
<ul>
<li>chaque fois que vous &ecirc;tes invit&eacute; &agrave; remplir un formulaire sur le site Web, recherchez la case que vous pouvez cliquer pour indiquer que vous ne voulez pas que ces informations soit utilis&eacute;es par qui que ce soit des fins de marketing direct</li>
<li>si vous avez accepter que vos renseignements personnels soit utiliser &agrave; des fins de marketing direct, vous pouvez changer d'avis &agrave; tout moment il vous suffit de nous en informer par &eacute;crit ou par Email</li></ul>
</p><p>
Attention ! Jamais nous vendrons, ne distribuerons ou louerons vos informations personnelles &agrave; des tiers, sauf si nous avons votre permission ou somme tenus par la loi de le faire. Nous utiliserons vos renseignements personnels pour vous envoyer des informations promotionnelles sur des tiers qui, selon nous, vous trouverez int&eacute;ressant seulement avec votre accord.
</p><p>
Si vous pensez que toutes les informations sur vous sont incorrecte ou incompl&egrave;te, Merci d'&eacute;crire ou de nous envoyer un Email d&egrave;s que possible. Nous corrigerons rapidement toutes les informations r&eacute;v&eacute;l&eacute;e inexacte.
</p>");
define('_WELCOME_MSG_CONTENT', "Bienvenue {UNAME},

Votre compte a &eacute;t&eacute; activ&eacute; avec succ&egrave;s sur {X_SITENAME}. En tant que membre de notre site, vous b&eacute;n&eacute;ficierez de toutes les fonctionnalit&eacute;s r&eacute;serv&eacute;es aux membres inscrits !

Encore une fois, bienvenue sur notre site. Visitez-nous souvent!

Si vous ne vous &ecirc;tes pas inscrit sur notre site, merci de nous contacter &agrave; l'adresse suivante {X_ADMINMAIL}, nous r&eacute;tablirons la situation.

-----------
Cordialement,
{X_SITENAME}
{X_SITEURL}");
