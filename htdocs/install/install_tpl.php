...
if (isset($_COOKIE["xo_install_lang"])) {
	$icmsConfig["language"] = $icmsConfig["language"] = htmlentities(
...
<div id="wrapper">
	<div id="header">
		...
		<div id="info"><?php echo sprintf(XOOPS_INSTALL_WIZARD, XOOPS_VERSION) .
  	"<br />" .
  	INSTALL_STEP; ?>
			&nbsp;<?php echo $wizard->currentPage +
   	1 .
   	INSTALL_OUTOF .
   	count($wizard->pages); ?></div>
	</div>
	...
	<div id="page">
		<form action='<?php echo htmlentities($_SERVER["PHP_SELF"]); ?>' method='post'>
     	...
     	$wizard->currentPageName != $wizard->lastpage
     ) { ?>
					...
					<?php } ?>
					<?php if ($wizard->currentPageName == $wizard->lastpage) { ?>
						<button id="hmo" title="<?php echo BUTTON_SHOW_SITE; ?>" type="button"
								onclick="location.href='<?php echo $wizard->pageURI(
        	$wizard->lastpage,
        ); ?>?success=true'"
								class="finish">
					...
					<button type="submit">
						...
						<?php } elseif ($wizard->pages[$wizard->currentPage] != $wizard->lastpage) { ?>
						<?php if (@$pageHasForm) { ?>
