	...
	$.fn.passStrength = function(options) {
		...
		return this.each(function() {
			...
			$.fn.teststrength = function(password, uname, option) {
				...
				//password < 4
				if (password.length < option.minPass) {
					this.resultStyle = option.shortPass;
					return $(this).shortPass;
				}

				//password == user name
				if (password.toLowerCase() == uname.toLowerCase()) {
				...
				score +=
					($.fn.checkRepetition(1, password).length -
						password.length) *
					1;
				score +=
					($.fn.checkRepetition(2, password).length -
						password.length) *
					1;
				score +=
					($.fn.checkRepetition(3, password).length -
						password.length) *
					1;
				score +=
					($.fn.checkRepetition(4, password).length -
						password.length) *
					1;
				...
				if (option.strongnessPass > 20) {
					score -= option.strongnessPass / 4;
				}
