...
//>>excludeEnd('excludeHbs')
  ...
  //>>excludeStart('excludeHbs', pragmas.excludeHbs)
  var fs, getXhr, progIds = ['Msxml2.XMLHTTP', 'Microsoft.XMLHTTP', 'Msxml2.XMLHTTP.4.0'],
    fetchText = function () {
      ...
      function fetchAndRegister(langMap) {
        fetchText(path, function (text) {
          ...
          var nodes = Handlebars.parse(text),
            ...
            helpDepStr = config.hbs && config.hbs.disableHelpers ? "" : (function () {
              var i, paths = [],
          ...
          var mapping = disableI18n ? false : _.extend(langMap, config.localeMapping),
            prec = precompile(text, mapping, {
              originalKeyFallback: (config.hbs || {}).originalKeyFallback
            });
