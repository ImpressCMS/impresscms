        ...
        miniColors: function(o, data) {
            ...
            var create = function(input, o, data) {
                ...
                var color = expandHex(input.val()) || 'ffffff',
                    hsb = hex2hsb(color),
                    rgb = hsb2rgb(hsb),
                    alpha = parseFloat(input.attr('data-opacity'))
                        .toFixed(2);
            ...
            var destroy = function(input) {
                ...
                input.data('trigger')
                    .parent()
                    .remove();
                input.attr('autocomplete', input.data('original-autocomplete'))
            ...
            var show = function(input) {
                ...
                selector.find('.miniColors-huePicker')
                    .css('top', huePosition + 'px');
                ...
                selector.find('.miniColors-opacityPicker')
                    .css('top', opacityPosition + 'px');
                ...
                var trigger = input.data('trigger'),
                    ...
                    top = hidden ? trigger.offset()
                        .top + trigger.outerHeight() : input.offset()
                        .top + input.outerHeight(),
                    left = hidden ? trigger.offset()
                        .left : input.offset()
                        .left,
                ...
                if (!$.browser.msie || ($.browser.msie && $.browser.version >= 9)) {
                    $(window)
                        .on('resize.miniColors', function(event) {
                        hide(input);
                    });
                }
                ...
                if (input.data('open')) {
                    input.data('open')
                        .call(input.get(0), '#' + hsb2hex(hsb), $.extend(hsb2rgb(hsb), {
                    ...
                    $(selector)
                        ...
                        if (input.data('close')) {
                            var hsb = input.data('hsb'),
                                hex = hsb2hex(hsb);
                            input.data('close')
                                .call(input.get(0), '#' + hex, $.extend(hsb2rgb(hsb), {
                                a: parseFloat(input.attr('data-opacity'))
                            }));
                        }
                        $(this)
            ...
            var moveColor = function(input, event) {
                ...
                position.x = position.x - input.data('selector')
                    .find('.miniColors-colors')
                    .offset()
                    .left - 6;
                position.y = position.y - input.data('selector')
                    .find('.miniColors-colors')
                    .offset()
                    .top - 6;
                ...
                colorPicker.css('left', position.x)
                    .css('top', position.y)
                    .show();
            ...
            var moveHue = function(input, event) {
                ...
                position = position - input.data('selector')
                    .find('.miniColors-colors')
                    .offset()
                    .top - 1;
            ...
            var moveOpacity = function(input, event) {
                ...
                opacityPicker.css('top', position)
                    .show();

                // Calculate opacity
                var alpha = parseFloat((150 - position - 1) / 150)
                    .toFixed(2);
                if (alpha < 0) alpha = 0;
                if (alpha > 1) alpha = 1;

                // Update opacity
                input.data('alpha', alpha)
            ...
            var setColor = function(input, hsb, updateInput) {
                ...
                if (input.data('opacity')) {
                    input.data('trigger')
                        .css('backgroundColor', 'rgba(' + rgb.r + ', ' + rgb.g + ', ' + rgb.b + ', ' + input.attr('data-opacity') + ')');
                }
            ...
            var setColorFromInput = function(input) {
                ...
                var colorPicker = $(input.data('colorPicker'));
                colorPicker.css('top', colorPosition.y + 'px')
                    .css('left', colorPosition.x + 'px');
