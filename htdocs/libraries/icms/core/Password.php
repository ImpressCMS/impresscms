...
final class icms_core_Password
{
     ...
     */
    public static function createSalt($slength=64)
    {
        $salt = '';
    ...
    private function _passExpired($uname)
    {
        ...
        if ($table->fieldExists('loginname')) {
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT pass_expired FROM %s WHERE loginname = %s',
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT pass_expired FROM %s WHERE login_name = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($pass_expired) = icms::$xoopsDB->fetchRow($sql);
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT pass_expired FROM %s WHERE uname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($pass_expired) = icms::$xoopsDB->fetchRow($sql);
    ...
    private function _getUserSalt($uname)
    {
        ...
        if ($table->fieldExists('loginname')) {
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT salt FROM %s WHERE loginname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT salt FROM %s WHERE login_name = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($salt) = icms::$xoopsDB->fetchRow($sql);
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT salt FROM %s WHERE uname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($salt) = icms::$xoopsDB->fetchRow($sql);
    ...
    private function _getUserEncType($uname)
    {
        ...
        if ($table->fieldExists('loginname')) {
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT enc_type FROM %s WHERE loginname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT enc_type FROM %s WHERE login_name = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($enc_type) = icms::$xoopsDB->fetchRow($sql);
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT enc_type FROM %s WHERE uname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($enc_type) = icms::$xoopsDB->fetchRow($sql);
    ...
    private function _getUserHash($uname)
    {
        ...
        if ($table->fieldExists('loginname')) {
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT pass FROM %s WHERE loginname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT pass FROM %s WHERE login_name = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($pass) = icms::$xoopsDB->fetchRow($sql);
            ...
            $sql = icms::$xoopsDB->query(sprintf(
                                             'SELECT pass FROM %s WHERE uname = %s',
                                             icms::$xoopsDB->prefix('users'),
                                             icms::$xoopsDB->quoteString($uname)
            ));
            list($pass) = icms::$xoopsDB->fetchRow($sql);
