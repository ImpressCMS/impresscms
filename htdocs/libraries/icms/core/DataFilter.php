...
class icms_core_DataFilter
{
    ...
    * @public	array
    */
    public static $displaySmileys = [];

    /**
    * @public	array
    */
    public static $allSmileys = [];
    ...
    */
     ...
     * @return	array
     */
    public static function checkVarArray(array $input, array $filters, $strict = true)
    {
        foreach (array_intersect_key($input, $filters) as $key => $value) {
            ...
            if (is_array($input[$key])) {
                $output[$key] = self::checkVarArray($input[$key], [$key => $filter], false);
            } else {
        ...
        if (!$strict) {
            foreach ($diff = array_diff_key($input, $filters) as $key => $value) {
                if (is_array($diff[$key])) {
                    $output[$key] = self::checkVarArray($diff[$key], [$key => 'str'], false);
                } else {
    ...
    {
        icms::$preload->triggerEvent('beforeFilterTextareaInput', [&$text]);

        $text = self::htmlSpecialChars($text);
        $text = self::stripSlashesGPC($text);

        icms::$preload->triggerEvent('afterFilterTextareaInput', [&$text]);
    ...
    {
        icms::$preload->triggerEvent('beforeFilterTextareaDisplay', [&$text, $smiley, $icode, $image, $br]);
        ...
        icms::$preload->triggerEvent('afterFilterTextareaDisplay', [&$text, $smiley, $icode, $image, $br]);
        return $text;
    }
    ...
    {
        icms::$preload->triggerEvent('beforeFilterHTMLinput', [&$html, 1, 1, 1, $br]);
        ...
        icms::$preload->triggerEvent('afterFilterHTMLinput', [&$html, 1, 1, 1, $br]);
        return $html;
    }
    ...
    public static function filterHTMLdisplay($html, $icode = 1, $br = 0)
    {
        ...
        icms::$preload->triggerEvent('beforeFilterHTMLdisplay', [&$html, 1, $br]);
        ...
        icms::$preload->triggerEvent('afterFilterHTMLdisplay', [&$html, 1, $br]);
        return $html;
    }
    ...
    public static function makeClickable($text)
    {
        ...
        if ($icmsConfigPersona['shorten_url'] == 1) {
            ...
            for ($i = 0; $i < $countlinks; $i++) {
                ...
                $middleurl = ' ... ';
                $chunked = (strlen($urlname) > $maxlength && preg_match(
                    '#^(https://|http://|ftp://|www\.)#is',
                    $urlname
                )) ? substr_replace($urlname, $middleurl, $cutlength, $endlength) : $urlname;
