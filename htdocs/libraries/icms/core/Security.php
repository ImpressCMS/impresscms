...
class icms_core_Security
{
    ...
    public static function service()
    {
        ...
        icms_Event::attach('icms', 'loadService-config', [$instance, 'checkBadips']);
        return $instance;
    }
    ...
    public function createToken($timeout = 0, $name = _CORE_TOKEN)
    {
        ...
        $token_data = ['id' => $token_id, 'expire' => time() + (int) ($timeout)];
    ...
    public function garbageCollection($name = _CORE_TOKEN)
    {
        if (isset($_SESSION[$name . '_SESSION']) && count($_SESSION[$name . '_SESSION']) > 0) {
            $_SESSION[$name . '_SESSION'] = array_filter($_SESSION[$name . '_SESSION'], [$this, 'filterToken']);
        }
    }
    /**
    ...
    {
        foreach ([
                     'GLOBALS', '_SESSION', 'HTTP_SESSION_VARS', '_GET', 'HTTP_GET_VARS', '_POST', 'HTTP_POST_VARS',
