...
class icms_config_category_Handler extends icms_core_ObjectHandler
{
    ...
    public function insert(&$confcat)
    {
        ...
        if ($confcat->isNew()) {
            ...
            $sql = sprintf(
				...
				VALUES ('%u', %s, '%u')",
                $this->db->prefix('configcategory'),
                (int) ($confcat_id),
                $this->db->quoteString($confcat_name),
                (int) ($confcat_order)
                );
        } else {
            $sql = sprintf(
    ...
    public function delete(&$confcat)
    {
        ...
        $sql = sprintf(
            ...
            $this->db->prefix('configcategory'),
            (int) ($configcategory->getVar('confcat_id'))
            );
        if (!$result = $this->db->query($sql)) {
            return false;
        }
    ...
    public function getObjects($criteria = null, $id_as_key = false)
    {
        ...
        if (isset($criteria) && is_subclass_of($criteria, 'icms_db_criteria_Element')) {
            ...
            $sort = !in_array($criteria->getSort(), ['confcat_id', 'confcat_name', 'confcat_order'])
                    ? 'confcat_order'
                    : $criteria->getSort();
