...
class icms_image_category_Handler extends icms_core_ObjectHandler
{
    ...
    public function insert(&$imgcat)
    {
        ...
        if ($imgcat->isNew()) {
            ...
            $sql = sprintf(
                "INSERT INTO %s (imgcat_id, imgcat_pid, imgcat_name, imgcat_foldername, imgcat_display, imgcat_weight, imgcat_maxsize, imgcat_maxwidth, imgcat_maxheight, imgcat_type, imgcat_storetype) VALUES ('%u', '%u', %s, %s, '%u', '%u', '%u', '%u', '%u', %s, %s)",
            ...
            $sql = sprintf(
                "UPDATE %s SET imgcat_pid = %u, imgcat_name = %s, imgcat_foldername = %s, imgcat_display = '%u', imgcat_weight = '%u', imgcat_maxsize = '%u', imgcat_maxwidth = '%u', imgcat_maxheight = '%u', imgcat_type = %s WHERE imgcat_id = '%u'",
                $this->db->prefix('imagecategory'),
                (int) $imgcat_pid,
                $this->db->quoteString($imgcat_name),
                $this->db->quoteString($imgcat_foldername),
    ...
    /**
        ...
        */
    public function getCategList($groups = [], $perm = 'imgcat_read', $display = null, $storetype = null, $imgcat_id=null)
    {
        $criteria = new icms_db_criteria_Compo();
        if (is_array($groups) && !empty($groups)) {
            $criteriaTray = new icms_db_criteria_Compo();
