...
class icms_ipf_About
{
    ...
    public function render()
    {
        ...
        if ($people) {
            $this->_tpl->assign(
                'module_people_developers',
                isset($people['developers'])
                ? array_map([$this, 'sanitize'], $people['developers'])
                : false
            );
            $this->_tpl->assign(
                'module_people_testers',
                isset($people['testers'])
                ? array_map([$this, 'sanitize'], $people['testers'])
                : false
            );
            $this->_tpl->assign(
                'module_people_translators',
                isset($people['translators'])
                ? array_map([$this, 'sanitize'], $people['translators'])
                : false
            );
            $this->_tpl->assign(
                'module_people_documenters',
                isset($people['documenters'])
                ? array_map([$this, 'sanitize'], $people['documenters'])
                : false
            );
            $this->_tpl->assign(
                'module_people_other',
                isset($people['other'])
                ? array_map([$this, 'sanitize'], $people['other'])
                : false
            );
        }
        ...
        if ($manual) {
            $this->_tpl->assign('module_manual', isset($manual['wiki']) ? array_map([$this, 'sanitize'], $manual['wiki']) : false);
        }
