...
class icms_data_avatar_Handler extends icms_core_ObjectHandler
{
    ...
    public static function getListFromDir($avatar_dir="")
    {
        $avatars = [];
        if ($avatar_dir != "") {
            $avatars = icms_core_Filesystem::getFileList(ICMS_ROOT_PATH . "/images/avatar/" . $avatar_dir . "/", $avatar_dir . "/", ['gif', 'jpg', 'png']);
        } else {
            $avatars = icms_core_Filesystem::getFileList(ICMS_ROOT_PATH . "/images/avatar/", '', ['gif', 'jpg', 'png']);
        }
        return $avatars;
    }
    ...
    public static function getAllFromDir()
    {
        ...
        if (count($dirlist) > 0) {
            foreach ($dirlist as $dir) {
                $avatars[$dir] =& icms_core_Filesystem::getFileList(ICMS_ROOT_PATH . "/images/avatar/" . $dir . "/", $dir . "/", ['gif', 'jpg', 'png']);
            }
        } else {
            return false;
        }
