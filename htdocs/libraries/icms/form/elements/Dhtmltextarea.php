...
class icms_form_elements_Dhtmltextarea extends icms_form_elements_Textarea
{
    ...
    public function __construct($caption, $name, $value, $rows=5, $cols=50, $hiddentext= 'xoopsHiddenText', $options = [])
    {
        ...
        if (file_exists(ICMS_EDITOR_PATH . '/' . $editor_default . '/xoops_version.php') && $gperm_handler->checkRight('use_wysiwygeditor', $moduleid, $groups, 1, false)) {
            ...
            $this->htmlEditor = [$editorversion['class'], ICMS_EDITOR_PATH . '/' . $editorversion['dirname'] . '/' . $editorversion['file']];
        }
    ...
    public function render()
    {
        ...
        $ret = "<a name='moresmiley'></a>"
            . "<img onmouseover='style.cursor=\"pointer\"' src='" . ICMS_URL . "/images/url.gif' alt='url' onclick='xoopsCodeUrl(\"" . $ele_name . '", "'
               . htmlspecialchars(_ENTERURL, ENT_QUOTES) . '", "'
               . htmlspecialchars(_ENTERWEBTITLE, ENT_QUOTES) . "\");' />&nbsp;"
            . "<img onmouseover='style.cursor=\"pointer\"' src='" . ICMS_URL . "/images/email.gif' alt='email' onclick='javascript:xoopsCodeEmail(\"" . $ele_name . '", "'
               . htmlspecialchars(_ENTEREMAIL, ENT_QUOTES) . "\");' />&nbsp;"
            . "<img onclick='javascript:xoopsCodeImg(\"" . $ele_name . '", "'
               . htmlspecialchars(_ENTERIMGURL, ENT_QUOTES) . '", "'
               . htmlspecialchars(_ENTERIMGPOS, ENT_QUOTES) . '", "'
               . htmlspecialchars(_IMGPOSRORL, ENT_QUOTES) . '", "'
               . htmlspecialchars(_ERRORIMGPOS, ENT_QUOTES) . "\");' onmouseover='style.cursor=\"pointer\"' src='" . ICMS_URL . "/images/imgsrc.gif' alt='imgsrc' />&nbsp;"
            . "<img onmouseover='style.cursor=\"pointer\"' onclick='javascript:openWithSelfMain(\"" . ICMS_URL . '/modules/system/admin/images/browser.php?target='
               . $ele_name . "&type=iman\",\"imgmanager\",985,470);' src='" . ICMS_URL . "/images/image.gif' alt='image' />&nbsp;";
        $jscript = '';
        ...
        if ($easiestml_exist) {
            ...
            foreach ($easiestml_langs as $l => $lang) {
                $ret .= "<img onclick='javascript:icmsCode_languages(\"" . $ele_name . '", "'
                        . htmlspecialchars(sprintf(_ENTERLANGCONTENT, $langlocalnames[$l]), ENT_QUOTES) . '", "'
                        . $lang . "\");' onmouseover='style.cursor=\"pointer\"' src='" . ICMS_URL . '/images/flags/'
                        . $langnames[$l] . ".gif' alt='" . $langlocalnames[$l] . "' />&nbsp;";
            }
            $ret .= "<br />\n";
        }
