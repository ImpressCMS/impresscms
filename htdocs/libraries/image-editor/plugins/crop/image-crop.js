...
var crop_script_browserIsOpera =
...
function crop_createDivElements() {
	...
	cropDiv_dotted.style.width =
		crop_imageWidth - cropToolBorderWidth * 2 + "px";
	cropDiv_dotted.style.height =
		crop_imageHeight - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_tl.style.left =
		-offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	smallSquare_tl.style.top =
		-offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_tr.style.left =
		crop_imageWidth - offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	smallSquare_tr.style.top =
		-offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_bl.style.left =
		-offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	smallSquare_bl.style.top =
		crop_imageHeight - offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_br.style.left =
		crop_imageWidth - offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	smallSquare_br.style.top =
		crop_imageHeight - offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_tc.style.left =
		Math.floor(crop_imageWidth / 2) -
		offsetSmallSquares -
		cropToolBorderWidth * 2 +
		"px";
	smallSquare_tc.style.top =
		-offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_bc.style.left =
		Math.floor(crop_imageWidth / 2) -
		offsetSmallSquares -
		cropToolBorderWidth * 2 +
		"px";
	smallSquare_bc.style.top =
		crop_imageHeight - offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	...
	smallSquare_lc.style.left =
		-offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	smallSquare_lc.style.top =
		Math.floor(crop_imageHeight / 2) -
		offsetSmallSquares -
		cropToolBorderWidth * 2 +
		"px";
	...
	smallSquare_rc.style.left =
		crop_imageWidth - offsetSmallSquares - cropToolBorderWidth * 2 + "px";
	smallSquare_rc.style.top =
		Math.floor(crop_imageHeight / 2) -
		offsetSmallSquares -
		cropToolBorderWidth * 2 +
		"px";
...
function cropScript_executeCrop(buttonObj) {
	...
	var url =
		crop_script_server_file +
		"?image_path=" +
		document.getElementById("image_path").value +
		"&image_url=" +
		document.getElementById("image_url").value +
		"&x=" +
		document.getElementById("input_crop_x").value +
		"&y=" +
		document.getElementById("input_crop_y").value +
		"&width=" +
		document.getElementById("input_crop_width").value +
		"&height=" +
		document.getElementById("input_crop_height").value +
		"&percentSize=" +
		document.getElementById("crop_percent_size").value;
...
function crop_delpreview() {
	...
	var url =
		crop_script_server_file +
		"?image_path=" +
		document.getElementById("fimage_path").value +
		"&image_url=" +
		document.getElementById("fimage_url").value +
		"&delprev=1";
...
function cropScript_mouseMove(e) {
	...
	if (crop_resizeCounter == 10) {
		if (
			crop_currentResizeType == "e-resize" ||
			crop_currentResizeType == "ne-resize" ||
			crop_currentResizeType == "se-resize"
		) {
			cropDiv_dotted.style.width =
				Math.max(
					crop_minimumWidthHeight,
					imageDiv_currentWidth + e.clientX - cropEvent_eventX
				) + "px";
		}
		if (
			crop_currentResizeType == "s-resize" ||
			crop_currentResizeType == "sw-resize" ||
			crop_currentResizeType == "se-resize"
		) {
			cropDiv_dotted.style.height =
				Math.max(
					crop_minimumWidthHeight,
					imageDiv_currentHeight + e.clientY - cropEvent_eventY
				) + "px";
		}

		if (
			crop_currentResizeType == "w-resize" ||
			crop_currentResizeType == "sw-resize" ||
			crop_currentResizeType == "nw-resize"
		) {
			var tmpTop = cropDiv_dotted.style.left.replace("px", "") / 1;
			var newTop = Math.max(
				0,
				imageDiv_currentLeft + e.clientX - cropEvent_eventX
			);
			if (
				newTop + crop_minimumWidthHeight >
				cropDiv_dotted.style.left.replace("px", "") / 1 +
					cropDiv_dotted.style.width.replace("px", "") / 1
			) {
				newTop =
					cropDiv_dotted.style.left.replace("px", "") / 1 +
					cropDiv_dotted.style.width.replace("px", "") / 1 -
		...
		if (
			crop_currentResizeType == "n-resize" ||
			crop_currentResizeType == "nw-resize" ||
			crop_currentResizeType == "ne-resize"
		) {
			...
			var newTop = Math.max(
				0,
				imageDiv_currentTop + e.clientY - cropEvent_eventY
			);
			if (
				newTop + crop_minimumWidthHeight >
				cropDiv_dotted.style.top.replace("px", "") / 1 +
					cropDiv_dotted.style.height.replace("px", "") / 1
			) {
				newTop =
					cropDiv_dotted.style.top.replace("px", "") / 1 +
					cropDiv_dotted.style.height.replace("px", "") / 1 -
		...
		else preserveAspectRatio = false;

		if (preserveAspectRatio) {
			var tmpRatio =
				cropDiv_dotted.offsetWidth / cropDiv_dotted.offsetHeight;
			if (tmpRatio < cropWidthRatio) {
				...
				cropDiv_dotted.style.height =
					cropDiv_dotted.style.width.replace("px", "") /
						1 /
						cropWidthRatio +
					"px";
			}
			...
			if (
				...
				var ratioToAdjust =
					(crop_imageHeight -
						cropDiv_dotted.style.top.replace("px", "") / 1) /
					(cropDiv_dotted.offsetHeight + cropToolBorderWidth * 2);
				if (
					Math.round(
						cropDiv_dotted.style.width.replace("px", "") *
							ratioToAdjust +
					...
					cropDiv_dotted.style.height =
						Math.round(
							cropDiv_dotted.style.height.replace("px", "") *
								ratioToAdjust +
								cropToolBorderWidth
						) + "px";
					cropDiv_dotted.style.width =
			...
			if (
				...
				var ratioToAdjust =
					(crop_imageWidth -
						cropDiv_dotted.style.left.replace("px", "") / 1) /
					(cropDiv_dotted.offsetWidth + cropToolBorderWidth * 2);
				if (
					Math.round(
						cropDiv_dotted.style.height.replace("px", "") *
							ratioToAdjust +
	...
	if (crop_moveCounter == 10) {
		...
		if (
			tmpLeft + imageDiv_currentWidth + cropToolBorderWidth * 2 >
			crop_imageWidth
		)
			tmpLeft =
				crop_imageWidth -
				imageDiv_currentWidth -
				cropToolBorderWidth * 2;
		cropDiv_dotted.style.left = tmpLeft + "px";
		var tmpTop = imageDiv_currentTop + e.clientY - cropEvent_eventY;
		if (tmpTop < 0) tmpTop = 0;
		if (
			tmpTop + imageDiv_currentHeight + cropToolBorderWidth * 2 >
			crop_imageHeight
		)
			tmpTop =
				crop_imageHeight -
				imageDiv_currentHeight -
				cropToolBorderWidth * 2;
...
function resizeTransparentSquares() {
	...
	cropDiv_right.style.left =
		cropDiv_dotted.style.width.replace("px", "") / 1 +
		cropToolBorderWidth * 2 +
		cropDiv_dotted.style.left.replace("px", "") / 1 +
		"px";
	cropDiv_bottom.style.height =
		Math.max(
			0,
			crop_imageHeight -
				cropToolBorderWidth * 2 -
	...
	cropDiv_bottom.style.width =
		cropDiv_dotted.style.width.replace("px", "") / 1 +
		cropToolBorderWidth * 2 +
		"px";
	cropDiv_top.style.left = cropDiv_dotted.style.left;
	cropDiv_top.style.width =
		cropDiv_dotted.style.width.replace("px", "") / 1 +
		cropToolBorderWidth * 2 +
		"px";
...
function cropScript_updateFormValues() {
	...
	document.getElementById("input_crop_width").value = Math.round(
		(cropDiv_dotted.style.width.replace("px", "") / 1 +
			cropToolBorderWidth * 2) *
			(crop_originalImageWidth / crop_imageWidth)
	);
	document.getElementById("input_crop_height").value = Math.round(
		(cropDiv_dotted.style.height.replace("px", "") / 1 +
			cropToolBorderWidth * 2) *
			(crop_originalImageHeight / crop_imageHeight)
	);
...
function cropScript_setCropSizeByInput() {
	...
	cropDiv_dotted.style.left =
		Math.round(
			(obj_x.value / 1) * (crop_imageWidth / crop_originalImageWidth)
		) + "px";
	cropDiv_dotted.style.top =
		Math.round(
			(obj_y.value / 1) * (crop_imageHeight / crop_originalImageHeight)
		) + "px";
	cropDiv_dotted.style.width =
		Math.round(
			(obj_width.value / 1 - cropToolBorderWidth * 2) *
				(crop_imageWidth / crop_originalImageWidth)
		) + "px";
	cropDiv_dotted.style.height =
		Math.round(
			(obj_height.value / 1 - cropToolBorderWidth * 2) *
				(crop_imageHeight / crop_originalImageHeight)
		) + "px";
...
function cropScript_setBasicEvents() {
	...
	document.getElementById(
		"crop_percent_size"
	).onblur = cropScript_validatePercent;
}
...
function crop_initFixedRatio() {
	if (crop_script_fixedRatio > 1) {
		document.getElementById("input_crop_height").value =
			Math.round(document.getElementById("input_crop_width").value) /
			crop_script_fixedRatio;
	} else {
		document.getElementById("input_crop_width").value =
			Math.round(document.getElementById("input_crop_height").value) /
			crop_script_fixedRatio;
	}
