...
if ($action != 'showallbyuser') {
    if ($andor != "exact") {
        $ignored_queries = []; // holds kewords that are shorter than allowed minimum length
        ...
        $queries = [icms_core_DataFilter::addSlashes($query)];
    }
}
...
switch ($action) {
        ...
        foreach ($mids as $mid) {
            ...
            if (in_array($mid, $available_modules)) {
                ...
                if (!is_array($results) || $count == 0) {
                    ...
                    $all_results[$modname] = [
                        "search_more_title" => _SR_SHOWALLR,
    ...
    case 'showallbyuser':
        ...
        if (is_array($results) && $count > 0) {
            ...
            $all_results[$modname] = [
                "results"  =>array_slice($results, $start, $num_show_this_page),
