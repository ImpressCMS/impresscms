...
{
	global $content_content_handler, $xoopsTpl, $xoopsUser;
	...
	if (!$contentObj->isNew()){
		...
		$sform->assign($xoopsTpl, 'content_contentform');
		$xoopsTpl->assign('content_category_path', $contentObj->getVar('content_title') . ' > ' . _EDIT);
	} else {
		...
		$sform->assign($xoopsTpl, 'content_contentform');
		$xoopsTpl->assign('content_category_path', _SUBMIT);
	}
}
...
$tag  = (!empty($tag))?urldecode($tag):null;
...
if (in_array($clean_op,$valid_op,true)){ 
  switch ($clean_op) {
	...
	case "del":
		...
		$xoopsTpl->assign('content_category_path', $content_content_handler->getBreadcrumbForPid($contentObj->getVar('content_id','e'),1) . ' > ' . _DELETE);
	...
	default:
		if (is_object($contentObj) && $contentObj->accessGranted()) {
			...
			$xoopsTpl->assign('content_content', $contentObj->toArray());
			$xoopsTpl->assign('showInfo',$xoopsModuleConfig['show_contentinfo']);

			if ($xoopsModuleConfig['show_breadcrumb']){
			    $xoopsTpl->assign('content_category_path', $content_content_handler->getBreadcrumbForPid($contentObj->getVar('content_id','e'),1));
			}else{
			    $xoopsTpl->assign('content_category_path',false);
			}
		} else {
			redirect_header(CONTENT_URL, 3, _NOPERM);
		}

		if ($xoopsModuleConfig['com_rule'] && $contentObj->getVar('content_cancomment')) {
			$xoopsTpl->assign('content_content_comment', true);
...
$xoopsTpl->assign('content_module_home', content_getModuleName(true, true));
