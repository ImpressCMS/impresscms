...
if ($dbVersion < $newDbVersion) {
    ...
    if ($table->fieldExists('border_color')) {
        $icmsDatabaseUpdater->runQuery($sql = 'ALTER TABLE `' . $table->name() . "` CHANGE border_color color_border varchar(6) NOT NULL default ''", sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    }
    if ($table->fieldExists('background_color')) {
        $icmsDatabaseUpdater->runQuery($sql = 'ALTER TABLE `' . $table->name() . "` CHANGE background_color color_background varchar(6) NOT NULL default ''", sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    }
    if ($table->fieldExists('link_color')) {
        $icmsDatabaseUpdater->runQuery($sql = 'ALTER TABLE `' . $table->name() . "` CHANGE link_color color_link varchar(6) NOT NULL default ''", sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    }
    if ($table->fieldExists('url_color')) {
        $icmsDatabaseUpdater->runQuery($sql = 'ALTER TABLE `' . $table->name() . "` CHANGE url_color color_url varchar(6) NOT NULL default ''", sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    }
    if ($table->fieldExists('text_color')) {
        $icmsDatabaseUpdater->runQuery($sql = 'ALTER TABLE `' . $table->name() . "` CHANGE text_color color_text varchar(6) NOT NULL default ''", sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    }
    unset($table);
    ...
    if ($table->fieldExists('content')) {
        $icmsDatabaseUpdater->runQuery($sql = 'ALTER TABLE `' . $table->name() . '` CHANGE content customtag_content text NOT NULL', sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    }
    unset($table);
    ...
    $configs = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'sourceeditor_default']));
    ...
    $configs = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_HTML_AttrNameUseCDATA']));
    ...
    // retrieve the value of the position before the config to be inserted.
    $configs = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_Filter_YouTube']));
    $p = $configs[0]->getVar('conf_order') + 1;
    //move all the other options down
    ...
    // retrieve the value of the position before the config to be inserted.
    $configs = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_Core_RemoveInvalidImg']));
    $p = $configs[0]->getVar('conf_order') + 1;
    //move all the other options down
    $icmsDatabaseUpdater->runQuery($sql = 'UPDATE `' . $table->name() . '` SET conf_order = conf_order + 1 WHERE conf_order >= ' . $p . ' AND conf_catid = ' . ICMS_CONF_PURIFIER, sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
...
if ($dbVersion < $newDbVersion) {
    ...
    */
    ...
    // retrieve the value of the position before the config to be inserted.
    $configs = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_URI_AllowedSchemes']));
    $p = $configs[0]->getVar('conf_order') + 1;
    ...
    // retrieve the value of the position before the config to be inserted.
    $configs = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_HTML_SafeObject']));
    $p = $configs[0]->getVar('conf_order') + 1;
    //move all the other options down
    $icmsDatabaseUpdater->runQuery($sql = 'UPDATE `' . $table->name() . '` SET conf_order = conf_order + 2 WHERE conf_order >= ' . $p . ' AND conf_catid = ' . ICMS_CONF_PURIFIER, sprintf(_DATABASEUPDATER_MSG_QUERY_SUCCESSFUL, $sql), sprintf(_DATABASEUPDATER_MSG_QUERY_FAILED, $sql));
    ...
    $allowElements = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_HTML_AllowedElements']));
    ...
    $allowAttributes = icms::$config->getConfigs(icms_buildCriteria(['conf_name' => 'purifier_HTML_AllowedAttributes']));
