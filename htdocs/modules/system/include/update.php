...
function xoops_module_update_system(& $module) {
	...
	if ($dbVersion < $newDbVersion) {
		...
		if (getDbValue($icmsDatabaseUpdater->db, 'newblocks', 'show_func', 'show_func="b_social_bookmarks"') == 0) {
			$sql = "SELECT bid FROM `" . $icmsDatabaseUpdater->db->prefix('newblocks') . "` WHERE show_func='b_social_bookmarks'";
			$result = $icmsDatabaseUpdater->db->query($sql);
			list ($new_block_id) = $icmsDatabaseUpdater->db->FetchRow($result);
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("block_module_link") . " VALUES (" . $new_block_id . ", 0, 1);");
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 3, " . $new_block_id . ", 1, 'block_read');");
		}

		if (getDbValue($icmsDatabaseUpdater->db, 'newblocks', 'show_func', 'show_func="b_content_show"') == 0) {
			$sql = "SELECT bid FROM `" . $icmsDatabaseUpdater->db->prefix('newblocks') . "` WHERE show_func='b_content_show'";
			$result = $icmsDatabaseUpdater->db->query($sql);
			list ($new_block_id) = $icmsDatabaseUpdater->db->FetchRow($result);
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("block_module_link") . " VALUES (" . $new_block_id . ", 0, 0);");
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 3, " . $new_block_id . ", 1, 'block_read');");
		}

		if (getDbValue($icmsDatabaseUpdater->db, 'newblocks', 'show_func', 'show_func="b_content_menu_show"') == 0) {
			$sql = "SELECT bid FROM `" . $icmsDatabaseUpdater->db->prefix('newblocks') . "` WHERE show_func='b_content_menu_show'";
			$result = $icmsDatabaseUpdater->db->query($sql);
			list ($new_block_id) = $icmsDatabaseUpdater->db->FetchRow($result);
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("block_module_link") . " VALUES (" . $new_block_id . ", 0, 0);");
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 3, " . $new_block_id . ", 1, 'block_read');");
		...
		if (getDbValue($icmsDatabaseUpdater->db, 'newblocks', 'show_func', 'show_func="b_content_relmenu_show"') == 0) {
			$sql = "SELECT bid FROM `" . $icmsDatabaseUpdater->db->prefix('newblocks') . "` WHERE show_func='b_content_relmenu_show'";
			$result = $icmsDatabaseUpdater->db->query($sql);
			list ($new_block_id) = $icmsDatabaseUpdater->db->FetchRow($result);
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("block_module_link") . " VALUES (" . $new_block_id . ", 0, 0);");
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 3, " . $new_block_id . ", 1, 'block_read');");
		}
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 16, 1, 'system_admin');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 17, 1, 'system_admin');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 18, 1, 'system_admin');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 19, 1, 'system_admin');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 20, 1, 'system_admin');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 1, 1, 'content_admin');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 2, 1, 'group_manager');");
		$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("group_permission") . " VALUES ('', 1, 3, 1, 'group_manager');");
	...
	if ($icmsDatabaseUpdater->dbVersion < $newDbVersion) {
		echo "Database migrate to version " . $newDbVersion . "<br />";
		$icmsDatabaseUpdater->db = $GLOBALS['xoopsDB'];
		if (getDbValue($icmsDatabaseUpdater->db, 'configcategory', 'confcat_name', 'confcat_name="_MD_AM_CAPTCHA"') == 0) {
			$icmsDatabaseUpdater->db->queryF(" INSERT INTO " . $icmsDatabaseUpdater->db->prefix("configcategory") . " (confcat_id,confcat_name) VALUES ('11','_MD_AM_CAPTCHA')");
		}
		...
		$config_id = $icmsDatabaseUpdater->db->getInsertId();

		$sql = "INSERT INTO " . $icmsDatabaseUpdater->db->prefix('configoption') .
		...
		if (!$icmsDatabaseUpdater->db->queryF($sql)) {
			return false;
		}
		...
		$sql2 = "INSERT INTO " . $icmsDatabaseUpdater->db->prefix('configoption') .
		" (confop_id, confop_name, confop_value, conf_id)" .
		" VALUES" .
		...
		if (!$icmsDatabaseUpdater->db->queryF($sql2)) {
			return false;
		}
	...
	if ($icmsDatabaseUpdater->dbVersion < $newDbVersion) {
		echo "Database migrate to version " . $newDbVersion . "<br />";
		$icmsDatabaseUpdater->db->queryF("UPDATE `" . $icmsDatabaseUpdater->db->prefix('config') . "` SET conf_formtype = 'textsarea', conf_valuetype = 'array' WHERE conf_name = 'reg_disclaimer'");
	}
