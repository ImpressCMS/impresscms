...
function editcustomtag($customtagid = 0, $clone = false)
{
    ...
    if (isset($_POST["op"]) && $_POST["op"] == "changedField" && in_array($_POST["changedField"], ["customtag_type"])) {
    ...
    switch ($customtagObj->getVar("customtag_type")) {
        ...
        case ICMS_CUSTOMTAG_TYPE_HTML:
            $customtagObj->setControl("customtag_content", ["name" => "source", "syntax" => "html"]);
            break;
            
        case ICMS_CUSTOMTAG_TYPE_PHP:
            $customtagObj->setControl("customtag_content", ["name" => "source", "syntax" => "php"]);
            break;
...
$valid_op = ["mod", "changedField", "clone", "addcustomtag", "del", ""];
