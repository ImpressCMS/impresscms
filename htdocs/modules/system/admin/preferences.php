...
switch ($op) {
	...
	case 'show':
		...
		icms_cp_header();
		echo '<div class="CPbigTitle" style="background-image: url(' . ICMS_MODULES_URL . '/system/admin/preferences/images/preferences_big.png)"><a href="admin.php?fct=preferences">' . _MD_AM_PREFMAIN . '</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;' . constant($confcat->getVar('confcat_name')) . '<br /><br /></div>';
		$form->display();
		icms_cp_footer();
		break;
	...
	case 'showmod':
		...
		for ($i = 0; $i < $count; $i++) {
			$title =(! defined($config[$i]->getVar('conf_desc')) || constant($config[$i]->getVar('conf_desc')) == '') ? constant($config[$i]->getVar('conf_title')) : constant($config[$i]->getVar('conf_title')) . '<span class="class="helptip"><img src="'. ICMS_IMAGES_SET_URL . '/actions/acp_help.png" alt="' . _MD_AM_HELP_TIP . '" title="' . _MD_AM_HELP_TIP . '" /></span><span class="helptext">' . constant($config[$i]->getVar('conf_desc')) . '</span>';
			switch ($config[$i]->getVar('conf_formtype')) {
				case 'textsarea' :
					if ($config[$i]->getVar('conf_valuetype') == 'array') {
