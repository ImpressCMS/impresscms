<?php
// $Id: main.php 12449 2014-06-18 03:26:05Z skenow $
//  ------------------------------------------------------------------------ //
...
* @version		$Id: main.php 12449 2014-06-18 03:26:05Z skenow $
*/
...
function imanager_index($imgcat_id = NULL) {
	global $icmsAdminTpl, $icmsConfig, $limit, $icmsUser;
	...
	$form->addElement(new XoopsFormSelectGroup(_MD_IMGCATRGRP, 'readgroup', TRUE, ICMS_GROUP_ADMIN, 5, TRUE));
	$form->addElement(new XoopsFormSelectGroup(_MD_IMGCATWGRP, 'writegroup', TRUE, ICMS_GROUP_ADMIN, 5, TRUE));
...
function imanager_listimg($imgcat_id, $start = 0) {
	global $icmsAdminTpl, $query, $limit, $icmsUser;
	...
	if (TRUE !== empty($query)) {
		$criteria->add(new Criteria('image_nicename', '%' . mysql_real_escape_string($query) . '%', 'LIKE'));
	}
...
function imanager_updatecat() {
	...
	global $imgcat_id, $imgcat_name, $imgcat_display, $imgcat_maxsize,
		$imgcat_maxwidth, $imgcat_maxheight, $imgcat_weight, $readgroup, $writegroup;
