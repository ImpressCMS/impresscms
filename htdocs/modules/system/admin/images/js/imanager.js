	...
	if (!type) {
		...
		if (target == "src") {
			if (win.document.getElementById("title"))
				window.opener.document.getElementById("title").value = name;
			if (win.document.getElementById("alt"))
				window.opener.document.getElementById("alt").value = name;
			if (win.XoopsimagemanagerDialog.showPreviewImage)
				window.opener.XoopsimagemanagerDialog.showPreviewImage(itemurl);
		}
	}
	window.close();
	return;
...
function filter_preview(id, xurl, img, width, height) {
	...
	var url =
		xurl +
		"/modules/system/admin/images/preview.php?file=" +
		img +
		"&filter=" +
		filter;
	...
	window.open(
		url,
		"preview_image",
		"width=" + (width + 20) + ",height=" + (height + 20) + ",resizable=yes"
	);
}
