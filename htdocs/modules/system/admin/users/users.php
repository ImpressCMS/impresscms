...
function displayUsers()
{
    ...
    $op_select->addOptionArray(['modifyUser' =>_AM_MODIFYUSER, 'delUser' =>_AM_DELUSER]);
    ...
    $op_select = new icms_form_elements_Select('', 'op');
    $op_select->addOptionArray(['modifyUser' =>_AM_MODIFYUSER]);
...
function modifyUser($user)
{
    ...
    if (is_object($user)) {
        if (!$user->isActive()) {
            icms_core_Message::confirm(['fct' => 'users', 'op' => 'reactivate', 'uid' => $user->getVar('uid')], 'admin.php', _AM_NOTACTIVE);
            icms_cp_footer();
            exit();
        }
...
function synchronize($id, $type)
{
    switch ($type) {
        case 'user':
            ...
            $tables[] = ['table_name' => 'xoopscomments', 'uid_column' => 'com_uid', 'criteria' => new icms_db_criteria_Item('com_status', XOOPS_COMMENT_ACTIVE)];
            // Count forum posts
