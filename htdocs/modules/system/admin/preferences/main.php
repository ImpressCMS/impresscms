    ...
    case 'show':
        ...
        for ($i = 0; $i < $confcount; $i++) {
            ...
            switch ($config[$i]->getVar('conf_formtype')) {
                ...
                case 'select_font':
                    $ele = new icms_form_elements_Select($title, $config[$i]->getVar('conf_name'), $config[$i]->getConfValueForOutput());
                    $dirlist = icms_core_Filesystem::getFileList(ICMS_LIBRARIES_PATH . '/icms/form/elements/captcha/fonts/', '', ['ttf']);
                    if (! empty($dirlist)) {
                        asort($dirlist);
                        $ele->addOptionArray($dirlist);
                    }
                ...
                case 'select_geshi':
                    $ele = new icms_form_elements_Select($title, $config[$i]->getVar('conf_name'), $config[$i]->getConfValueForOutput());
                    $dirlist = str_replace('.php', '', icms_core_Filesystem::getFileList(ICMS_LIBRARIES_PATH . '/geshi/geshi/', '', ['php']));
                    if (! empty($dirlist)) {
                        asort($dirlist);
                        $ele->addOptionArray($dirlist);
                    }
    ...
    case 'save':
        ...
        if ($count > 0) {
            for ($i = 0; $i < $count; $i++) {
                ...
                icms::$preload->triggerEvent('savingSystemAdminPreferencesItem', [(int) $config->getVar('conf_catid'), $config->getVar('conf_name'), $config->getVar('conf_value')]);
                ...
                if (!isset($saved_config_items[$config->getVar('conf_catid')])) {
                    $saved_config_items[$config->getVar('conf_catid')] = [];
                }
                $saved_config_items[$config->getVar('conf_catid')][$config->getVar('conf_name')] = [$old_value, $config->getVar('conf_value')];
            }
        }
        ...
        register_shutdown_function([&$xoopsTpl, 'clear_all_cache']);
