...
if (!empty($_POST)) {
    ...
    if (isset($_POST['filter_get'])) {
        unset($$_POST['filter_get']);
    }
...
if ($op == "form") {
    ...
    $mailok_radio->addOptionArray(["mailok" =>_AM_MAILOK, "mailng" =>_AM_MAILNG, "both" =>_AM_BOTH]);
    $type_radio = new icms_form_elements_Radio(_AM_SHOWTYPE, "user_type", "actv");
    $type_radio->addOptionArray(["actv" =>_AM_ACTIVE, "inactv" =>_AM_INACTIVE, "both" =>_AM_BOTH]);
    $sort_select = new icms_form_elements_Select(_AM_SORT, "user_sort");
    $sort_select->addOptionArray(["uname" =>_AM_UNAME, "login_name" =>_AM_LOGINNAME, "email" =>_AM_EMAIL, "last_login" =>_AM_LASTLOGIN, "user_regdate" =>_AM_REGDATE, "posts" =>_AM_POSTS]);
    $order_select = new icms_form_elements_Select(_AM_ORDER, "user_order");
    $order_select->addOptionArray(["ASC" =>_AM_ASC, "DESC" =>_AM_DESC]);
    ...
    $groups = empty($selgroups) ? [] : array_map('intval', $selgroups);
    $validsort = ["uname", "login_name", "email", "last_login", "user_regdate", "posts"];
