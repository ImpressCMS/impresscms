            ...
            if (!$module_handler->insert($module)) {
                ...
                if ($templates !== false) {
                    ...
                    foreach ($templates as $tpl) {
                        ...
                        if (!$tplfile_handler->insert($tplfile)) {
                            $msgs[] = sprintf(
                                '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_INSERT_FAIL . '</span>',
                                '<strong>' . $tpl['file'] . '</strong>'
                            );
                        } else {
                            $newtplid = $tplfile->getVar('tpl_id');
                            $msgs[] = sprintf(
                                '&nbsp;&nbsp;' . _MD_AM_TEMPLATE_INSERTED,
                                '<strong>' . $tpl['file'] . '</strong>',
                                '<strong>' . $newtplid . '</strong>'
                            );

                            // generate compiled file
                            if (!$icmsAdminTpl->template_touch($newtplid)) {
                                $msgs[] = sprintf(
                                    '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_COMPILE_FAIL . '</span>',
                                    '<strong>' . $tpl['file'] . '</strong>',
                                    '<strong>' . $newtplid . '</strong>'
                                );
                            } else {
                ...
                if ($blocks !== false) {
                    ...
                    foreach ($blocks as $blockkey => $block) {
                        ...
                        if (!$db->query($sql)) {
                            $msgs[] = sprintf(
                                '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCKS_ADD_FAIL . '</span>',
            ...
            foreach ($groups as $mygroup) {
                ...
                foreach ($blocks as $blc) {
                    ...
                    if (!$gperm_handler->insert($bperm)) {
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCK_ACCESS_FAIL . '</span>',
                            '<strong>' . icms_conv_nr2local($blc) . '</strong>',
                            '<strong>' . icms_conv_nr2local($mygroup) . '</strong>'
                        );
                    } else {
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;' . _MD_AM_BLOCK_ACCESS_ADDED,
                            '<strong>' . icms_conv_nr2local($blc) . '</strong>',
                            '<strong>' . icms_conv_nr2local($mygroup) . '</strong>'
                        );
                    }
...
function xoops_module_uninstall($dirname)
{
    ...
    if ($module->getVar('dirname') == 'system') {
        ...
        if ($pages > 0) {
            ...
            foreach ($pages as $page) {
                if (!$page_handler->delete($page)) {
                    $msgs[] = sprintf(
                        '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_SYMLINK_DELETE_FAIL . '</span>',
                        $page->getVar('page_title'),
                        '<strong>'. $page->getVar('page_id') . '</strong>'
                    );
                } else {
                    $msgs[] = sprintf(
                        '&nbsp;&nbsp;' . _MD_AM_SYMLINK_DELETED,
                        '<strong>' . $page->getVar('page_title') . '</strong>',
                        '<strong>' . $page->getVar('page_id') . '</strong>'
                    );
                }
            }
        ...
        if (!$module_handler->delete($module)) {
            ...
            if ($tcount > 0) {
                ...
                for ($i = 0; $i < $tcount; $i++) {
                    if (!$tplfile_handler->delete($templates[$i])) {
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_DELETE_FAIL . '</span>',
            ...
            if (is_array($block_arr)) {
                ...
                for ($i = 0; $i < $bcount; $i++) {
                    if (!$icms_block_handler->delete($block_arr[$i])) {
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCK_DELETE_FAIL . '</span>',
                        ...
                        if ($btcount > 0) {
                            for ($j = 0; $j < $btcount; $j++) {
                                if (!$tplfile_handler->delete($templates[$j])) {
                                    $msgs[] = sprintf(
                                        '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_BLOCK_TMPLT_DELETE_FAILED . '</span>',
            ...
            if ($modtables !== false && is_array($modtables)) {
                ...
                foreach ($modtables as $table) {
                    ...
                    if (!in_array($table, $reservedTables)) {
                        ...
                        if (!$db->query($sql)) {
                            $msgs[] = sprintf(
                                '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_MOD_TABLE_DELETE_FAIL . '</span>',
                        ...
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_MOD_TABLE_DELETE_NOTALLOWED . '</span>',
                            '<strong>' . $db->prefix($table) . '</strong>'
                            );
                    }
                }
                ...
                if ($confcount > 0) {
                    ...
                    for ($i = 0; $i < $confcount; $i++) {
                        if (!$config_handler->deleteConfig($configs[$i])) {
                            $msgs[] = sprintf(
                                '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_CONFIGOPTION_DELETE_FAIL .'</span>',
            ...
            $urllink_handler->deleteAll(icms_buildCriteria(["mid" => $module->getVar("mid")]));

            // delete files
            $file_handler = icms::handler('icms_data_file');
            $file_handler->deleteAll(icms_buildCriteria(["mid" => $module->getVar("mid")]));
                            ...
                            if ($template != '') {
                                ...
                                if (!$tplfile_handler->insert($tplfile)) {
                                    ...
                                    if ($icmsConfig['template_set'] == 'default') {
                                        if (!$icmsAdminTpl->template_touch($newid)) {
                                            $msgs[] = sprintf(
                                                '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_TEMPLATE_RECOMPILE_FAIL . '</span>',
                                                '<strong>' . $blocks[$i]['template'] . '</strong>'
                                            );
                                        } else {
                                            $msgs[] = sprintf('&nbsp;&nbsp;' . _MD_AM_TEMPLATE_RECOMPILED, '<strong>' . $blocks[$i]['template'] . '</strong>');
            ...
            foreach ($configs as $config) {
                ...
                if (!in_array($config['name'], $config_delng)) {
                    ...
                    if (false !== $config_handler->insertConfig($confobj)) {
                        ...
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_CONFIG_ADD_FAIL . '</span>',
        ...
        if (isset($atasks) && is_array($atasks) && (count($atasks) > 0)) {
            ...
            if (is_array($atasks)) {
                foreach ($atasks as $taskID => $taskData) {
                    ...
                    if (!($atasks_handler->insert($task))) {
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;<span style="color:#ff0000;">' . _MD_AM_AUTOTASK_FAIL . '</span>',
                            '<strong>' . $taskData['name'] . '</strong>'
                        );
                    } else {
                        $msgs[] = sprintf(
                            '&nbsp;&nbsp;' . _MD_AM_AUTOTASK_ADDED,
                            '<strong>' . $taskData['name'] . '</strong>'
                        );
                    }
                }
