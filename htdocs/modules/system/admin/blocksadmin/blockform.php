...
if(isset($block['name'])) {$form->addElement(new XoopsFormLabel(_AM_NAME, $block['name']));}
...
else
{
	...
	if($block['edit_form'] != false) {$form->addElement(new XoopsFormLabel(_AM_OPTIONS, $block['edit_form']));}
}
...
if(isset($block['bid'])) {$form->addElement(new XoopsFormHidden('bid', $block['bid']));}
...
else {$btn->setExtra('onclick="document.getElementById(\'new\').style.display = \'none\'; return false;"');}
