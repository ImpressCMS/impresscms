...
class SystemBlocksadmin extends icms_view_block_Object
{
    ...
    {
        if ($format == 's' && in_array($key, ['visible', 'mid', 'side'])) {
...
class SystemBlocksadminHandler extends icms_view_block_Handler
{
    ...
    {
        return ['H' => _AM_HTML, 'P' => _AM_PHP, 'S' => _AM_AFWSMILE, 'T' => _AM_AFNOSMILE];
    }

    public function getBlockCacheTimeArray()
    {
        $rtn = ['0' => _NOCACHE, '30' => sprintf(_SECONDS, 30), '60' => _MINUTE, '300' => sprintf(_MINUTES, 5), '1800' => sprintf(_MINUTES, 30), '3600' => _HOUR, '18000' => sprintf(_HOURS, 5), '86400' => _DAY, '259200' => sprintf(_DAYS, 3), '604800' => _WEEK, '2592000' => _MONTH];
        return $rtn;
    }
