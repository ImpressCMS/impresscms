...
/**
 * Handler for analytics object
 * @package		Administration
 * @subpackage	analytics
 */
class mod_system_analyticsHandler extends icms_ipf_Handler {
	...
	public function __construct(&$db) {
		parent::__construct($db, 'analytics', 'analyticsid', 'tag', 'description', 'system');
		$this->buildFormats();
	}

	/**
	 * Create a list of formats for the analytics units
	 */
	...
	/**
	 * Action to take before the analytics object is saved
	 * @param	object 	$obj
	 * @return	boolean
	 */
	...
	/**
	 * Retrieve an analytics unit by its tag
	 * @return	array	Object array 
	 */
	public function getanalyticssByTag() {
		if (!$this->_objects) {
			$analyticssObj = $this->getObjects(NULL, TRUE);
			$ret = array();
			foreach ($analyticssObj as $analyticsObj) {
				$ret[$analyticsObj->getVar('tag')] = $analyticsObj;
			}
			$this->_objects = $ret;
		}
