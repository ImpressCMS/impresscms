        ...
        if (icms::$xoopsDB->getRowsNum($result) > 0) {
            ...
            while ($userinfo = icms::$xoopsDB->fetchArray($result)) {
                ...
                if (isset(icms::$user) && is_object(icms::$user)) {
                    $block['groups'][$i]['users'][] = ['id' => $userinfo['uid'], 'name' => icms_core_DataFilter::htmlSpecialChars($userinfo['uname']), 'msglink' => "<a href=\"javascript:openWithSelfMain('" . ICMS_URL . '/pmlite.php?send2=1&amp;to_userid='
                                                                                                                                                                    . $userinfo['uid'] . "','pmlite',800,680);\"><img src=\"" . ICMS_URL . '/images/icons/'
                                                                                                                                                                    . $GLOBALS['icmsConfig']['language'] . '/pm_small.gif" width="27px" height="17px" alt="" /></a>', 'avatar' => ICMS_UPLOAD_URL . '/' . $userinfo['user_avatar']];
                } else {
                    if ($userinfo['user_viewemail']) {
                        $block['groups'][$i]['users'][] = ['id' => $userinfo['uid'], 'name' => icms_core_DataFilter::htmlSpecialChars($userinfo['uname']), 'msglink' => '<a href="mailto:' . $userinfo['email'] . '"><img src="' . ICMS_URL . '/images/icons/' . $GLOBALS['icmsConfig']['language'] . '/em_small.gif" width="16px" height="14px" alt="" /></a>', 'avatar' => ICMS_UPLOAD_URL . '/' . $userinfo['user_avatar']];
                    } else {
                        $block['groups'][$i]['users'][] = ['id' => $userinfo['uid'], 'name' => icms_core_DataFilter::htmlSpecialChars($userinfo['uname']), 'msglink' => '&nbsp;', 'avatar' => ICMS_UPLOAD_URL . '/' . $userinfo['user_avatar']];
                    }
                }
                $i++;
            }
...
function b_system_comments_show($options)
{
    ...
    $moduleperm_handler = icms::handler('icms_member_groupperm');
    $gperm_groupid = is_object(icms::$user) ? icms::$user->getGroups() : [XOOPS_GROUP_ANONYMOUS];
...
function b_system_topposters_edit($options)
{
    ...
    $ranks = icms_getModuleHandler('userrank', 'system')->getList(icms_buildCriteria(['rank_special' => '1']));
    $size = count($options);
...
function b_system_themes_show($options)
{
    ...
    if ($options[0] == 1) {
        $block['theme_select'] = '<img vspace="2" id="xoops_theme_img" src="'
                                 . ICMS_THEME_URL . '/'
                                 . $icmsConfig['theme_set'] . '/shot.gif" alt="screenshot" width="'
                                 . (int)$options[1] . "\" /><br /><select id=\"theme_select\" name=\"theme_select\" onchange=\"showImgSelected('xoops_theme_img', 'theme_select', 'themes', '/shot.gif', '" . ICMS_URL . "');\">" . $theme_options . '</select><input type="submit" value="'
                                 . _GO . '" />';
    } else {
    ...
    foreach ($notifications as $n) {
        ...
        if ($item != $prev_item) {
            ...
            if (!empty($lookup_func)) {
                ...
                $item_info = ['name' => '[' . _NOT_NAMENOTAVAILABLE . ']', 'url' =>''];
            }
        }

        if ($n->getVar('not_event') == 'bookmark') {
            $block[$module_name][] = ['name' => $item_info['name'], 'url' => $item_info['url']];
        }
    }
