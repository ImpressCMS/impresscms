...
function b_system_admin_warnings_show()
{
    ...
    if (version_compare(PHP_VERSION, '7.2', '<')) {
        array_push($block['msg'], icms_core_Message::warning(sprintf(_OUTDATED_PHP, PHP_VERSION)), '', false);
    }
    ...
    foreach ($dirlist as $file) {
        ...
        if ($modversion['hasAdmin']) {
            ...
            if (false != $all_ok || in_array($modversion['category'], $ok_syscats)) {
                $sysmod = ['title' => $modversion['name'], 'link' => ICMS_MODULES_URL . '/system/admin.php?fct=' . $file, 'image' => ICMS_MODULES_URL . '/system/admin/' . $file . '/images/' . $file . '_big.png'];
...
function b_system_admin_modules_show()
{
    ...
    foreach ($modules as $module) {
        ...
        if ((isset($hasconfig) && $hasconfig == 1) || (isset($hascomments) && $hascomments == 1)) {
            ...
            $subs = ['title' => _PREFERENCES, 'link' => ICMS_MODULES_URL . '/system/admin.php?fct=preferences&amp;op=showmod&amp;mod=' . $module->getVar('mid')];
            $rtn['subs'][] = $subs;
        } else {
            $rtn['hassubs'] = 0;
            unset($rtn['subs']);
