...
function editvideos($videos_id = 0)
{
	...
	if (!$videosObj->isNew()){
		$icmsModule->displayAdminMenu(10, _AM_PROFILE_VIDEOS . " > " . _CO_ICMS_EDITING);
		$sform = $videosObj->getForm(_AM_PROFILE_VIDEOS_EDIT, 'addvideos');
		$sform->assign($icmsAdminTpl);

	} else {
		$icmsModule->displayAdminMenu(10, _AM_PROFILE_VIDEOS . " > " . _CO_ICMS_CREATINGNEW);
...
if (in_array($clean_op,$valid_op,true)){
  switch ($clean_op) {
  	...
  	default:
  		...
  		$icmsModule->displayAdminMenu(10, _AM_PROFILE_VIDEOS);
