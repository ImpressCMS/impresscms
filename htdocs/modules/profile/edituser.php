...
if ($op == 'editprofile') {
    ...
    include_once 'include/forms.php';
    echo '<a href="'.ICMS_URL.'/modules/'.basename( dirname( __FILE__ ) ).'/userinfo.php?uid='.$xoopsUser->getVar('uid').'">'. _PROFILE_MA_PROFILE .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'. _PROFILE_MA_EDITPROFILE .'<br /><br />';
...
if ($op == 'delete') {
	$config_handler =& xoops_gethandler('config');
	$icmsConfigUser =& $config_handler->getConfigsByCat(XOOPS_CONF_USER);
    if (!$xoopsUser || $icmsConfigUser['self_delete'] != 1) {
...
if ($op == 'avatarform') {
    ...
    include ICMS_ROOT_PATH.'/header.php';
    echo '<a href="'.ICMS_URL.'/modules/'.basename( dirname( __FILE__ ) ).'/userinfo.php?uid='.$xoopsUser->getVar('uid').'">'. _PROFILE_MA_PROFILE .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'. _PROFILE_MA_UPLOADMYAVATAR .'<br /><br />';
    ...
    if ($icmsConfigUser['avatar_allow_upload'] == 1 && $xoopsUser->getVar('posts') >= $icmsConfigUser['avatar_minposts']) {
        include_once ICMS_ROOT_PATH.'/class/xoopsformloader.php';
