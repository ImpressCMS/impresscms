...
function bannerstats()
{
    ...
    $result = icms::$xoopsDB->query(sprintf(
        "SELECT cid, name, passwd FROM %s WHERE login=%s",
    ...
    if ($_SESSION['banner_pass'] == $passwd) {
        ...
        $result = icms::$xoopsDB->query(
            "select bid, imptotal, impmade, clicks, date from
        ...
        $result = icms::$xoopsDB->query(
            "select bid, imageurl, clickurl, htmlbanner, htmlcode from
										" . icms::$xoopsDB->prefix('banner') . " where cid='". (int) ($cid) . "'"
                                    );
...
function EmailStats($cid, $bid)
{
    ...
    if ($_SESSION['banner_login'] != "" && $_SESSION['banner_pass'] != "") {
        $cid = (int) ($cid);
        $bid = (int) ($bid);
        if ($result2 = icms::$xoopsDB->query(sprintf(
            "SELECT name, email, passwd FROM %s WHERE cid='%u' AND login=%s",
            icms::$xoopsDB->prefix('bannerclient'),
            $cid,
            icms::$xoopsDB->quoteString($_SESSION['banner_login'])
        ))) {
...
function change_banner_url_by_client($cid, $bid, $url)
{
    if ($_SESSION['banner_login'] != "" && $_SESSION['banner_pass'] != "" && $url != "") {
        ...
        $bid = (int) $bid;
        $sql = sprintf(
            "SELECT passwd FROM %s WHERE cid='%u' AND login=%s",
            icms::$xoopsDB->prefix('bannerclient'),
            $cid,
            icms::$xoopsDB->quoteString($_SESSION['banner_login'])
                    );
        if ($result = icms::$xoopsDB->query($sql)) {
            ...
            if ($_SESSION['banner_pass'] == $passwd) {
                $sql = sprintf(
                    "UPDATE %s SET clickurl=%s WHERE bid='%u' AND cid='%u'",
                    icms::$xoopsDB->prefix('banner'),
...
switch ($op) {
    ...
    case _BANNERS_CHANGE:
        ...
        $bid = $cid = 0;
        if (!empty($_POST['url'])) {
            $url = icms_core_DataFilter::stripslashesGPC(trim($_POST['url']));
        }
        if (!empty($_POST['bid'])) {
            $bid = (int) $_POST['bid'];
        }
