...
class icms_AutologinEventHandler
{
    ...
    {
        icms_Event::attach('icms_core_Session', 'sessionStart', [__CLASS__, 'onSessionStart']);
        icms_Event::attach('icms_core_Session', 'sessionClose', [__CLASS__, 'onSessionClose']);
    }
    ...
    public static function sessionAutologin($autologinName, $autologinPass)
    {
        // autologin V2 GIJ
        if (!empty($_POST)) {
        ...
        if (false != $user && $user->getVar('level') > 0) {
            ...
            setcookie(
                'autologin_pass',
                $Ynj . ':' . md5($user->getVar('pass') . ICMS_DB_PASS . ICMS_DB_PREFIX . $Ynj),
                $expire,
                $icms_cookie_path,
                '',
                $secure,
