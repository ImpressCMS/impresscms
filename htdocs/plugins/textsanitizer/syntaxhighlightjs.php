...
function textsanitizer_syntaxhighlightjs(&$ts, &$source, $language )
{
	$source = MyTextSanitizer::undoHtmlSpecialChars($source);
	$source = _textsanitizer_geshi_js_highlight( $source, $language );
	return $source;
}


function _textsanitizer_geshi_js_highlight( $source, $language )
{
