...
class IcmsAutoTasksCron extends icms_sys_autotasks_System
{
    ...
    public function start(int $interval)
    {
        ...
        $this->_lines[$this->_line_id] = [$arx, 4];
        $this->writeCronTab();
        return true;
    }
    ...
    public function &getIntervalArray($interval)
    {
        $hours = $days = $months = 0;
        if ($interval>60) {
            $minutes   = $interval % 60;
        ...
        $rez = ['minute' => $this->getCronTabValue($minutes), 'hour' => $this->getCronTabValue($hours), 'dayofmonth' => $this->getCronTabValue($days), 'month' => $this->getCronTabValue($months), 'dayofweek' => '*'];
        return $rez;
    }

    public function getCronTabValue($number)
    {
        if ($number == 0) {
            return '*';
        }
    ...
    public function getNormalValue($crontab_number)
    {
        if ($crontab_number == '*') {
            return 0;
        }
    ...
    public function getCommandLine()
    {
        ...
        $autotasks_helper_path = $config_atasks['autotasks_helper_path'].str_replace(['%path%', '%url%'], [ICMS_ROOT_PATH, ICMS_URL], trim($config_atasks['autotasks_helper'])) . '/include/autotasks.php > /dev/null';
    ...
    public function getCronTabUser()
    {
        static $user = null;
        if ($user === null) {
            $atasks_handler = &icms_getModuleHandler('autotasks', 'system');
    ...
    public function readCronTab()
    {
        ...
        foreach ($crons as $line) {
            ...
            if (preg_match('/(.*)=(.*)/', $line, $assign)) {
                $this->_lines[] = [['name' => $assign[1], 'value' => $assign[2]], 2];
                continue;
            }
            // Checking if this is a special -entry. check man 5 crontab for more info
            if ($line[0] == '@') {
                $this->_lines[] = [preg_split("/[ \t]/", $line, 2), 3];
                continue;
            }
            // It's a regular crontab-entry
            $ct = preg_split("/[ \t]/", $line, 6);
            $this->_lines[] = [['minute' => $ct[0], 'hour' => $ct[1], 'dayofmonth' => $ct[2], 'month' => $ct[3], 'dayofweek' => $ct[4], 'command' => $ct[5]], 4];
        }
