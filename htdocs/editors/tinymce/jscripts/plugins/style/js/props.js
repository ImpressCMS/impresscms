			...
			for (var name in parsedStyles) {
				if (parsedStyles.hasOwnProperty(name)) {
					if (mergedStyles[name] === undefined) {
						...
						if (
							mergedStyles[name].indexOf(parsedStyles[name]) ===
							-1
						) {
							mergedStyles[name] =
								mergedStyles[name] + " " + parsedStyles[name];
...
function init(ed) {
	...
	document.getElementById(
		...
		"background_color_pick",
		"background_color"
	);
	document.getElementById(
		"border_color_top_pickcontainer"
	).innerHTML = getColorPickerHTML(
		"border_color_top_pick",
		"border_color_top"
	);
	document.getElementById(
...
function setupFormData() {
	...
	selectByValue(
		f,
		"text_size_measurement",
		getMeasurement(ce.style.fontSize)
	);
...
function applyAction() {
	...
	if (applyActionIsInsert) {
		...
		ed.dom.setAttrib(
			nodes,
			"style",
			tinyMCEPopup.editor.dom.serializeStyle(newStyles)
		);
	}
}
	...
	for (i = 0; i < f.length; i++) {
		for (a = 0; a < f[i].elements.length; a++) {
			if (f[i].elements[a].disabled)
				...
				tinyMCEPopup.editor.dom.removeClass(
					f[i].elements[a],
					"disabled"
				);
		}
