var XoopsimagemanagerDialog = {
	...
	init: function(ed) {
		...
		if (n.nodeName == "IMG") {
			...
			if (
				/^\s*this.src\s*=\s*\'([^\']+)\';?\s*$/.test(
					dom.getAttrib(n, "onmouseover")
				)
			)
				nl.onmouseoversrc.value = dom
					.getAttrib(n, "onmouseover")
					.replace(/^\s*this.src\s*=\s*\'([^\']+)\';?\s*$/, "$1");

			if (
				/^\s*this.src\s*=\s*\'([^\']+)\';?\s*$/.test(
					dom.getAttrib(n, "onmouseout")
				)
			)
				nl.onmouseoutsrc.value = dom
					.getAttrib(n, "onmouseout")
					.replace(/^\s*this.src\s*=\s*\'([^\']+)\';?\s*$/, "$1");
		...
		if (tinyMCEPopup.getParam("accessibility_warnings", 1)) {
			if (!f.alt.value) {
				tinyMCEPopup.editor.windowManager.confirm(
					tinyMCEPopup.getLang("xoopsimagemanager_dlg.missing_alt"),
					function(s) {
	...
	insertAndClose: function() {
		...
		if (f.onmousemovecheck.checked) {
			if (nl.onmouseoversrc.value)
				args.onmouseover =
					"this.src='" + nl.onmouseoversrc.value + "';";
	...
	changeHeight: function() {
		...
		tp =
			(parseInt(f.width.value) / parseInt(t.preloadImg.width)) *
			t.preloadImg.height;
	...
	changeWidth: function() {
		...
		tp =
			(parseInt(f.height.value) / parseInt(t.preloadImg.height)) *
			t.preloadImg.width;
	...
	updateStyle: function(ty) {
		...
		if (tinyMCEPopup.editor.settings.inline_styles) {
			...
			// Merge
			dom.get("style").value = dom.serializeStyle(
				dom.parseStyle(img.style.cssText)
			);
		}
	},
	...
	showPreviewImage: function(u, st) {
		...
		if (
			!st &&
			tinyMCEPopup.getParam(
				"xoopsimagemanager_update_dimensions_onchange",
				true
			)
		)
			this.resetImageData();
		...
		if (!st)
			tinyMCEPopup.dom.setHTML(
				"prev",
				'<img id="previewImg" src="' +
					u +
					'" border="0" onload="XoopsimagemanagerDialog.updateImageData(this);" onerror="XoopsimagemanagerDialog.resetImageData();" />'
			);
		else
			tinyMCEPopup.dom.setHTML(
				"prev",
				'<img id="previewImg" src="' +
					u +
					'" border="0" onload="XoopsimagemanagerDialog.updateImageData(this, 1);" />'
			);
	}
};
