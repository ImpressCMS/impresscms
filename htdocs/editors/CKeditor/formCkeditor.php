...
class icmsFormCKEditor extends icms_form_elements_Textarea {
	...
	public function render() {
		...
		// CSRF token for AJAX: createToken() already returns the hashed token expected by validateToken()
		$__tokenVal = icms::$security->createToken(0);

		// Build JS with real newlines to avoid invalid escape sequences in the browser.
		$textareaId = $this->_name . '_tarea';
		$icmsUrl = ICMS_URL;
		$script = <<<JS
var config = {
  filebrowserImageBrowseUrl: "{$icmsUrl}/editors/CKeditor/imagebrowser.php",
  toolbar: "{$toolbar}",
...
$(function() {
  ...
  $("#{$textareaId}").parents("form").submit(function() {
    var data = $("#{$textareaId}").html();
    $("#{$textareaId}").html(data);
  });
});
JS;
		$ret .= $xoTheme->addScript('', array('type' => 'text/javascript'), $script);
		$ret .= parent::render();
