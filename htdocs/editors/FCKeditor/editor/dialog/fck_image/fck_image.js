...
var bImageButton =
	...
	document.location.search.substr(1) == "ImageButton";
...
if (FCKConfig.ImageUpload) dialog.AddTab("Upload", FCKLang.DlgLnkUpload);
...
function LoadSelection() {
	...
	if (sUrl == null) sUrl = GetAttribute(oImage, "src", "");
	...
	GetE("txtHeight").value = iHeight
		? iHeight
		: GetAttribute(oImage, "height", "");
	...
	if (oLink) {
		...
		if (sLinkUrl == null) sLinkUrl = oLink.getAttribute("href", 2);
...
function Ok() {
	...
	if (sLnkUrl.length == 0) {
		...
		// Creating a new link.
			...
			if (!bHasImage) oEditor.FCKSelection.SelectNode(oImage);
...
function UpdateImage(e, skipId) {
	...
	if (!skipId) SetAttribute(e, "id", GetE("txtAttId").value);
...
function CheckUpload() {
	...
	if (
		(FCKConfig.ImageUploadAllowedExtensions.length > 0 &&
			!oUploadAllowedExtRegex.test(sFile)) ||
		(FCKConfig.ImageUploadDeniedExtensions.length > 0 &&
