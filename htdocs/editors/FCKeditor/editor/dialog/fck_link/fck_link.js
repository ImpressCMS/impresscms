...
if (FCKConfig.LinkUpload) dialog.AddTab("Upload", FCKLang.DlgLnkUpload, true);
	...
	if (aLinkInfo && aLinkInfo[1] == "mailto") {
		...
		if (func != null) {
			try {
				...
				aLinkInfo[2] = aLinkInfo[2].replace(
					/\\'/g,
					"###SINGLE_QUOTE###"
				);
...
oParser.CreateEMailUri = function(address, subject, body) {
	...
	if (subject.length > 0) sParams = "?subject=" + encodeURIComponent(subject);
...
function LoadSelection() {
	...
	if (oEMailInfo) {
		...
		sType = "anchor";
		GetE("cmbAnchorName").value = GetE("cmbAnchorId").value = sHRef.substr(
			1
		);
	...
	if (!oPopupMatch) {
		...
		if (sTarget && sTarget.length > 0) {
			...
			} else GetE("cmbTarget").value = "frame";
...
function SetLinkType(linkType) {
	...
	if (!FCKConfig.LinkDlgHideAdvanced)
		dialog.SetTabVisibility(
			"Advanced",
			linkType != "anchor" || bHasAnchors
		);

	if (linkType == "email") dialog.SetAutoSize(true);
}

//#### Target type selection.
function SetTarget(targetType) {
	GetE("tdTargetFrame").style.display = targetType == "popup" ? "none" : "";
	GetE("tdPopupName").style.display = GetE(
		"tablePopupFeatures"
	).style.display = targetType == "popup" ? "" : "none";
	...
	if (targetType == "popup") dialog.SetAutoSize(true);
}
...
function OnTargetNameChange() {
	...
	if (sFrame.length == 0) GetE("cmbTarget").value = "";
...
function BuildOnClickPopup() {
	var sWindowName =
		"'" + GetE("txtPopupName").value.replace(/\W/gi, "") + "'";
...
function FillPopupFields(windowName, features) {
	if (windowName) GetE("txtPopupName").value = windowName;

	var oFeatures = new Object();
	var oFeaturesMatch;
	while ((oFeaturesMatch = oRegex.PopupFeatures.exec(features)) != null) {
		var sValue = oFeaturesMatch[2];
		if (sValue == ("yes" || "1")) oFeatures[oFeaturesMatch[1]] = true;
	...
	var aChkFeatures = document.getElementsByName("chkFeature");
	for (var i = 0; i < aChkFeatures.length; i++) {
		if (oFeatures[aChkFeatures[i].value]) aChkFeatures[i].checked = true;
	}
...
function Ok() {
	...
	for (var i = 0; i < aLinks.length; i++) {
		...
		if (aHasSelection) sInnerHtml = oLink.innerHTML; // Save the innerHTML (IE changes it if it is like an URL).
		...
		else SetAttribute(oLink, "target", null);

		// Let's set the "id" only for the first link to avoid duplication.
		if (i == 0) SetAttribute(oLink, "id", GetE("txtAttId").value);
		...
		if (oEditor.FCKBrowserInfo.IsIE) {
			...
			if (GetE("txtAttName").value.length != 0) sClass += " FCK__AnchorC";
...
function CheckUpload() {
	...
	if (
		(FCKConfig.LinkUploadAllowedExtensions.length > 0 &&
...
function SetDefaultTarget() {
	...
	if (oLink || target.length == 0) return;
