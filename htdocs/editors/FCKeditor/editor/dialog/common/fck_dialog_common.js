...
function CreateNamedElement(oEditor, oOriginal, nodeName, oAttributes) {
	...
	if (oOriginal && oEditor.FCKBrowserInfo.IsIE) {
		...
		for (var attName in oAttributes)
			bChanged |=
				oOriginal.getAttribute(attName, 2) != oAttributes[attName];
	...
	if (oOriginal) {
		...
		if (oEditor.FCKBrowserInfo.IsIE) {
			...
			oNewNode = oEditor.FCK.EditorDocument.createElement(
				sbHTML.join("")
			);
...
function CopyAttributes(oSource, oDest, oSkipAttributes) {
	...
	for (var n = 0; n < aAttributes.length; n++) {
		...
		if (oAttribute.specified) {
			...
			if (sAttValue == null) sAttValue = oAttribute.nodeValue;
...
function RenameNode(oNode, newTag) {
	...
	if (oNode.nodeType != 1) return null;

	// If it's already correct exit here.
	if (oNode.nodeName == newTag) return oNode;
