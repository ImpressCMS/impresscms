...
function openChecker() {
	this.spellCheckerWin = window.open(
		this.popUpUrl,
		this.popUpName,
		this.popUpProps
	);
	...
	for (var i = ti; i < this.wordWin.textInputs.length; i++) {
		for (var j = 0; j < this.wordWin.totalWords(i); j++) {
			if ((i == ti && j > wi) || i > ti) {
				...
				if (
					this.wordWin.getTextVal(i, j) == s_word_to_repl &&
					!this.wordFlags[i][j]
				) {
	...
	if (this.wordWin.totalPreviousWords(ti, wi) > 0) {
		...
		do {
			...
			if (this.currentWordIndex == 0 && this.currentTextIndex > 0) {
				...
				this.currentWordIndex =
					this.wordWin.totalWords(this.currentTextIndex) - 1;
			...
			this.wordFlags[this.currentTextIndex][this.currentWordIndex] ==
				this.fromIgnrAll ||
			this.wordFlags[this.currentTextIndex][this.currentWordIndex] ==
				this.fromReplAll
		);
...
function _getTotalReplaced() {
	...
	for (var i = 0; i < this.wordFlags.length; i++) {
		for (var j = 0; j < this.wordFlags[i].length; j++) {
			if (
				this.wordFlags[i][j] == this.replWordFlag ||
