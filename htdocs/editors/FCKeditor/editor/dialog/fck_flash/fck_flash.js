...
function LoadSelection() {
	...
	GetE("chkAutoPlay").checked =
		GetAttribute(oEmbed, "play", "true") == "true";
...
function Ok() {
	...
	if (!oFakeImage) {
		oFakeImage = oEditor.FCKDocumentProcessor_CreateFakeImage(
			"FCK__Flash",
			oEmbed
		);
...
function SetPreviewElement(previewEl) {
	...
	if (GetE("txtUrl").value.length > 0) UpdatePreview();
}

function UpdatePreview() {
	if (!ePreview) return;

	while (ePreview.firstChild) ePreview.removeChild(ePreview.firstChild);

	if (GetE("txtUrl").value.length == 0) ePreview.innerHTML = "&nbsp;";
	else {
...
function CheckUpload() {
	...
	if (
		(FCKConfig.FlashUploadAllowedExtensions.length > 0 &&
			!oUploadAllowedExtRegex.test(sFile)) ||
		(FCKConfig.FlashUploadDeniedExtensions.length > 0 &&
