	...
	var fixSizes = (window.DoResizeFixes = function() {
		...
		for (var i = 0; i < fckDlg.childNodes.length; i++) {
			...
			switch (child.className) {
				case "contents":
					child.style.width = Math.max(
						0,
						fckDlg.offsetWidth - 16 - 16
					); // -left -right
					child.style.height = Math.max(
						0,
						fckDlg.clientHeight - 20 - 2
					); // -bottom -top
					break;
				...
				case "cover":
					child.style.width = Math.max(
						0,
						fckDlg.offsetWidth - 16 - 16 + 4
					); // -left -right + 4
					child.style.height = Math.max(
						0,
						fckDlg.clientHeight - 20 - 2 + 4
					); // -bottom -top + 4
					break;
