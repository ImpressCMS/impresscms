...
String.prototype.Equals = function() {
	...
	if (aArgs.length == 1 && aArgs[0].pop) aArgs = aArgs[0];

	for (var i = 0; i < aArgs.length; i++) {
		if (this == aArgs[i]) return true;
	}
	return false;
};
...
String.prototype.IEquals = function() {
	...
	for (var i = 0; i < aArgs.length; i++) {
		if (thisUpper == aArgs[i].toUpperCase()) return true;
	}
	return false;
};
...
String.prototype.Replace = function(regExp, replacement, thisObj) {
	...
	} else return this.replace(regExp, replacement);
};
