...
function FCKToolbarSet_Create(overhideLocation) {
		...
		default:
			...
			if (oOutMatch) {
				if (FCKBrowserInfo.IsAIR)
					...
					eToolbarTarget = eval(
						"parent." + oOutMatch[1]
					).document.getElementById(oOutMatch[2]);
			} else {
				// Out:[TargetId]
				oOutMatch = sLocation.match(/^Out:(\w+)$/);
				if (oOutMatch)
					eToolbarTarget = parent.document.getElementById(
						oOutMatch[1]
					);
			}
			...
			oToolbarSet = eToolbarTarget.__FCKToolbarSet = new FCKToolbarSet(
				eTargetDocument
			);
			oToolbarSet._IFrame = eToolbarIFrame;

			if (FCK.IECleanup)
				FCK.IECleanup.AddItem(
					eToolbarTarget,
	...
	if (!oToolbarSet.ToolbarItems) oToolbarSet.ToolbarItems = FCKToolbarItems;
...
function FCK_OnBlur(editorInstance) {
	...
	if (eToolbarSet.CurrentInstance == editorInstance) eToolbarSet.Disable();
}
...
function FCK_OnFocus(editorInstance) {
	...
	oToolbarset.CurrentInstance.FocusManager.RemoveWindow(
		oToolbarset._IFrame.contentWindow
	);
...
var FCKToolbarSet = function(targetDocument) {
	...
	if (FCKConfig.ToolbarCanCollapse) eCollapseHandle.style.display = "";
	else targetDocument.getElementById("xTBLeftBorder").style.display = "";
...
FCKToolbarSet.prototype.Load = function(toolbarSetName) {
	...
	for (var x = 0; x < ToolbarSet.length; x++) {
		...
		if (!oToolbarItems) continue;

		var oToolbar;

		if (typeof oToolbarItems == "string") {
			if (oToolbarItems == "/") oToolbar = new FCKToolbarBreak();
		} else {
			...
			for (var j = 0; j < oToolbarItems.length; j++) {
				...
				if (sItem == "-") oToolbar.AddSeparator();
				else {
...
FCKToolbarSet.prototype.Enable = function() {
	...
	for (var i = 0; i < aItems.length; i++) aItems[i].RefreshState();
};
...
FCKToolbarSet.prototype.Disable = function() {
	...
	var aItems = this.Items;
	for (var i = 0; i < aItems.length; i++) aItems[i].Disable();
};

FCKToolbarSet.prototype.RefreshModeState = function(editorInstance) {
	if (FCK.Status != FCK_STATUS_COMPLETE) return;
	...
	if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
		...
		for (var i = 0; i < aItems.length; i++) aItems[i].Enable();
		...
		for (var j = 0; j < aItems.length; j++) aItems[j].Disable();
	}
};

FCKToolbarSet.prototype.RefreshItemsState = function(editorInstance) {
	var aItems = (editorInstance ? editorInstance.ToolbarSet : this)
		.ItemsContextSensitive;
