...
var FCKStyles = (FCK.Styles = {
	...
	ApplyStyle: function(style) {
		if (typeof style == "string") style = this.GetStyles()[style];

		if (style) {
			if (style.GetType() == FCK_STYLE_OBJECT)
				style.ApplyToObject(FCKSelection.GetSelectedElement());
			else style.ApplyToSelection(FCK.EditorWindow);
	...
	AttachStyleStateChange: function(styleName, callback, callbackOwner) {
		...
		if (!callbacks) callbacks = this._Callbacks[styleName] = [];
	...
	RemoveAll: function() {
		...
		if (range.CheckIsCollapsed()) return;
	...
	_LoadStylesCore: function() {
		...
		for (var styleName in styleDefs) {
			...
			var style = (styles["_FCK_" + styleName] = new FCKStyle(
				styleDefs[styleName]
			));
			style.IsCore = true;
		}
	...
	_LoadStylesCustom: function() {
		...
		if (styleDefs) {
			for (var styleName in styleDefs) {
				var style = (styles[styleName] = new FCKStyle(
					styleDefs[styleName]
				));
	...
	_LoadStylesXml: function() {
		...
		if (!stylesXmlPath || stylesXmlPath.length == 0) return styles;

		// Load the XML file into a FCKXml object.
		var xml = new FCKXml();
		xml.LoadUrl(stylesXmlPath);

		var stylesXmlObj = FCKXml.TransformToObject(
			xml.SelectSingleNode("Styles")
		);
				...
				for (j = 0; j < cssStyleOverrideNodes.length; j++) {
					...
					if (overrideAttNode) {
						...
						for (var k = 0; k < overrideAttNode.length; k++) {
							var overrideAttValue =
								overrideAttNode[k].value || null;
							if (overrideAttValue) {
								// Check if the override attribute value is a regular expression.
								var regexMatch =
									overrideAttValue &&
									FCKRegexLib.RegExp.exec(overrideAttValue);
								if (regexMatch)
