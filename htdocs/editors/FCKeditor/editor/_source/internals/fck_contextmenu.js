	...
	switch (listenerName) {
			...
			return {
				AddItems: function(menu, tag, tagName) {
					menu.AddItem(
						...
						FCKLang.Cut,
						7,
						FCKCommands.GetCommand("Cut").GetState() ==
					...
					menu.AddItem(
						...
						FCKLang.Copy,
						8,
						FCKCommands.GetCommand("Copy").GetState() ==
							FCK_TRISTATE_DISABLED
					);
					menu.AddItem(
						"Paste",
						FCKLang.Paste,
						9,
						FCKCommands.GetCommand("Paste").GetState() ==
							FCK_TRISTATE_DISABLED
					);
				}
			};
		...
		case "Table":
			return {
				AddItems: function(menu, tag, tagName) {
					...
					var bIsCell =
						!bIsTable && FCKSelection.HasAncestorNode("TABLE");
		...
		case "Link":
			return {
				AddItems: function(menu, tag, tagName) {
					var bInsideLink =
						tagName == "A" || FCKSelection.HasAncestorNode("A");
...
function FCK_ContextMenu_OnBeforeOpen() {
	...
	if ((oTag = FCKSelection.GetSelectedElement())) sTagName = oTag.tagName;
