...
FCKXHtml.GetXHTML = function(node, includeNode, format) {
	FCKDomTools.CheckAndRemovePaddingNode(
		FCKTools.GetElementDocument(node),
		FCKConfig.EnterMode
	);
	...
	if (includeNode) this._AppendNode(this.MainNode, node);
	else this._AppendChildNodes(this.MainNode, node, false);
	 ...
	 * refs #2156 and #2834
	 */
	if (FCKBrowserInfo.IsIE) FCKXHtml._RemoveXHtmlJobProperties(node);
	...
	else sXHTML = sXHTML.replace(FCKRegexLib.SpaceNoClose, " />");
	...
	try {
		if (attributeValue == undefined || attributeValue == null)
			...
			attributeValue = attributeValue.replace(
				FCKXHtmlEntities.EntitiesRegex,
				FCKXHtml_GetEntity
			);
		}
...
FCKXHtml._AppendChildNodes = function(xmlNode, htmlNode, isBlockElement) {
	...
	if (xmlNode.childNodes.length == 0) {
		...
		if (FCKListsLib.InlineChildReqElements[sNodeName]) return null;
	...
	switch (htmlNode.nodeType) {
		...
		case 1:
			...
			if (
				...
				var val = "\r";
				if (htmlNode == htmlNode.parentNode.firstChild) val += "\r";
				return FCKXHtml._AppendNode(
					xmlNode,
					this.XML.createTextNode(val)
				);
			}
			...
			if (
				...
				(htmlNode.hasAttribute("_moz_editor_bogus_node") ||
					htmlNode.getAttribute("type") == "_moz")
			) {
				if (htmlNode.nextSibling) return false;
				else {
			...
			if (htmlNode.getAttribute("_fcktemp")) return false;
			...
			if (FCKBrowserInfo.IsIE) {
				...
				if (
					htmlNode.scopeName &&
					htmlNode.scopeName != "HTML" &&
					htmlNode.scopeName != "FCK"
				)
					sNodeName =
						htmlNode.scopeName.toLowerCase() + ":" + sNodeName;
			} else {
			...
			if (!FCKRegexLib.ElementName.test(sNodeName)) return false;

			// The already processed nodes must be marked to avoid then to be duplicated (bad formatted HTML).
			// So here, the "mark" is checked... if the element is Ok, then mark it.
			if (
				htmlNode._fckxhtmljob &&
			...
			if (oTagProcessor) oNode = oTagProcessor(oNode, htmlNode, xmlNode);
			else
				oNode = this._AppendChildNodes(
					oNode,
					htmlNode,
		...
		case 8:
			...
			if (FCKBrowserInfo.IsIE && !htmlNode.innerHTML) break;
...
function FCKXHtml_GetEntity(character) {
	...
	var sEntity =
		FCKXHtmlEntities.Entities[character] || "#" + character.charCodeAt(0);
	return "#?-:" + sEntity + ";";
}
...
FCKXHtml.TagProcessors = {
	...
	iframe: function(node, htmlNode) {
		...
		if (FCKBrowserInfo.IsGecko) sHtml = FCKTools.HTMLDecode(sHtml);

		// Remove the saved urls here as the data won't be processed as nodes
		sHtml = sHtml.replace(/\s_fcksavedurl="[^"]*"/g, "");

		node.appendChild(
			FCKXHtml.XML.createTextNode(FCKXHtml._AppendSpecialItem(sHtml))
		);
	...
	img: function(node, htmlNode) {
		...
		if (htmlNode.style.width) node.removeAttribute("width");
		if (htmlNode.style.height) node.removeAttribute("height");
	...
	ol: function(node, htmlNode, targetNode) {
		if (htmlNode.innerHTML.Trim().length == 0) return false;
	...
	style: function(node, htmlNode) {
		...
		if (FCKBrowserInfo.IsIE)
			// Bug #403 : IE always appends a \r\n to the beginning of StyleNode.innerHTML
			cssText = cssText.replace(/^(\r\n|\n|\r)/, "");

		node.appendChild(
			FCKXHtml.XML.createTextNode(FCKXHtml._AppendSpecialItem(cssText))
		);
