...
(function() {
	...
	function ScaytEngineLoad(callback) {
		...
		var init = function() {
			...
			ScaytCombobox &&
				ScaytCombobox.SetEnabled(scyt_control && scyt_control.disabled);
			InitSetup();
		};
		...
		if (callback) scaytOnLoad.push(callback);
	...
	ScaytCommand.prototype.Execute = function(action) {
			...
			case "About":
				...
				if (isEngineLoaded && scaytReady)
					FCKDialog.OpenDialog(
						"Scayt",
						"SCAYT Settings",
						"dialog/fck_scayt.html?" + action.toLowerCase(),
						343,
						343
					);
				break;
		...
		if (!scaytReady) return ScaytMessage("SCAYT is not ready") || false;
	...
	function ShowScaytState() {
		...
		if (!combo || !combo._Combo || !combo._Combo._OuterTable) return;
	...
	function GetStatusLabel() {
		if (!scaytReady) return "<b>Enable SCAYT</b>";
	...
	function InitSetup() {
		FCK.ContextMenu.RegisterListener({
			AddItems: function(menu) {
				...
				if (!scayt_control) return;

				var node = scayt_control.getScaytNode();

				if (!node) return;

				var suggestions = scayt.getSuggestion(
					scayt_control.getWord(node),
					scayt_control.getLang()
				);
				...
				var suggAveCount =
					maxSuggestions == -1 ? suggestions.length : maxSuggestions;
