...
FCKSelection.GetBoundaryParentElement = function(startBoundary) {
	...
	if (this.GetType() == "Control")
		...
		if (oSel && oSel.rangeCount > 0) {
			var range = oSel.getRangeAt(
				startBoundary ? 0 : oSel.rangeCount - 1
			);
...
FCKSelection.Collapse = function(toStart) {
	...
	if (toStart == null || toStart === true) oSel.collapseToStart();
	else oSel.collapseToEnd();
};
...
FCKSelection.MoveToAncestorNode = function(nodeTagName) {
	...
	while (oContainer) {
		if (oContainer.nodeName.IEquals(nodeTagName)) return oContainer;
