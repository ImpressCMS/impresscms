...
var FCK = {
	...
	FixBody: function() {
		...
		if (sBlockTag != "p" && sBlockTag != "div") return;
		...
		while (oNode) {
			...
			switch (oNode.nodeType) {
				...
				case 1:
					...
					if (
						!FCKListsLib.BlockElements[nodeName] &&
						nodeName != "li" &&
			...
			if (bMoveNode) {
				...
				if (!oNewBlock)
					oNewBlock = oParent.insertBefore(
						oDocument.createElement(sBlockTag),
						oNode
					);
		...
		if (oNewBlock) FCKDomTools.TrimNode(oNewBlock);
	},
	...
	UpdateLinkedField: function() {
		...
		if (FCKConfig.HtmlEncodeOutput) value = FCKTools.HTMLEncode(value);
	...
	OnDoubleClick: function(element) {
		var oCalls =
			FCK.RegisteredDoubleClickHandlers[element.tagName.toUpperCase()];

		if (oCalls) {
			for (var i = 0; i < oCalls.length; i++) oCalls[i](element);
		}
	...
	ProtectTags: function(html) {
		...
		sTags = "META";
		if (FCKBrowserInfo.IsIE) sTags += "|HR";
	...
	SetData: function(data, resetIsDirty) {
		...
		if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
			...
			if (
				FCK.TempBaseTag.length > 0 &&
				!FCKRegexLib.HasBaseTag.test(data)
			)
				data = data.replace(
					FCKRegexLib.HeadOpener,
					"$&" + FCK.TempBaseTag
				);
			...
			if (FCKBrowserInfo.IsIE) sHeadExtra += FCK._GetBehaviorsStyle();
			else if (FCKConfig.ShowBorders)
				sHeadExtra += FCKTools.GetStyleHtml(
			...
			if (resetIsDirty) this.ResetIsDirty();
	...
	GetNamedCommandValue: function(commandName) {
		...
		if (eState == FCK_TRISTATE_DISABLED) return null;
	...
	SwitchEditMode: function(noUndo) {
		...
		if (bIsWysiwyg) {
			...
			if (!noUndo && FCKBrowserInfo.IsIE) FCKUndo.SaveUndoStep();

			sHtml = FCK.GetXHTML(FCKConfig.FormatSource);

			if (FCKBrowserInfo.IsIE) FCKTempBin.ToHtml();

			if (sHtml == null) return false;
		} else sHtml = this.EditingArea.Textarea.value;
	...
	InsertElement: function(element) {
		...
		if (FCKListsLib.BlockElements[elementName] != null) {
			...
			var next = FCKDomTools.GetNextSourceElement(
				element,
				false,
				null,
			...
			if (!next && FCKConfig.EnterMode != "br") {
				next = this.EditorDocument.body.appendChild(
					this.EditorDocument.createElement(FCKConfig.EnterMode)
				);

				if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(next);
			}

			if (FCKListsLib.EmptyElements[elementName] == null)
				range.MoveToElementEditStart(element);
			else if (next) range.MoveToElementEditStart(next);
			else range.MoveToPosition(element, 4);

			if (FCKBrowserInfo.IsGeckoLike) {
				if (next) FCKDomTools.ScrollIntoView(next, false);
	...
	_KeyDownListener: function(evt) {
		if (!evt) evt = FCK.EditorWindow.event;
		if (FCK.EditorWindow) {
			if (
				!FCK._IsFunctionKey(evt.keyCode) && // do not capture function key presses, like arrow keys or shift/alt/ctrl
				!(evt.ctrlKey || evt.metaKey) && // do not capture Ctrl hotkeys, as they have their snapshot capture logic
				!(evt.keyCode == 46)
			)
				// do not capture Del, it has its own capture logic in fckenterkey.js
				FCK._KeyDownUndo();
		}
		return true;
	},
...
function _FCK_MouseEventsListener(evt) {
	if (!evt) evt = window.event;
	if (evt.type == "mousedown") FCK.MouseDownFlag = true;
	else if (evt.type == "mouseup") FCK.MouseDownFlag = false;
	else if (evt.type == "mousemove") FCK.Events.FireEvent("OnMouseMove", evt);
}
		...
		if (parentElement && parentElement.nodeName.IEquals("body")) {
			if (
				...
				if (!paddingNode.childNodes.firstChild) {
					paddingNode.appendChild(
						FCKTools.GetElementDocument(paddingNode).createTextNode(
							"\ufeff"
						)
					);
					clearContents = true;
				...
				range.select();
				if (clearContents) range.pasteHTML("");
			}
		}
	}
}
...
function _FCK_EditingArea_OnLoad() {
	...
	if (FCKBrowserInfo.IsSafari) {
		...
		FCKTools.AddEventListener(FCK.EditorDocument, "paste", function(evt) {
			...
			setTimeout(function() {
				...
				nodeAfter = FCK.EditorDocument.getElementById(
					"fck_paste_padding"
				);
	...
	if (FCK.Status != FCK_STATUS_NOTLOADED) return;
...
function _FCK_KeystrokeHandler_OnKeystroke(keystroke, keystrokeValue) {
	if (FCK.Status != FCK_STATUS_COMPLETE) return false;
	...
	if (oCommand.GetState() == FCK_TRISTATE_DISABLED) return false;

	return (
		oCommand.Execute.apply(
			oCommand,
			FCKTools.ArgumentsToArray(arguments, 2)
		) !== false
	);
}
...
(function() {
	...
	while (eLinkedField || i == 0) {
		if (
			eLinkedField &&
			eLinkedField.tagName.toLowerCase().Equals("input", "textarea")
		) {
	...
	ToElements: function() {
		...
		for (var i = 0; i < this.Elements.length; i++) {
			if (this.Elements[i].isHtml) {
				...
				this.Elements[i] = node.firstChild.removeChild(
			...
			oTarget =
				win.nodeType == 1
					...
					: win.frameElement
						? win.frameElement
						: win.document;
		else if (FCKBrowserInfo.IsSafari) oTarget = win;
		else oTarget = win.document;
