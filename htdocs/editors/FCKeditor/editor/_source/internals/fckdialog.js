...
var FCKDialog = (function() {
	...
	while (topWindow.parent && topWindow.parent != topWindow) {
		try {
			if (topWindow.parent.document.domain != document.domain) break;
			if (
				topWindow.parent.document.getElementsByTagName("frameset")
					.length > 0
			)
				break;
		} catch (e) {
	...
	var getZIndex = function() {
		if (!baseZIndex) baseZIndex = FCKConfig.FloatingPanelsZIndex + 999;
		return ++baseZIndex;
	};
	...
	var resizeHandler = function() {
		...
		FCKDomTools.SetElementStyles(cover, {
			width:
				Math.max(
					relElement.scrollWidth,
					relElement.clientWidth,
					topDocument.scrollWidth || 0
				) -
				1 +
				"px",
			height:
				Math.max(
					relElement.scrollHeight,
					relElement.clientHeight,
					topDocument.scrollHeight || 0
				) -
				1 +
				"px"
		});
	...
	return {
		...
		OpenDialog: function(
			...
			var useAbsolutePosition =
				FCKBrowserInfo.IsIE &&
				(!FCKBrowserInfo.IsIE7 ||
					!FCKTools.IsStrictMode(topWindow.document));
			if (useAbsolutePosition)
				scrollPosition = FCKTools.GetScrollPosition(topWindow);
			var iTop = Math.max(
				scrollPosition.Y + (viewSize.Height - height - 20) / 2,
				0
			);
			var iLeft = Math.max(
				scrollPosition.X + (viewSize.Width - width - 20) / 2,
				0
			);
		...
		DisplayMainCover: function() {
			...
			var el = FCK.ToolbarSet.CurrentInstance.GetInstanceObject(
				"frameElement"
			);
