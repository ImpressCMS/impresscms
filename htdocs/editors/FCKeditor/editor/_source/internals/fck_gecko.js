...
FCK.InitializeBehaviors = function() {
	...
	this._ExecCheckCaret = function(evt) {
		...
		var blockEmptyStop = function(node) {
			...
			return (
				FCKListsLib.BlockElements[tag] || FCKListsLib.EmptyElements[tag]
			);
		};
		...
		var moveCursor = function() {
			...
			if (node.nodeValue.length != range.endOffset) return;
			...
			var nextTextNode = FCKTools.GetNextTextNode(
				node,
				node.parentNode,
				blockEmptyStop
			);
			...
			nextTextNode = FCKTools.GetNextTextNode(
				node,
			...
			if (nextTextNode) {
				...
				if (FCKBrowserInfo.IsOpera && keyCode == 37) return;
				...
				if (
					...
					while (stopNode) {
						...
						if (
							FCKListsLib.BlockElements[stopTag] ||
					...
					else node.parentNode.appendChild(marker);
...
FCK.InsertHtml = function(html) {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		if (
			currentNode &&
			FCKListsLib.BlockElements[currentNode.nodeName.toLowerCase()]
		)
			range.SplitBlock();
...
FCK.CreateLink = function(url, noUndo) {
	...
	if (FCKSelection.GetSelection().isCollapsed) return aCreatedLinks;
	...
	if (url.length > 0) {
		...
		var oLinksInteractor = this.EditorDocument.evaluate(
			"//a[@href='" + sTempUrl + "']",
...
FCK._FillEmptyBlock = function(emptyBlockNode) {
	if (!emptyBlockNode || emptyBlockNode.nodeType != 1) return;
	var nodeTag = emptyBlockNode.tagName.toLowerCase();
	if (nodeTag != "p" && nodeTag != "div") return;
	if (emptyBlockNode.firstChild) return;
