...
FCKTools.DisableSelection = function(element) {
	if (FCKBrowserInfo.IsGecko) element.style.MozUserSelect = "none";
	// Gecko only.
	else if (FCKBrowserInfo.IsSafari) element.style.KhtmlUserSelect = "none";
	// WebKit only.
	else element.style.userSelect = "none"; // CSS3 (not supported yet).
};
...
FCKTools.AddEventListenerEx = function(
	sourceObject,
	eventName,
	listener,
	paramsArray
...
FCKTools.RestoreStyles = function(element, savedStyles) {
	...
	if (savedStyles.Inline)
		element.setAttribute("style", savedStyles.Inline, 0);
	// 0 : Case Insensitive
	else element.removeAttribute("style", 0);
	FCKTools.RestoreFormStyles(element, data);
};
		...
		*/
		...
		if (el.offsetParent) el = el.offsetParent;
			...
			if (oOwnerWindow != oWindow) {
				...
				if (el) oOwnerWindow = FCKTools.GetElementWindow(el);
			} else {
				c.X += el.scrollLeft;
				c.Y += el.scrollTop;
				break;
