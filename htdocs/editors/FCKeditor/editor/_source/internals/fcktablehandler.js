...
FCKTableHandler.DeleteRows = function(row) {
	...
	if (!row) {
		...
		for (var i = aRowsToDelete.length; i >= 0; i--) {
			if (aRowsToDelete[i]) FCKTableHandler.DeleteRows(aRowsToDelete[i]);
		}
		return;
	}
...
FCKTableHandler.InsertColumn = function(insertBefore) {
	...
	for (var i = 0; i < oTable.rows.length; i++) {
		...
		if (oRow.cells.length < iIndex + 1) continue;

		oCell = oRow.cells[iIndex].cloneNode(false);

		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(oCell);
		...
		oRow.insertBefore(
			oCell,
			insertBefore ? oBaseCell : oBaseCell.nextSibling
		);
	}
};
...
FCKTableHandler.DeleteColumns = function(oCell) {
	...
	for (var i = oTable.rows.length - 1; i >= 0; i--) {
		...
		if (oRow.cells[iIndex]) oRow.removeChild(oRow.cells[iIndex]);
	}
};
...
FCKTableHandler.InsertCell = function(cell, insertBefore) {
	...
	if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(oNewCell);

	if (!insertBefore && oCell.cellIndex == oCell.parentNode.cells.length - 1)
		oCell.parentNode.appendChild(oNewCell);
	else
		oCell.parentNode.insertBefore(
			oNewCell,
			insertBefore ? oCell : oCell.nextSibling
		);
...
FCKTableHandler._MarkCells = function(cells, label) {
	for (var i = 0; i < cells.length; i++) cells[i][label] = true;
};
...
FCKTableHandler._ReplaceCellsByMarker = function(tableMap, marker, substitute) {
	for (var i = 0; i < tableMap.length; i++) {
		for (var j = 0; j < tableMap[i].length; j++) {
			if (tableMap[i][j][marker]) tableMap[i][j] = substitute;
		}
	}
};

FCKTableHandler._GetMarkerGeometry = function(
	tableMap,
	rowIdx,
	colIdx,
	markerName
	...
	for (
		var i = colIdx;
		tableMap[rowIdx][i] && tableMap[rowIdx][i][markerName];
		i++
	)
		selectionWidth++;
	for (
	...
	for (
		var i = rowIdx;
		tableMap[i] && tableMap[i][colIdx] && tableMap[i][colIdx][markerName];
		i++
	)
		selectionHeight++;
	for (
...
FCKTableHandler.CheckIsSelectionRectangular = function() {
	...
	var geometry = this._GetMarkerGeometry(
		tableMap,
		rowIdx,
		colIdx,
		"_CellSelected"
	...
	if (geometry.width >= geometry.height) {
		for (
			colIdx = baseColIdx;
			colIdx < baseColIdx + geometry.width;
			colIdx++
		) {
		...
		for (
			rowIdx = baseRowIdx;
			rowIdx < baseRowIdx + geometry.height;
			rowIdx++
		) {
...
FCKTableHandler.MergeCells = function() {
	...
	var selectionGeometry = this._GetMarkerGeometry(
		tableMap,
		rowIdx,
		colIdx,
		"_SelectedCells"
	...
	var cellContents = FCKTools.GetElementDocument(
		refCell
	...
	for (var i = 0; i < selectionGeometry.height; i++) {
		...
		for (var j = 0; j < selectionGeometry.width; j++) {
			...
			while (currentCell.childNodes.length > 0) {
				...
				if (
					node.nodeType != 1 ||
					(node.getAttribute("type", 2) != "_moz" &&
						node.getAttribute("_moz_dirty") != null)
				) {
...
FCKTableHandler.MergeDown = function() {
	...
	if (cellContents.firstChild)
		cellContents.insertBefore(
			FCK.EditorDocument.createElement("br"),
...
FCKTableHandler.HorizontalSplitCell = function() {
	...
	if (cellSpan > 1) {
		...
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(newCell);
		...
		for (var r = rowIdx; r < rowIdx + rowSpan; r++) {
			for (var i = startIdx; i < endIdx; i++) tableMap[r][i] = newCell;
		}
	} else {
...
FCKTableHandler.VerticalSplitCell = function() {
	...
	var cellIndex = FCKTableHandler._GetCellIndexSpan(
		tableMap,
		currentRowIndex,
		currentCell
	);
	...
	if (currentRowSpan > 1) {
		...
		if (currentColSpan > 1) newCell.colSpan = currentColSpan;
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(newCell);
		currentCell.parentNode.parentNode.parentNode.rows[
			newCellRowIndex
		].insertBefore(newCell, insertMarker);
	} else {
		// 1. Insert a new row.
		...
		else tSection.appendChild(newRow);
...
FCKTableHandler._GetCellIndexSpan = function(tableMap, rowIndex, cell) {
	if (tableMap.length < rowIndex + 1) return null;

	var oRow = tableMap[rowIndex];

	for (var c = 0; c < oRow.length; c++) {
		if (oRow[c] == cell) return c;
	}
	...
	for (var i = 0; i < aRows.length; i++) {
		...
		for (var j = 0; j < aRows[i].cells.length; j++) {
			...
			for (var rs = 0; rs < iRowSpan; rs++) {
				if (!aMap[r + rs]) aMap[r + rs] = new Array();
...
FCKTableHandler._InstallTableMap = function(tableMap, table) {
	...
	for (var i = 0; i < tableMap.length; i++) {
		for (var j = 0; j < tableMap[i].length; j++) {
			...
			if (cell._colScanned === true) continue;
			if (tableMap[i][j - 1] == cell) cell.colSpan++;
			if (tableMap[i][j + 1] != cell) cell._colScanned = true;
		}
	}
...
FCKTableHandler.ClearRow = function(tr) {
	...
	for (var i = 0; i < aCells.length; i++) {
		...
		if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(aCells[i]);
	}
};
...
FCKTableHandler.GetMergeRightTarget = function() {
	...
	var refGeometry = this._GetMarkerGeometry(
		tableMap,
		rowIdx,
		colIdx,
		"_SizeTest"
	);
	var nextGeometry = this._GetMarkerGeometry(
		tableMap,
		rowIdx,
		nextColIdx,
		"_SizeTest"
	...
	if (
		refGeometry.height != nextGeometry.height ||
...
FCKTableHandler.GetMergeDownTarget = function() {
	...
	if (!tableMap[newRowIdx]) return null;
	...
	if (
		refGeometry.width != nextGeometry.width ||
