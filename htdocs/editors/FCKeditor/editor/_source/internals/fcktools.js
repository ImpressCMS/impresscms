...
FCKTools.FixCssUrls = function(urlFixPrefix, cssStyles) {
	if (!urlFixPrefix || urlFixPrefix.length == 0) return cssStyles;
	...
	return cssStyles.replace(/url\s*\(([\s'"]*)(.*?)([\s"']*)\)/g, function(
		...
		else return "url(" + opener + urlFixPrefix + path + closer + ")";
	});
};
...
FCKTools._GetUrlFixedCss = function(cssStyles, urlFixPrefix) {
	...
	if (match) return FCKTools.FixCssUrls(match[1], match[2]);
	else return cssStyles;
};
...
FCKTools.AppendStyleSheet = function(domDocument, cssFileOrArrayOrDef) {
	...
	if (typeof cssFileOrArrayOrDef == "string") {
		...
		if (/[\\\/\.][^{}]*$/.test(cssFileOrArrayOrDef)) {
			...
			return this.AppendStyleSheet(
				domDocument,
				cssFileOrArrayOrDef.split(",")
			);
		} else
			return [
				this.AppendStyleString(
					domDocument,
					FCKTools._GetUrlFixedCss(cssFileOrArrayOrDef)
				)
			];
	} else {
		var styles = [];
		for (var i = 0; i < cssFileOrArrayOrDef.length; i++)
			styles.push(
				this._AppendStyleSheet(domDocument, cssFileOrArrayOrDef[i])
			);
		return styles;
	}
};
	...
	return function(cssFileOrArrayOrDef, markTemp) {
		...
		if (typeof cssFileOrArrayOrDef == "string") {
			...
			if (/[\\\/\.][^{}]*$/.test(cssFileOrArrayOrDef)) {
				...
				return getStyle(
					this._GetUrlFixedCss(cssFileOrArrayOrDef),
					markTemp
				);
		} else {
			var html = "";
...
FCKTools._ProcessLineBreaksForPMode = function(
	oEditor,
	text,
	liState,
	node,
	...
	for (var i = 0; i < text.length; i++) {
		...
		if (n == "\n") {
			...
			if (closeState) strArray.push(blockEndTag);
			strArray.push(blockStartTag);
			closeState = 1;
		} else strArray.push(lineBreakTag);
	}
};

FCKTools._ProcessLineBreaksForDivMode = function(
	oEditor,
	text,
	liState,
	node,
...
FCKTools._ProcessLineBreaksForBrMode = function(
	oEditor,
	text,
	liState,
	node,
	...
	for (var i = 0; i < text.length; i++) {
		...
		if (closeState && blockEndTag.length) strArray.push(blockEndTag);
		strArray.push(blockStartTag);
		closeState = 1;
	}
};
...
FCKTools.ProcessLineBreaks = function(oEditor, oConfig, text) {
	...
	if (enterMode == "p")
		...
		this._ProcessLineBreaksForBrMode(
			oEditor,
			text,
			liState,
			node,
...
FCKTools.AddSelectOption = function(selectElement, optionText, optionValue) {
	var oOption = FCKTools.GetElementDocument(selectElement).createElement(
		"OPTION"
	);
...
FCKTools.RunFunction = function(func, thisObject, paramsArray, timerWindow) {
	if (func) this.SetTimeout(func, 0, thisObject, paramsArray, timerWindow);
};

FCKTools.SetTimeout = function(
	func,
	milliseconds,
	thisObject,
	paramsArray,
	...
	return (timerWindow || window).setTimeout(function() {
		if (paramsArray) func.apply(thisObject, [].concat(paramsArray));
		else func.apply(thisObject);
	}, milliseconds);
};

FCKTools.SetInterval = function(
	func,
	milliseconds,
	thisObject,
	paramsArray,
...
FCKTools.IsStrictMode = function(document) {
	...
	return (
		"CSS1Compat" ==
		(document.compatMode || (FCKBrowserInfo.IsSafari ? "CSS1Compat" : null))
	);
};
...
FCKTools.AppendBogusBr = function(element) {
	...
	if (
		!eLastChild ||
		(eLastChild.getAttribute("type", 2) != "_moz" &&
			eLastChild.getAttribute("_moz_dirty") == null)
	) {
		var doc = this.GetElementDocument(element);

		if (FCKBrowserInfo.IsOpera) element.appendChild(doc.createTextNode(""));
		else element.appendChild(this.CreateBogusBR(doc));
	}
};
...
FCKTools.GetDocumentPosition = function(w, node) {
	...
	while (
		...
		!(
			curWindow == w &&
			(curNode == w.document.body ||
				curNode == w.document.documentElement)
		)
	) {
		...
		if (curNode.offsetParent) curNode = curNode.offsetParent;
			...
			if (curWindow != w) {
				...
				prevNode = null;
				if (curNode) curWindow = curNode.contentWindow.parent;
			} else curNode = null;
		}
	}
...
FCKTools.RestoreFormStyles = function(formNode, hijackRecord) {
	...
	if (hijackRecord.length > 0) {
		for (var i = hijackRecord.length - 1; i >= 0; i--) {
			...
			if (sibling) formNode.insertBefore(node, sibling);
			else formNode.appendChild(node);
		}
	}
};
...
FCKTools.GetNextNode = function(node, limitNode) {
	...
	else if (node.nextSibling) return node.nextSibling;
	else {
		var ancestor = node.parentNode;
		while (ancestor) {
			if (ancestor == limitNode) return null;
			if (ancestor.nextSibling) return ancestor.nextSibling;
...
FCKTools.GetNextTextNode = function(textnode, limitNode, checkStop) {
	...
	if (checkStop && node && checkStop(node)) return null;
...
FCKTools.AppendLengthProperty = function(targetObject, propertyName) {
	...
	for (var n in targetObject) counter++;
