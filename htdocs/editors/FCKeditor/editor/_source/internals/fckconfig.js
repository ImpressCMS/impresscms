...
FCKConfig.ProcessHiddenField = function() {
	...
	var oConfigField = window.parent.document.getElementById(
		FCK.Name + "___Config"
	);
	...
	for (var i = 0; i < aCouples.length; i++) {
		...
		if (sKey == "CustomConfigurationsPath")
			// The Custom Config File path must be loaded immediately.
			FCKConfig[sKey] = sVal;
		...
		else this.PageConfig[sKey] = sVal;
	}
};

function FCKConfig_LoadPageConfig() {
	var oPageConfig = FCKConfig.PageConfig;
	for (var sKey in oPageConfig) FCKConfig[sKey] = oPageConfig[sKey];
}
...
function FCKConfig_PreProcess() {
	...
	if (!oConfig.PluginsPath.EndsWith("/")) oConfig.PluginsPath += "/";
...
FCKConfig.ApplyBodyAttributes = function(oBody) {
	// Add ID and Class to the body
	if (this.BodyId && this.BodyId.length > 0) oBody.id = FCKConfig.BodyId;
	if (this.BodyClass && this.BodyClass.length > 0)
