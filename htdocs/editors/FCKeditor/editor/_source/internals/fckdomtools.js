...
var FCKDomTools = {
	...
	MoveNode: function(source, target, toTargetStart) {
		if (toTargetStart)
			target.insertBefore(
				FCKDomTools.RemoveNode(source),
				target.firstChild
			);
		else target.appendChild(FCKDomTools.RemoveNode(source));
	},
	...
	GetFirstChild: function(node, childNames) {
		...
		if (typeof childNames == "string") childNames = [childNames];
	 ...
	 */
	GetPreviousSourceElement: function(
		currentNode,
		ignoreSpaceTextOnly,
		stopSearchElements,
		ignoreElements
		...
		if (currentNode.previousSibling)
			...
			return this.GetPreviousSourceElement(
		...
		while (currentNode) {
			...
			if (currentNode.lastChild) currentNode = currentNode.lastChild;
			else
				return this.GetPreviousSourceElement(
					currentNode,
					ignoreSpaceTextOnly,
	...
	GetNextSourceElement: function(
		...
		while (
			(currentNode = this.GetNextSourceNode(
				currentNode,
				startFromSibling
			)) // Only one "=".
		) {
			if (currentNode.nodeType == 1) {
				...
				if (
					...
					return this.GetNextSourceElement(
						currentNode,
						ignoreSpaceTextOnly,
						stopSearchElements,
						ignoreElements
	...
	GetNextSourceNode: function(
		...
		if (!startFromSibling && currentNode.firstChild)
			...
			if (stopSearchNode && currentNode == stopSearchNode) return null;
	 ...
	 * Get the next DOM node available in source order.
	 */
	GetPreviousSourceNode: function(
		currentNode,
		startFromSibling,
		nodeType,
		stopSearchNode
		...
		if (!startFromSibling && currentNode.lastChild)
			...
			if (
				...
				return this.GetPreviousSourceNode(
					currentNode.parentNode,
					true,
					nodeType,
					stopSearchNode
	...
	GetCommonParentNode: function(node1, node2, tagList) {
		var tagMap = {};
		if (!tagList.pop) tagList = [tagList];
		while (tagList.length > 0) tagMap[tagList.pop().toLowerCase()] = 1;
	...
	GetIndexOf: function(node) {
		...
		while (currentNode) {
			...
			if (currentNode == node) return currentIndex;
		...
		try {
			if (
				...
				(doc.body.lastChild.nodeType != 1 ||
					doc.body.lastChild.tagName.toLowerCase() ==
						tagName.toLowerCase())
			)
				return;
		} catch (e) {
		...
		try {
			if (
				...
				(paddingNode.firstChild &&
					paddingNode.firstChild.nodeValue != "\xa0" &&
					String(paddingNode.firstChild.tagName).toLowerCase() !=
						"br")
			) {
	...
	HasAttribute: function(element, attributeName) {
		if (element.hasAttribute) return element.hasAttribute(attributeName);
		else {
		...
		for (var i = 0; i < attributes.length; i++) {
			if (FCKBrowserInfo.IsIE) {
				...
				if (attributeNodeName == "class") {
					...
					if (element.className.length > 0) return true;
					continue;
				}
			}
			if (attributes[i].specified) return true;
		}
	...
	GetAttributeValue: function(element, att) {
		...
		if (typeof att == "string") att = element.attributes[att];
		else attName = att.nodeName;

		if (att && att.specified) {
			// IE returns "null" for the nodeValue of a "style" attribute.
			if (attName == "style") return element.style.cssText;
	...
	Contains: function(mainElement, otherElement) {
		...
		while (
			...
			if (otherElement == mainElement) return true;
		}
		return false;
	},
		...
		while (
			...
			for (var i = 0; i < parentNode.childNodes.length; i++) {
				...
				if (
					...
					continue;
				currentIndex++;
				if (parentNode.childNodes[i] == node) break;
			}
	...
	ClearElementJSProperty: function(element, attrName) {
		if (FCKBrowserInfo.IsIE) element.removeAttribute(attrName);
		else delete element[attrName];
	},
	...
	SetElementMarker: function(markerObj, element, attrName, value) {
		...
		if (!markerObj[id]) markerObj[id] = { element: element, markers: {} };
	...
	ClearElementMarkers: function(markerObj, element, clearMarkerObj) {
		...
		if (clearMarkerObj) delete markerObj[id];
	},
	...
	ListToArray: function(
		...
		if (!listNode.nodeName.IEquals(["ul", "ol"])) return [];
		...
		for (var i = 0; i < listNode.childNodes.length; i++) {
			...
			if (!listItem.nodeName.IEquals("li")) continue;
			var itemObj = {
				parent: listNode,
			...
			if (markerObj)
				this.SetElementMarker(
					markerObj,
					listItem,
					"_FCK_ListArray_Index",
					baseArray.length
				);
			...
			if (item.indent == indentLevel) {
				...
				for (var i = 0; i < item.contents.length; i++)
					currentListItem.appendChild(
						item.contents[i].cloneNode(true)
					);
				currentIndex++;
				...
				if (item.grandparent.nodeName.IEquals(["ul", "ol"]))
					...
					if (
						...
						currentListItem = doc.createElement(
							FCKConfig.EnterMode
						);
	...
	CheckIsEmptyElement: function(element, elementCheckCallback) {
		...
		return elementChild
			? this.CheckIsEmptyElement(elementChild, elementCheckCallback)
			: true;
	},
	...
	GetCurrentElementStyle: function(element, propertyName) {
		if (FCKBrowserInfo.IsIE) return element.currentStyle[propertyName];
		else
	...
	GetPositionedAncestor: function(element) {
		...
		while (
			currentElement !=
			FCKTools.GetElementDocument(currentElement).documentElement
		) {
			if (
				this.GetCurrentElementStyle(currentElement, "position") !=
				"static"
			)
			...
			if (
				currentElement ==
					FCKTools.GetElementDocument(currentElement)
						.documentElement &&
				currentWindow != w
			)
	...
	ScrollIntoView: function(element, alignTop) {
		...
		if (alignTop === false) {
			...
			offset +=
				parseInt(
					this.GetCurrentElementStyle(element, "marginBottom") || 0,
					10
				) || 0;
		}
		...
		if (
			offset > 0 &&
			(offset > currentScroll || offset < currentScroll - windowHeight)
		)
	...
	GetSelectedDivContainers: function() {
		...
		while (currentNode && currentNode != endNode) {
			if (
				...
				if (
					blockLimit &&
					blockLimit.nodeName.IEquals("div") &&
