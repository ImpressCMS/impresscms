...
FCK._GetBehaviorsStyle = function() {
	if (!FCK._BehaviorsStyle) {
		...
		if (FCKConfig.ShowBorders)
			sTableBehavior =
				"url(" + sBasePath + "css/behaviors/showtableborders.htc)";
		...
		if (FCKConfig.DisableObjectResizing) {
			sStyle += ",IMG";
			sTableBehavior +=
				" url(" + sBasePath + "css/behaviors/disablehandles.htc)";
		}
...
FCK.InsertHtml = function(html) {
	...
	if (oSel.type.toLowerCase() == "control") oSel.clear();
	...
	if (fake.parentNode.childNodes.length == 1) fake = fake.parentNode;
...
FCK.PasteAsPlainText = function(forceText) {
	...
	var sText = null;
	if (!forceText) sText = clipboardData.getData("Text");
	else sText = forceText;
...
FCK._CheckIsPastingEnabled = function(returnContents) {
	...
	if (FCK._PasteIsEnabled) {
		if (!returnContents) oReturn = true;
	} else oReturn = false;
