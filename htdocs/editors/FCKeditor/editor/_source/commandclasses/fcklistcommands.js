...
FCKListCommand.prototype = {
	GetState: function() {
		...
		while (listNode) {
			if (listNode.nodeName.IEquals(["ul", "ol"])) break;
	...
	Execute: function() {
		...
		while (nextRangeExists) {
			...
			if (FCKBrowserInfo.IsIE) nextRangeExists = false;
			else {
				if (rangeQueue == null) {
					rangeQueue = [];
					...
					for (
						var i = 1;
						selectionObject && i < selectionObject.rangeCount;
						i++
					)
						rangeQueue.push(selectionObject.getRangeAt(i));
				}
				if (rangeQueue.length < 1) nextRangeExists = false;
				else {
					var internalRange = FCKW3CRange.CreateFromRange(
						doc,
						rangeQueue.shift()
					);
		...
		while (listGroups.length > 0) {
			...
			if (state == FCK_TRISTATE_OFF) {
				...
				else this._CreateList(groupObj, listsCreated);
			} else if (
	...
	_ChangeListType: function(groupObj, markerObj, listsCreated) {
		...
		for (var i = 0; i < groupObj.contents.length; i++) {
			...
			if (!itemNode || itemNode._FCK_ListItem_Processed) continue;
			selectedListItems.push(itemNode);
			FCKDomTools.SetElementMarker(
				markerObj,
				itemNode,
				"_FCK_ListItem_Processed",
				true
			);
		}
		var fakeParent = FCKTools.GetElementDocument(
			groupObj.root
		...
		if (contents.length == 1 && contents[0] == groupObj.root) {
			...
			while (contents[0].firstChild)
				divBlock.appendChild(
					contents[0].removeChild(contents[0].firstChild)
				);
	...
	_RemoveList: function(groupObj, markerObj) {
		...
		for (var i = lastListIndex + 1; i < listArray.length; i++) {
			if (listArray[i].indent > listArray[i - 1].indent + 1) {
				var indentOffset =
					listArray[i - 1].indent + 1 - listArray[i].indent;
		...
		if (
			groupObj.root.nextSibling == null ||
