...
FCKSaveCommand.prototype.Execute = function() {
	...
	if (typeof oForm.onsubmit == "function") {
		...
		if (bRet != null && bRet === false) return;
	}
	...
	if (typeof oForm.submit == "function") oForm.submit();
	else oForm.submit.click();
};
...
FCKSourceCommand.prototype.Execute = function() {
	if (FCKConfig.SourcePopup) {
		// Until v2.2, it was mandatory for FCKBrowserInfo.IsGecko.
...
FCKUnlinkCommand.prototype.Execute = function() {
	...
	if (FCKBrowserInfo.IsGeckoLike) {
		...
		if (oLink) FCKTools.RemoveOuterTags(oLink);
...
FCKUnlinkCommand.prototype.GetState = function() {
	...
	if (state == FCK_TRISTATE_OFF && FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
		...
		var bIsAnchor =
			oLink && oLink.name.length > 0 && oLink.href.length == 0;
		if (bIsAnchor) state = FCK_TRISTATE_DISABLED;
	}
...
FCKVisitLinkCommand.prototype = {
	GetState: function() {
		...
		if (state == FCK_TRISTATE_OFF) {
			...
			if (!el.href) state = FCK_TRISTATE_DISABLED;
		}
	...
	Execute: function() {
		...
		if (!/:\/\//.test(url)) {
			...
			if (/^\//.test(url)) {
				try {
					...
					baseHref =
						parentWindow.document.location.protocol +
						"://" +
		...
		if (!window.open(url, "_blank")) alert(FCKLang.VisitLinkBlocked);
	}
};
...
FCKCutCopyCommand.prototype = {
	Execute: function() {
		...
		if (!enabled) alert(FCKLang["PasteError" + this.Name]);
	},
...
FCKAnchorDeleteCommand.prototype = {
	Execute: function() {
		if (FCK.Selection.GetType() == "Control") {
			...
			if (oFakeImage) {
				if (
					oFakeImage.tagName == "IMG" &&
					oFakeImage.getAttribute("_fckanchor")
				)
					oAnchor = FCK.GetRealElement(oFakeImage);
				else oFakeImage = null;
			}
