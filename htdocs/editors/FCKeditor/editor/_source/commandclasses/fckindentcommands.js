...
FCKIndentCommand.prototype = {
	Execute: function() {
		...
		var nearestListBlock = FCKDomTools.GetCommonParentNode(
			range.StartNode || range.StartContainer,
			range.EndNode || range.EndContainer,
			["ul", "ol"]
		);
		if (nearestListBlock) this._IndentList(range, nearestListBlock);
		else this._IndentBlock(range);
	...
	GetState: function() {
		...
		var listNode = FCKDomTools.GetCommonParentNode(
			startContainer,
			endContainer,
			["ul", "ol"]
		);
		...
		if (listNode) {
			if (this.Name.IEquals("outdent")) return FCK_TRISTATE_OFF;
		...
		if (!firstBlock) return FCK_TRISTATE_DISABLED;

		if (FCKIndentCommand._UseIndentClasses) {
			var indentClass = firstBlock.className.match(
			...
			if (
				(this.Name == "outdent" && indentStep == 0) ||
				(this.Name == "indent" &&
					indentStep == FCKConfig.IndentClasses.length)
			)
				return FCK_TRISTATE_DISABLED;
			return FCK_TRISTATE_OFF;
		} else {
			...
			if (indent <= 0) return FCK_TRISTATE_DISABLED;
			return FCK_TRISTATE_OFF;
		}
	},
	...
	_IndentBlock: function(range) {
		...
		var commonParents = FCKDomTools.GetCommonParents(
		...
		while ((block = iterator.GetNextParagraph())) {
			...
			if (FCKIndentCommand._UseIndentClasses) {
				...
				var indentClass = block.className.match(
					FCKIndentCommand._ClassNameRegex
				);
				var indentStep = 0;
				if (indentClass != null) {
				...
				if (this.Name.IEquals("outdent")) indentStep--;
				else if (this.Name.IEquals("indent")) indentStep++;
				indentStep = Math.min(
					indentStep,
				...
				if (indentStep < 1) block.className = className;
				else
					block.className =
				...
				var currentOffset = parseInt(
					block.style[this.IndentCSSProperty],
					10
				);
				...
				currentOffset =
					Math.ceil(currentOffset / this.Offset) * this.Offset;
	...
	_IndentList: function(range, listNode) {
		...
		while (stopFlag == false) {
			if (block == endContainer) stopFlag = true;
		...
		for (
			var i = startItem._FCK_ListArray_Index;
			i <= lastItem._FCK_ListArray_Index;
			i++
		)
			listArray[i].indent += indentOffset;
		for (
			var i = lastItem._FCK_ListArray_Index + 1;
