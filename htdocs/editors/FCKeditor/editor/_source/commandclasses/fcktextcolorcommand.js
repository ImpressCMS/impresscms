...
FCKTextColorCommand.prototype.SetColor = function(color) {
	...
	var style = FCKStyles.GetStyle(
		"_FCK_" + (this.Type == "ForeColor" ? "Color" : "BackColor")
	);

	if (!color || color.length == 0) FCK.Styles.RemoveStyle(style);
	else {
...
FCKTextColorCommand.prototype._CreatePanelBody = function(
	targetDocument,
	targetDiv
) {
	...
	if (!FCKBrowserInfo.IsIE) oDiv.style.width = "96%";
	...
	while (iCounter < aColors.length) {
		...
		for (var i = 0; i < 8; i++, iCounter++) {
			...
			if (iCounter >= aColors.length) oDiv.style.visibility = "hidden";
			else
				FCKTools.AddEventListenerEx(
					oDiv,
					"click",
