...
var FCKJustifyCommand = function(alignValue) {
	...
	this.IsDefaultAlign =
		(alignValue == "left" && contentDir == "ltr") ||
	...
	if (cssClassName && cssClassName.length > 0)
		this._CssClassRegex = new RegExp(
			"(?:^|\\s+)" + cssClassName + "(?=$|\\s)"
		);
};
...
FCKJustifyCommand._GetClassNameRegex = function() {
	...
	if (classes) {
		for (var i = 0; i < 4; i++) {
			...
			if (className && className.length > 0) names.push(className);
		}
	}
...
FCKJustifyCommand.prototype = {
	Execute: function() {
		...
		if (currentState == FCK_TRISTATE_DISABLED) return;
	...
	GetState: function() {
		...
		if (FCKBrowserInfo.IsIE)
			...
			currentAlign = FCK.EditorWindow.getComputedStyle(
				firstBlock,
				""
			).getPropertyValue("text-align");
		currentAlign = currentAlign.replace(/(-moz-|-webkit-|start|auto)/i, "");
		if (
			(!currentAlign && this.IsDefaultAlign) ||
