...
FCKBlockQuoteCommand.prototype = {
	Execute: function() {
		...
		if (FCKBrowserInfo.IsIE) {
			...
			if (
				...
				while ((cursor = cursor.nextSibling)) {
					if (
						FCKListsLib.BlockElements[cursor.nodeName.toLowerCase()]
					)
			...
			if (
				...
				while ((cursor = cursor.nextSibling)) {
					if (
						...
						else FCKDomTools.MoveNode(bEnd, cursor, true);
					}
				}
			}
		}
		...
		if (state == FCK_TRISTATE_OFF) {
			...
			if (paragraphs.length < 1) {
				para = range.Window.document.createElement(
					FCKConfig.EnterMode.IEquals("p") ? "p" : "div"
				);
				range.InsertNode(para);
				para.appendChild(
					range.Window.document.createTextNode("\ufeff")
				);
			...
			for (var i = 0; i < paragraphs.length; i++) {
				block = paragraphs[i];
				commonParent = FCKDomTools.GetCommonParents(
					block.parentNode,
					commonParent
				).pop();
			}
			...
			while (paragraphs.length > 0) {
				...
				if (block != lastBlock) tmp.push(block);
				lastBlock = block;
			}
			...
			while (tmp.length > 0) {
				...
				if (block.nodeName.IEquals("blockquote")) {
					var docFrag = FCKTools.GetElementDocument(
						block
			...
			while (moveOutNodes.length > 0) {
				...
				if (node == node.parentNode.firstChild)
					bqBlock.parentNode.insertBefore(
						bqBlock.removeChild(node),
						bqBlock
					);
				...
				else FCKDomTools.BreakParent(node, node.parentNode, range);
			...
			for (var i = processedBlockquoteBlocks.length - 1; i >= 0; i--) {
				...
				if (noBlockLeft(bqBlock)) FCKDomTools.RemoveNode(bqBlock);
			}
			FCKDomTools.ClearAllMarkers(elementMarkers);
					...
					if (node.nodeName.IEquals("div")) {
						...
						var needBeginBr =
							...
							!FCKListsLib.BlockBoundaries[
								node.previousSibling.nodeName.toLowerCase()
							];
						if (firstTime && needBeginBr)
							docFrag.appendChild(
								FCKTools.GetElementDocument(node).createElement(
									"br"
								)
							);
						var needEndBr =
							node.nextSibling &&
							!FCKListsLib.BlockBoundaries[
								node.nextSibling.nodeName.toLowerCase()
							];
	...
	GetState: function() {
		...
		var path = new FCKElementPath(
			FCKSelection.GetBoundaryParentElement(true)
		);
