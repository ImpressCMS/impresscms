...
FCKFitWindow.prototype.Execute = function() {
	...
	if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
		...
		oRange = !FCKBrowserInfo.IsIE && [
			eTextarea.selectionStart,
			eTextarea.selectionEnd
		];
	...
	if (!this.IsMaximized) {
		...
		else eMainWindow.addEventListener("resize", FCKFitWindow_Resize, true);
		...
		if (!FCKBrowserInfo.IsIE) {
			eEditorFrameStyle.borderRight = eEditorFrameStyle.borderBottom =
				"9999px solid white";
		...
		if (editorPos.x != 0) eEditorFrameStyle.left = -1 * editorPos.x + "px";
		if (editorPos.y != 0) eEditorFrameStyle.top = -1 * editorPos.y + "px";
		...
		if (FCKBrowserInfo.IsIE)
			...
			eMainWindow.removeEventListener(
				"resize",
				FCKFitWindow_Resize,
				true
			);
	...
	if (FCK.EditMode == FCK_EDITMODE_WYSIWYG) FCK.EditingArea.MakeEditable();
...
FCKFitWindow.prototype.GetState = function() {
	if (FCKConfig.ToolbarLocation != "In") return FCK_TRISTATE_DISABLED;
	else return this.IsMaximized ? FCK_TRISTATE_ON : FCK_TRISTATE_OFF;
};
