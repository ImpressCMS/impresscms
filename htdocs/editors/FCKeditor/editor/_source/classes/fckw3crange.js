...
FCKW3CRange.prototype = {
	...
	extractContents: function() {
		...
		if (!this.collapsed) this._ExecContentsAction(1, docFrag);
	...
	cloneContents: function() {
		...
		if (!this.collapsed) this._ExecContentsAction(2, docFrag);
	...
	_ExecContentsAction: function(action, docFrag) {
		...
		if (endNode.nodeType == 3) endNode = endNode.splitText(endOffset);
		else {
			...
			if (endNode.childNodes.length > 0) {
				...
				if (endOffset > endNode.childNodes.length - 1) {
					...
					endNode = FCKDomTools.InsertAfterNode(
						endNode.lastChild,
						this._Document.createTextNode("")
					);
					removeEndNode = true;
				} else endNode = endNode.childNodes[endOffset];
			}
		}
		...
		if (startNode.nodeType == 3) {
			...
			if (startNode == endNode) endNode = startNode.nextSibling;
		} else {
			...
			if (startOffset == 0) {
				// Let's create a temporary node and mark it for removal.
				startNode = startNode.insertBefore(
					this._Document.createTextNode(""),
				...
				// Let's create a temporary node and mark it for removal.
				startNode = startNode.appendChild(
					this._Document.createTextNode("")
				);
				removeStartNode = true;
			} else
		...
		if (docFrag) clone = docFrag.RootNode;
		...
		for (var j = i; j < startParents.length; j++) {
			...
			if (clone && levelStartNode != startNode)
				// action = 0 = Delete
				levelClone = clone.appendChild(
					levelStartNode.cloneNode(levelStartNode == startNode)
				);
		...
		for (var k = i; k < endParents.length; k++) {
			...
			// For Extract and Clone, we must clone this level.
			if (action > 0 && levelStartNode != endNode)
				// action = 0 = Delete
				levelClone = clone.appendChild(
					levelStartNode.cloneNode(levelStartNode == endNode)
				);
		...
		if (action == 2) {
			...
			if (
				topStart &&
				topEnd &&
				(startNode.parentNode != topStart.parentNode ||
					endNode.parentNode != topEnd.parentNode)
			) {
		...
		if (removeStartNode) startNode.parentNode.removeChild(startNode);
