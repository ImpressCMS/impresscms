...
var FCKPanel = function(parentWindow) {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		var oIFrame = (this._IFrame = this._Window.document.createElement(
			"iframe"
		));
...
FCKPanel.prototype.Preload = function(x, y, relElement) {
	...
	if (this._Popup) this._Popup.show(x, y, 0, 0, relElement);
};
...
FCKPanel.prototype.ResizeForSubpanel = function(panel, width, height) {
	...
	if (width == 0 && height == 0) {
		...
		if (this.Subpanel !== panel) return false;
		...
		if (this.IncreasedX >= width && this.IncreasedY >= height) return false;
...
FCKPanel.prototype.Show = function(x, y, relElement, width, height) {
	...
	if (this._Popup) {
		...
		if (FCKBrowserInfo.IsIE7) {
			if (
				this.ParentPanel &&
				this.ParentPanel.ResizeForSubpanel(
					this,
					iMainWidth,
		...
		if (this.IsRTL) {
			if (this.IsContextMenu) x = x - iMainWidth + 1;
			else if (relElement)
		...
		var oPos = FCKTools.GetDocumentPosition(
			this._Window,
			relElement.nodeType == 9
				? FCKTools.IsStrictMode(relElement)
		...
		var positionedAncestor = FCKDomTools.GetPositionedAncestor(
			this._IFrame.parentNode
		);
		if (positionedAncestor) {
			var nPos = FCKTools.GetDocumentPosition(
				FCKTools.GetElementWindow(positionedAncestor),
				positionedAncestor
			);
			oPos.x -= nPos.x;
			oPos.y -= nPos.y;
		...
		if (this.IsRTL) {
			if (this.IsContextMenu) x = x - iMainWidth + 1;
			else if (relElement) x = x + relElement.offsetWidth - iMainWidth;
		} else {
		...
		this._resizeTimer = setTimeout(function() {
			var iWidth =
				eMainNode.offsetWidth || eMainNode.firstChild.offsetWidth;
...
FCKPanel.prototype.Hide = function(ignoreOnHide, ignoreFocusManagerUnlock) {
	if (this._Popup) this._Popup.hide();
		...
		if (!ignoreOnHide) FCKTools.RunFunction(this.OnHide, this);
	}
};
...
function FCKPanel_Window_OnBlur(e, panel) {
	...
	if (panel._LockCounter == 0) FCKTools.RunFunction(panel.Hide, panel);
}
