...
FCKToolbar.prototype.AddButton = function(
	name,
	label,
	tooltip,
	...
	if (typeof iconPathOrStripInfoArrayOrIndex == "number")
		iconPathOrStripInfoArrayOrIndex = [
			this.DefaultIconsStrip,
			this.DefaultIconSize,
			iconPathOrStripInfoArrayOrIndex
		];

	var oButton = new FCKToolbarButtonUI(
		name,
		label,
		tooltip,
		iconPathOrStripInfoArrayOrIndex,
...
function FCKToolbar_OnItemClick(item) {
	...
	if (oToolbar.OnItemClick) oToolbar.OnItemClick(oToolbar, item);
}
