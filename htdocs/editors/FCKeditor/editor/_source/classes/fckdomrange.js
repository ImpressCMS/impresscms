...
FCKDomRange.prototype = {
	_UpdateElementInfo: function() {
		...
		if (!innerRange) this.Release(true);
		else {
			...
			if (innerRange.collapsed) {
				...
				if (eStart != eEnd) oElementPath = new FCKElementPath(eEnd);
				...
				if (innerRange.endOffset == 0) {
					...
					if (eEndNode) eEndNode = eEndNode.previousSibling;
	...
	Collapse: function(toStart) {
		if (this._Range) this._Range.collapse(toStart);
	...
	Clone: function() {
		...
		if (this._Range) oClone._Range = this._Range.cloneRange();
	...
	MoveToElementEditStart: function(targetElement) {
		...
		while (targetElement && targetElement.nodeType == 1) {
			...
			else if (editableElement) break; // If we already found an editable element, stop the loop.

			targetElement = targetElement.firstChild;
		}

		if (editableElement) this.MoveToElementStart(editableElement);
	},

	InsertNode: function(node) {
		if (this._Range) this._Range.insertNode(node);
	},
	...
	CheckStartOfBlock: function() {
		...
		if (bIsStartOfBlock != undefined) return bIsStartOfBlock;
		...
		if (offset > 0) {
			...
			} else currentNode = container.childNodes[offset - 1];
		}

		// We'll not have a currentNode if the container was a text node, or
		// the offset is zero.
		if (!currentNode)
			currentNode = FCKDomTools.GetPreviousSourceNode(
				container,
				true,
				null,
				block
	...
	CheckEndOfBlock: function(refreshSelection) {
		...
		if (isEndOfBlock != undefined) return isEndOfBlock;
		...
		} else currentNode = container.childNodes[offset];
		...
		if (!currentNode)
			currentNode = FCKDomTools.GetNextSourceNode(
				container,
				true,
				null,
				block
		...
		while (currentNode) {
			switch (currentNode.nodeType) {
				case 1:
					...
					if (FCKListsLib.InlineChildReqElements[nodeName]) break;
		...
		if (refreshSelection) this.Select();
	...
	CreateBookmark: function(includeNodes) {
		...
		} else this.MoveToPosition(eStartSpan, 4);
	...
	GetBookmarkNode: function(bookmark, start) {
		...
		else return bookmark.EndNode || doc.getElementById(bookmark.EndId);
	},
	...
	MoveToBookmark: function(bookmark, preserveBookmark) {
		...
		if (!preserveBookmark) FCKDomTools.RemoveNode(eStartSpan);

		// If collapsed, the end span will not be available.
		if (eEndSpan) {
			this.SetEnd(eEndSpan, 3);

			if (!preserveBookmark) FCKDomTools.RemoveNode(eEndSpan);
	...
	CreateBookmark2: function() {
		...
		if (
			addrStart.nodeType == 1 &&
			addrStart.childNodes[bookmark.Start[0]] &&
		...
		if (
			addrEnd.nodeType == 1 &&
			addrEnd.childNodes[bookmark.End[0]] &&
		...
		bookmark.Start = FCKDomTools.GetNodeAddress(addrStart, true).concat(
			bookmark.Start
		);
		bookmark.End = FCKDomTools.GetNodeAddress(addrEnd, true).concat(
			bookmark.End
		);
		return bookmark;
	},
	...
	MoveToBookmark2: function(bookmark) {
		...
		var curStart = FCKDomTools.GetNodeFromAddress(
			this.Window.document,
			bookmark.Start.slice(0, -1),
			true
		);
		var curEnd = FCKDomTools.GetNodeFromAddress(
			this.Window.document,
			bookmark.End.slice(0, -1),
			true
		);
		...
		while (curEnd.nodeType == 3 && endOffset > curEnd.length) {
			if (!curEnd.nextSibling || curEnd.nextSibling.nodeType != 3) break;
	...
	SetStart: function(targetElement, position, noInfoUpdate) {
		...
		if (!oRange) oRange = this._Range = this.CreateRange();
		...
		if (!noInfoUpdate) this._UpdateElementInfo();
	},
	...
	Expand: function(unit) {
		...
		switch (unit) {
			...
			case "inline_elements":
					...
					if (oNode) {
						while (
							FCKListsLib.InlineNonEmptyElements[
								oNode.nodeName.toLowerCase()
							]
						) {
							this._Range.setStartBefore(oNode);

							if (oNode != oNode.parentNode.firstChild) break;
				...
				if (
					(oNode.nodeType == 3 && offset >= oNode.nodeValue.length) ||
					(oNode.nodeType == 1 &&
						offset >= oNode.childNodes.length) ||
					...
					if (oNode) {
						while (
							...
							if (oNode != oNode.parentNode.lastChild) break;
			...
			case "list_contents":
				...
				if (
					...
					if (oNode.nodeType == 1) {
						var lastNode =
							oNode.childNodes[this._Range.startOffset];
						if (lastNode)
							oNode = FCKDomTools.GetPreviousSourceNode(
								lastNode,
								true
							);
						else oNode = oNode.lastChild || oNode;
					}
				...
				if (
					...
					if (oNode.nodeType == 1)
						oNode =
							oNode.childNodes[this._Range.endOffset] ||
							oNode.lastChild;
	...
	SplitBlock: function(forceBlockTag) {
		...
		if (this.StartBlockLimit == this.EndBlockLimit) {
			...
			if (blockTag != "br") {
				...
				if (!eEndBlock) eEndBlock = this.FixBlock(false, blockTag);
			}

			// Get the range position.
			var bIsStartOfBlock =
				eStartBlock != null && this.CheckStartOfBlock();
	...
	FixBlock: function(isStart, blockTag) {
		...
		if (
			FCKDomTools.CheckIsEmptyElement(oFixedBlock, function(element) {
	...
	Release: function(preserveWindow) {
		if (!preserveWindow) this.Window = null;
	...
	GetTouchedStartNode: function() {
		...
		if (range.collapsed || container.nodeType != 1) return container;
