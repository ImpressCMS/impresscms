...
FCKStyle.prototype = {
	...
	RemoveFromRange: function(range, selectIt, updateRange) {
		...
		if (range.CheckIsCollapsed()) {
			...
			for (var i = 0; i < path.Elements.length; i++) {
				...
				if (this.CheckElementRemovable(pathElement)) {
					if (
						isBoundary &&
						!FCKDomTools.CheckIsEmptyElement(pathElement, function(
							el
						) {
						...
						if (pathElementName == this.Element) {
							...
							for (var att in styleAttribs) {
								if (
									FCKDomTools.HasAttribute(pathElement, att)
								) {
									switch (att) {
										case "style":
											this._RemoveStylesFromElement(
												pathElement
											);
											break;
										...
										case "class":
											...
											if (
												...
												this.GetFinalAttributeValue(att)
											)
												continue;
				...
				} else if (isBoundary) boundaryElements.push(pathElement);

				// Check if we are still in a boundary (at the same side).
				isBoundary =
					isBoundary &&
					((isBoundaryRight &&
						!FCKDomTools.GetNextSibling(pathElement)) ||
						(!isBoundaryRight &&
							!FCKDomTools.GetPreviousSibling(pathElement)));
				...
				if (
					...
					for (var j = 0; j <= boundaryLimitIndex; j++) {
						var newElement = FCKDomTools.CloneElement(
							boundaryElements[j]
						);
			...
			if (selectIt) range.SelectBookmark(bookmark);

			if (updateRange) range.MoveToBookmark(bookmark);
		...
		while (currentNode) {
			...
			if (currentNode.nodeType == 1) {
				...
				} else mayRemove = !!styleOverrides[elementName];
	...
	CheckElementRemovable: function(element, fullMatch) {
		...
		if (elementName == this.Element) {
			...
			if (!fullMatch && !FCKDomTools.HasAttributes(element)) return true;
			...
			for (var att in attribs) {
				if (att == "_length") continue;

				if (
					this._CompareAttributeValues(
						att,
						FCKDomTools.GetAttributeValue(element, att),
						this.GetFinalAttributeValue(att) || ""
					)
				) {
					allMatched = true;
				...
				if (FCKDomTools.HasAttribute(element, attName)) {
					...
					if (
						...
						(typeof attValue == "string" &&
							FCKDomTools.GetAttributeValue(element, attName) ==
								attValue) ||
						attValue.test(
							FCKDomTools.GetAttributeValue(element, attName)
						)
	...
	_RemoveStylesFromElement: function(element) {
		...
		if (elementStyle.length > 0 && pattern.length == 0) return;
		...
		else element.style.cssText = elementStyle.replace(regex, "");
	},
	...
	_RemoveNoAttribElement: function(element) {
		...
		if (!FCKDomTools.HasAttributes(element)) {
			...
			if (firstChild != lastChild) this._MergeSiblings(lastChild);
		}
	},
	...
	BuildElement: function(targetDoc, element) {
		...
		if (attribs) {
			for (var att in attribs) {
				...
				if (att.toLowerCase() == "class") el.className = attValue;
				else el.setAttribute(att, attValue);
			}
		}
	...
	GetFinalAttributeValue: function(attName) {
		...
		if (!attValue && attName == "style") return this.GetFinalStyleValue();
	...
	SetVariable: function(name, value) {
		...
		if (!variables) variables = this._Variables = {};
	...
	_FromPre: function(doc, block, newBlock) {
		...
		innerHTML = innerHTML.replace(/^[ \t]+|[ \t]+$/g, function(
			match,
			offset,
			s
		) {
	...
	_ToPre: function(doc, block, newBlock) {
		...
		} else newBlock.innerHTML = results.join("");
	...
	_CheckAndMergePre: function(previousBlock, preBlock) {
		...
		if (
			previousBlock !=
			FCKDomTools.GetPreviousSourceElement(preBlock, true)
		)
			return;
		...
		var innerHTML =
			previousBlock.innerHTML.replace(/\n$/, "") +
			"\n\n" +
		...
		else preBlock.innerHTML = innerHTML;
	...
	_CheckAndSplitPre: function(newBlock) {
		...
		while (cursor) {
			...
			if (
				...
				lastNewBlock = FCKDomTools.InsertAfterNode(
					lastNewBlock || newBlock,
					FCKDomTools.CloneElement(newBlock)
				);
	...
	_ApplyBlockStyle: function(range, selectIt, updateRange) {
		...
		if (selectIt) bookmark = range.CreateBookmark();
		...
		while (
			...
			if (toPre) newBlock = this._ToPre(doc, block, newBlock);
			else if (fromPre) newBlock = this._FromPre(doc, block, newBlock);
			// Convering from a regular block to another regular block.
			else FCKDomTools.MoveChildren(block, newBlock);
			...
			} else if (fromPre) this._CheckAndSplitPre(newBlock); // Split <br><br> in successive <pre>s.
		}
	...
	_ApplyInlineStyle: function(range, selectIt, updateRange) {
		...
		while (currentNode) {
			...
			var nodeName =
				nodeType == 1 ? currentNode.nodeName.toLowerCase() : null;
			...
			if (!nodeName || elementDTD[nodeName]) {
				...
				if (
					(FCK.DTD[currentNode.parentNode.nodeName.toLowerCase()] ||
						FCK.DTD.span)[elementName] ||
					!FCK.DTD[elementName]
				) {
					// This node will be part of our range, so if it has not
					// been started, place its start right before the node.
					if (!range.CheckHasRange()) range.SetStart(currentNode, 3);
					...
					if (nodeType != 1 || currentNode.childNodes.length == 0) {
						...
						// to the range.
						while (
							includedNode == parentNode.lastChild &&
						...
						if (
							includedNode == includedNode.parentNode.lastChild &&
							!elementDTD[
								includedNode.parentNode.nodeName.toLowerCase()
							]
						)
							applyStyle = true;
					} else {
			...
			if (
				...
				if (styleNode.innerHTML.RTrim().length > 0) {
					...
					if (!FCKBrowserInfo.IsIE) styleNode.normalize();
				}
	...
	_MergeSiblings: function(element, attribs) {
		if (
			!element ||
			element.nodeType != 1 ||
			!FCKListsLib.InlineNonEmptyElements[element.nodeName.toLowerCase()]
		)
			return;
	...
	_MergeNextSibling: function(element, attribs) {
		...
		var hasBookmark =
			sibling &&
			sibling.nodeType == 1 &&
			sibling.getAttribute("_fck_bookmark");
		if (hasBookmark) sibling = sibling.nextSibling;
		...
		if (
			...
			if (this._CheckAttributesMatch(sibling, attribs)) {
				...
				if (innerSibling) this._MergeNextSibling(innerSibling);
			}
		}
	},
	...
	_MergePreviousSibling: function(element, attribs) {
		...
		var hasBookmark =
			...
			sibling.nodeType == 1 &&
			sibling.getAttribute("_fck_bookmark");
		if (hasBookmark) sibling = sibling.previousSibling;
		...
		if (
			...
			if (this._CheckAttributesMatch(sibling, attribs)) {
				...
				if (innerSibling) this._MergePreviousSibling(innerSibling);
			}
		}
	},
	...
	_GetStyleText: function() {
		...
		if (stylesText.length > 0) stylesText += ";";
	...
	_GetAttribsForComparison: function() {
		...
		if (styleAttribs) {
			for (var styleAtt in styleAttribs) {
				attribs[styleAtt.toLowerCase()] = styleAttribs[
					styleAtt
				].toLowerCase();
			}
		}
	...
	_GetOverridesForComparison: function() {
		...
		if (overridesDesc) {
			...
			for (var i = 0; i < overridesDesc.length; i++) {
				...
				overrideEl =
					overrides[elementName] || (overrides[elementName] = {});
	...
	_CreateElementAttribsForComparison: function(element) {
		...
		for (var i = 0; i < element.attributes.length; i++) {
			...
			if (att.specified) {
				attribs[
					att.nodeName.toLowerCase()
				] = FCKDomTools.GetAttributeValue(element, att).toLowerCase();
				attribsCount++;
			}
		}
