...
FCKEditingArea.prototype.Start = function(html, secondCall) {
	...
	if (this.Mode == FCK_EDITMODE_WYSIWYG) {
		...
		if (FCKBrowserInfo.IsIE)
			...
			} else this._BodyHTML = html; // Invalid HTML input.
		}
		...
		if (FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE) {
			window._FCKHtmlToLoad = html.replace(
				/<head>/i,
				"<head>" + sOverrideError
			);
			oIFrame.src =
		...
		if (FCKBrowserInfo.IsAIR) FCKAdobeAIR.EditingArea_Start(oDoc, html);
...
function FCKEditingArea_Document_AttributeNodeModified(evt) {
	...
	if (editingArea._timer) window.clearTimeout(editingArea._timer);
