...
FCKEnterKey.prototype.DoEnter = function(mode, hasShift) {
	...
	if (
		sMode == "br" ||
		(parentPath.Block && parentPath.Block.tagName.toLowerCase() == "pre")
	)
		return this._ExecuteEnterBr();
	else return this._ExecuteEnterBlock(sMode);
};
...
FCKEnterKey.prototype.DoBackspace = function() {
	...
	if (
		FCKBrowserInfo.IsIE &&
		this._CheckIsAllContentsIncluded(oRange, this.Window.document.body)
	) {
	...
	if (
		oRange.StartBlockLimit == oRange.EndBlockLimit &&
		oStartBlock &&
		oEndBlock
	) {
		if (!isCollapsed) {
		...
		if (oRange.CheckStartOfBlock()) {
			...
			var ePrevious = FCKDomTools.GetPreviousSourceElement(
				oCurrentBlock,
				true,
				["BODY", oRange.StartBlockLimit.nodeName],
				["UL", "OL"]
			);
...
FCKEnterKey.prototype._ExecuteBackspace = function(
	range,
	previous,
	currentBlock
) {
	...
	if (
		!previous &&
		currentBlock &&
		currentBlock.nodeName.IEquals("LI") &&
	...
	if (previous && currentBlock) {
		...
		if (
			FCKListsLib.EmptyElements[sPreviousName] != null ||
			sPreviousName == "table"
		) {
...
FCKEnterKey.prototype.DoDelete = function() {
	...
	if (
		...
		if (eCurrentCell) {
			...
			if (eNextCell != eCurrentCell) return true;
		}
	...
	if (oSplitInfo) {
		...
		if (eNextBlock) {
			...
			FCKDomTools.MoveNode(
				ePreviousBlock,
				ePreviousBlock.previousSibling
			);
		}
		...
		if (!bIsStartOfBlock && !bIsEndOfBlock) {
			...
			if (
				eNextBlock.nodeName.IEquals("li") &&
				eNextBlock.firstChild &&
				eNextBlock.firstChild.nodeName.IEquals(["ul", "ol"])
			)
				eNextBlock.insertBefore(
					FCKTools.GetElementDocument(eNextBlock).createTextNode(
						"\xa0"
					),
					eNextBlock.firstChild
				);
			// Move the selection to the end block.
			if (eNextBlock) oRange.MoveToElementEditStart(eNextBlock);
		} else {
			if (
				bIsStartOfBlock &&
				bIsEndOfBlock &&
			...
			if (elementPath) {
				for (
					var i = 0, len = elementPath.Elements.length;
					i < len;
					i++
				) {
			...
			if (FCKBrowserInfo.IsGeckoLike) FCKTools.AppendBogusBr(eNewBlock);
			...
			// Move the selection to the new block.
			oRange.MoveToElementEditStart(
				bIsStartOfBlock && !bIsEndOfBlock ? eNextBlock : eNewBlock
			);
		}
...
FCKEnterKey.prototype._ExecuteEnterBr = function(blockTag) {
	...
	if (oRange.StartBlockLimit == oRange.EndBlockLimit) {
		...
		var sStartBlockTag = oRange.StartBlock
			? oRange.StartBlock.tagName.toUpperCase()
			: "";
		...
		if (!bHasShift && bIsEndOfBlock && /^H[1-6]$/.test(sStartBlockTag)) {
			...
			oRange.SetStart(
				oRange.StartBlock.nextSibling,
				FCKBrowserInfo.IsIE ? 3 : 1
			);
		} else {
			var eLineBreak;
			bIsPre = sStartBlockTag.IEquals("pre");
			if (bIsPre)
				eLineBreak = this.Window.document.createTextNode(
					FCKBrowserInfo.IsIE ? "\r" : "\n"
				);
			else eLineBreak = this.Window.document.createElement("br");
			...
			if (FCKBrowserInfo.IsIE) oRange.SetStart(eLineBreak, 4);
			else oRange.SetStart(eLineBreak.nextSibling, 1);

			if (!FCKBrowserInfo.IsIE) {
				var dummy = null;
				if (FCKBrowserInfo.IsOpera)
					dummy = this.Window.document.createElement("span");
				else dummy = this.Window.document.createElement("br");

				eLineBreak.parentNode.insertBefore(
					dummy,
