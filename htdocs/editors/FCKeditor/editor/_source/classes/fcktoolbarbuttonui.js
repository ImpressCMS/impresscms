...
var FCKToolbarButtonUI = function(
	name,
	label,
	tooltip,
	iconPathOrStripInfoArray,
	style,
	state
) {
	this.Name = name;
...
FCKToolbarButtonUI.prototype.Create = function(parentElement) {
	...
	if (FCKBrowserInfo.IsGecko) oMainElement.onmousedown = FCKTools.CancelEvent;
	...
	if (this.Style == FCK_TOOLBARITEM_ONLYICON && !this.ShowArrow) {
		...
		else oCell.appendChild(this._CreatePaddingElement(oDoc));
...
function FCKToolbarButtonUI_OnMouseOver(ev, button) {
	if (button.State == FCK_TRISTATE_OFF) this.className = "TB_Button_Off_Over";
	else if (button.State == FCK_TRISTATE_ON)
		this.className = "TB_Button_On_Over";
}

function FCKToolbarButtonUI_OnMouseOut(ev, button) {
	if (button.State == FCK_TRISTATE_OFF) this.className = "TB_Button_Off";
	else if (button.State == FCK_TRISTATE_ON) this.className = "TB_Button_On";
}
