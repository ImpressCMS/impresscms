...
FCKDomRangeIterator.prototype = {
	...
	GetNextParagraph: function() {
		...
		if (!this._LastNode) {
			...
			range.Expand(
				this.ForceBrBreak ? "list_contents" : "block_contents"
			);
		...
		while (currentNode) {
			...
			if (includeNode) range.SetEnd(currentNode, 4, true);
		...
		if (!block) {
			...
			if (
				...
				block = this.Range.Window.document.createElement(
