...
var FCKContextMenu = function(parentWindow, langDir) {
	...
	if (FCKBrowserInfo.IsGecko)
		oPanel.Document.addEventListener(
			"draggesture",
			function(e) {
				e.preventDefault();
				return false;
...
*/
FCKContextMenu.prototype.AddItem = function(
	name,
	label,
	...
	var oItem = this._MenuBlock.AddItem(
		name,
		label,
		iconPathOrStripInfoArrayOrIndex,
		isDisabled,
...
FCKContextMenu.prototype.AttachToElement = function(element) {
	...
	else element._FCKContextMenu = this;
};
	...
	while (el) {
		if (el._FCKContextMenu) {
			...
			if (!overrideButton) {
				...
				overrideButton = FCKContextMenu_OverrideButton = doc.createElement(
					"input"
				);
...
function FCKContextMenu_AttachedElement_OnContextMenu(ev, fckContextMenu, el) {
	if (
		(fckContextMenu.CtrlDisable && (ev.ctrlKey || ev.metaKey)) ||
	...
	if (fckContextMenu._MenuBlock.Count() == 0) return false;
