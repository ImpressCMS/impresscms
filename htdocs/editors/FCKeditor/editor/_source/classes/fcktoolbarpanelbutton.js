...
var FCKToolbarPanelButton = function(commandName, label, tooltip, style, icon) {
	...
	var oUIButton = (this._UIButton = new FCKToolbarButtonUI(
		commandName,
		label,
		tooltip,
		oIcon,
...
FCKToolbarPanelButton.prototype.Create = function(parentElement) {
	...
	var oPanel = FCK.ToolbarSet.CurrentInstance.Commands.GetCommand(
		this.CommandName
	)._Panel;
...
FCKToolbarPanelButton.prototype.RegisterPanel = function(oPanel) {
	if (oPanel._FCKToolbarPanelButton) return;

	oPanel._FCKToolbarPanelButton = this;

	var eLineDiv = oPanel.Document.body.appendChild(
		oPanel.Document.createElement("div")
	);
	eLineDiv.style.position = "absolute";
	eLineDiv.style.top = "0px";

	var eLine = (oPanel._FCKToolbarPanelButtonLineDiv = eLineDiv.appendChild(
		oPanel.Document.createElement("IMG")
	));
...
function FCKToolbarPanelButton_OnButtonClick(toolbarButton) {
	...
	var oCommand = FCK.ToolbarSet.CurrentInstance.Commands.GetCommand(
		oButton.CommandName
	);
