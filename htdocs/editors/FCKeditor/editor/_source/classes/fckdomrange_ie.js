...
FCKDomRange.prototype.MoveToSelection = function() {
	...
	if (oSel.type != "Control") {
		...
		this._Range.setStart(
			eMarkerStart.parentNode,
			FCKDomTools.GetIndexOf(eMarkerStart)
		);
		eMarkerStart.parentNode.removeChild(eMarkerStart);

		// Set the end boundary.
		this._Range.setEnd(
			eMarkerEnd.parentNode,
			FCKDomTools.GetIndexOf(eMarkerEnd)
		);
...
FCKDomRange.prototype.SelectBookmark = function(bookmark, forceExpand) {
	...
	if (!eStartMarker) return;

	var eEndMarker;
	if (!bIsCollapsed) eEndMarker = this.GetBookmarkNode(bookmark, false);
	...
	if (eEndMarker) {
		...
		if (bIsStartMarkerAlone) {
			...
			eStartMarker.parentNode.insertBefore(
...
FCKDomRange.prototype._GetSelectionMarkerTag = function(toStart) {
	...
	if (oRange.parentElement().document != doc) return null;
