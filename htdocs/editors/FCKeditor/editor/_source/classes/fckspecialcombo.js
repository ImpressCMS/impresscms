...
FCKSpecialCombo.prototype.AddItem = function(id, html, label, bgColor) {
	...
	if (FCKBrowserInfo.IsIE) oDiv.style.width = "100%";
...
FCKSpecialCombo.prototype.SelectItemByLabel = function(itemLabel, setLabel) {
	for (var id in this.Items) {
		...
		if (oDiv.FCKItemLabel == itemLabel) {
			...
			if (setLabel) this.SetLabel(itemLabel);
		}
	}
};
...
FCKSpecialCombo.prototype.Create = function(targetElement) {
	...
	var eOuterTable = (this._OuterTable = targetElement.appendChild(
	...
	var oField = FCKTools.AppendElement(
		eOuterTable.rows[0].insertCell(-1),
		"div"
	);
	if (bShowLabel) {
		...
		oField.innerHTML =
			...
			'<td><img class="TB_Button_Padding" src="' +
			FCK_SPACER_PATH +
			'" /></td>' +
			'<td class="TB_Text">' +
			this.Caption +
			...
			FCK_SPACER_PATH +
			'" /></td>' +
			'<td class="TB_ButtonArrow"><img src="' +
			FCKConfig.SkinPath +
...
function FCKSpecialCombo_Cleanup() {
	...
	if (this.Items) {
		for (var key in this.Items) this.Items[key] = null;
	}
}
...
function FCKSpecialCombo_OnClick(e, specialCombo) {
	...
	if (specialCombo.Enabled) {
		...
		if (FCKBrowserInfo.IsIE) oPanel.Preload(0, this.offsetHeight, this);
