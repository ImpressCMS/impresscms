...
FCKKeystrokeHandler.prototype.SetKeystrokes = function() {
	...
	for (var i = 0; i < arguments.length; i++) {
		...
		if (typeof keyDef[0] == "object")
			// It is an array with arrays defining the keystrokes.
			this.SetKeystrokes.apply(this, keyDef);
		else {
			if (keyDef.length == 1)
				// If it has only one element, remove the keystroke.
...
function _FCKKeystrokeHandler_OnKeyDown(ev, keystrokeHandler) {
	...
	if (ev.ctrlKey || ev.metaKey) keyModifiers += CTRL;

	if (ev.shiftKey) keyModifiers += SHIFT;

	if (ev.altKey) keyModifiers += ALT;
	...
	if (keystrokeValue) {
		...
		if (
			keystrokeValue === true ||
			!(
				keystrokeHandler.OnKeystroke &&
				keystrokeHandler.OnKeystroke.apply(
					keystrokeHandler,
					keystrokeValue
				)
			)
	...
	if (
		cancelIt ||
		(keystrokeHandler.CancelCtrlDefaults &&
			keyModifiers == CTRL &&
			(keystroke < 33 || keystroke > 40))
	) {
		keystrokeHandler._CancelIt = true;

		if (ev.preventDefault) return ev.preventDefault();
