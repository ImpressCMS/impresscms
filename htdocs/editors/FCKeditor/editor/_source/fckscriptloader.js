...
FCKScriptLoader.Load = function(scriptName) {
	...
	if (scriptName in FCKScriptLoader._LoadedScripts) return;
	...
	if (
		oScriptInfo.Compatibility == FCK_GENERIC ||
		oScriptInfo.Compatibility == FCK_GENERIC_SPECIFIC
	)
		this._LoadScript(sBaseScriptName + ".js");

	if (
		oScriptInfo.Compatibility == FCK_SPECIFIC ||
		oScriptInfo.Compatibility == FCK_GENERIC_SPECIFIC
	) {
		if (this._IsIE) this._LoadScript(sBaseScriptName + "_ie.js");
		else this._LoadScript(sBaseScriptName + "_gecko.js");
	}
};
...
FCKScriptLoader._LoadScript = function(scriptPathFromSource) {
	document.write(
		'<script type="text/javascript" src="' +
			this.FCKeditorPath +
			"editor/_source/" +
			scriptPathFromSource +
			'"></script>'
	);
};

FCKScriptLoader.AddScript = function(
	scriptName,
	scriptBasePath,
	dependency,
	compatibility
...
FCKScriptLoader.AddScript(
	"FCKTools",
	"internals/",
	["FCKJSCoreExtensions", "FCKBrowserInfo"],
	FCK_GENERIC_SPECIFIC
);
FCKScriptLoader.AddScript(
	"FCKXHtml",
