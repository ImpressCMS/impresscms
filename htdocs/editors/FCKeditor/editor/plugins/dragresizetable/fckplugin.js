var FCKDragTableHandler = {
	...
	_GetBorderCells: function(w, tableNode, tableMap, mouse) {
		...
		for (var i = 0; i < tableNode.rows.length; i++) {
			...
			for (var j = 0; j < r.cells.length; j++) cells.push(r.cells[j]);
		}
		...
		*/
		...
		var colIdx = FCKTableHandler._GetCellIndexSpan(
			tableMap,
			rowIdx,
			rbCell
		);
		...
		if (!lbCell) return null;

		// Abort if too far from the border.
		lxDist = mouse.x - FCKTools.GetWindowPosition(w, lbCell).x;
		if (lxDist < 0 && minRxDist < 0 && minRxDist < -2) return null;
		if (lxDist > 0 && minRxDist > 0 && lxDist > 3) return null;
	...
	_GetResizeBarPosition: function() {
		...
		return FCKTableHandler._GetCellIndexSpan(
			this._TableMap,
			row.rowIndex,
			this._RightCell
		);
	...
	_ResizeBarMouseDownListener: function(evt) {
		...
		if (FCKBrowserInfo.IsIE)
			FCKDragTableHandler._ResizeBar.filters.item(
				"DXImageTransform.Microsoft.Alpha"
			).opacity = 50;
		else FCKDragTableHandler._ResizeBar.style.opacity = 0.5;
		...
		for (var r = 0; r < FCKDragTableHandler._TableMap.length; r++) {
			...
			var leftPosition = FCKTools.GetWindowPosition(
				FCK.EditorWindow,
				leftCell
			);
			var rightPosition = FCKTools.GetWindowPosition(
				FCK.EditorWindow,
				rightCell
			);
			var leftPadding = FCKDragTableHandler._GetCellPadding(
				table,
				leftCell
			);
			var rightPadding = FCKDragTableHandler._GetCellPadding(
				table,
				rightCell
			);
	...
	_ResizeBarMouseUpListener: function(evt) {
		...
		if (FCKDragTableHandler._LastX == null) return;

		// Calculate the delta value.
		var deltaX =
			FCKDragTableHandler._LastX - FCKDragTableHandler._OriginalX;
		...
		for (var i = 0; i < tableMap.length; i++) {
			for (var j = 0; j < tableMap[i].length; j++) {
				...
				if (tableMap[i][j - 1] != cell) cell.width = colArray[j].width;
				else
					cell.width =
						parseInt(cell.width, 10) +
		...
		for (var i = 0; i < processedCells.length; i++) {
			...
			else delete processedCells[i]._Processed;
		}
	...
	_ResizeBarMouseMoveListener: function(evt) {
		...
		else return FCKDragTableHandler._MouseDragHandler(FCK, evt);
	},
	...
	_GetCellPadding: function(table, cell) {
		...
		if (typeof window.getComputedStyle == "function") {
			...
			cssGuess =
				parseInt(styleObj.getPropertyValue("padding-left"), 10) +
				parseInt(styleObj.getPropertyValue("padding-right"), 10);
		} else
			cssGuess =
				parseInt(cell.currentStyle.paddingLeft, 10) +
				parseInt(cell.currentStyle.paddingRight, 10);

		var cssRuntime = cell.style.padding;
		if (isFinite(cssRuntime)) cssGuess = parseInt(cssRuntime, 10) * 2;
		else {
			cssRuntime = cell.style.paddingLeft;
			if (isFinite(cssRuntime)) cssGuess = parseInt(cssRuntime, 10);
		...
		if (isNaN(attrGuess)) attrGuess = 0;
		if (isNaN(cssGuess)) cssGuess = 0;
	...
	_GetCellWidth: function(table, cell) {
		...
		if (isNaN(clientWidth)) clientWidth = 0;
	...
	_MouseFindHandler: function(FCK, evt) {
		...
		if (this._ResizeBar && this._LeftCell) {
			var leftPos = FCKTools.GetWindowPosition(
				FCK.EditorWindow,
				this._LeftCell
			);
			var rightPos = FCKTools.GetWindowPosition(
				FCK.EditorWindow,
				this._RightCell
			);
			...
			if (lxDist >= 0 && rxDist <= 0) inRangeFlag = true;
			else if (rxDist > 0 && lxDist <= 3) inRangeFlag = true;
			else if (lxDist < 0 && rxDist >= -2) inRangeFlag = true;
			if (inRangeFlag) {
				this._ShowResizeBar(
					FCK.EditorWindow,
	...
	_MouseDragHandler: function(FCK, evt) {
		...
		if (mouse.x >= this._MaximumX - 5) mouse.x = this._MaximumX - 5;
		if (mouse.x <= this._MinimumX + 5) mouse.x = this._MinimumX + 5;

		var docX = mouse.x + FCKTools.GetScrollPosition(window).X;
		this._ResizeBar.style.left =
	...
	_ShowResizeBar: function(w, table, mouse) {
		if (this._ResizeBar == null) {
			...
			else paddingStyles.opacity = 0.1;
		...
		var paddingStyles = {
			...
			left:
				offset.x +
				mouse.x +
				FCKTools.GetScrollPosition(w).X -
				barWidth / 2 +
				"px"
		};
		if (FCKBrowserInfo.IsIE)
			paddingBar.filters.item(
				"DXImageTransform.Microsoft.Alpha"
			).opacity = 10;
		else paddingStyles.opacity = 0.1;
		...
		} else visibleBar = paddingBar.getElementsByTagName("div")[0];
