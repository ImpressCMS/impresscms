...
var oPlaceholderItem = new FCKToolbarButton(
	"Placeholder",
	FCKLang.PlaceholderBtn
);
oPlaceholderItem.IconPath =
	FCKPlugins.Items["placeholder"].Path + "placeholder.gif";
...
FCKPlaceholders._SetupClickListener = function() {
	...
	FCK.EditorDocument.addEventListener(
		"click",
		FCKPlaceholders._ClickListener,
		true
	);
};
...
FCKPlaceholders.Exist = function(name) {
	...
	for (var i = 0; i < aSpans.length; i++) {
		if (aSpans[i]._fckplaceholder == name) return true;
	}
	...
	FCKPlaceholders.Redraw = function() {
		...
		for (var i = 0; i < aPlaholders.length; i++) {
			if (oRange.findText(aPlaholders[i])) {
				...
				oRange.pasteHTML(
					'<span style="color: #000000; background-color: #ffff00" contenteditable="false" _fckplaceholder="' +
						sName +
						'">' +
						aPlaholders[i] +
						"</span>"
				...
				if (aPieces[i].length > 0) {
					if (aPieces[i].indexOf("[[") == 0) {
						...
						aNodes[n].parentNode.insertBefore(
							FCK.EditorDocument.createTextNode(aPieces[i]),
							aNodes[n]
						);
				}
			}
...
FCKXHtml.TagProcessors["span"] = function(node, htmlNode) {
	...
	else FCKXHtml._AppendChildNodes(node, htmlNode, false);
