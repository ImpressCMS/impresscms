...
if (FCKBrowserInfo.IsAIR) {
		...
		return {
			...
			ToolbarSet_GetOutElement: function(win, outMatch) {
				...
				while (targetWindowParts.length > 0) {
					...
					if (part.length > 0) toolbarTarget = toolbarTarget[part];
				}

				toolbarTarget = toolbarTarget.document.getElementById(
					outMatch[2]
				);
			},
			...
			ToolbarSet_InitOutFrame: function(doc) {
				...
				head.appendChild(doc.createElement("base")).href =
				...
				doc.body.innerHTML = document.getElementById(
					"xToolbarSpace"
				).innerHTML;
			}
		};
	...
	(function() {
		...
		FCKPanel_Window_OnFocus = function(e, panel) {
			...
			if (panel._focusTimer) clearTimeout(panel._focusTimer);
		};
		...
		FCK.StartEditor = function() {
			...
			window.FCK_InternalCSS =
				FCKConfig.BasePath + "css/fck_internal.css";
			window.FCK_ShowTableBordersCSS =
				FCKConfig.BasePath + "css/fck_showtableborders_gecko.css";
