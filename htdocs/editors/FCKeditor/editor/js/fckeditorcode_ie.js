...
(function(A) {
	...
	if (A.IsSafari)
		A.IsSafari3 = parseFloat(s.match(/ applewebkit\/(\d+)/)[1]) < 526;
})(FCKBrowserInfo);
var FCKURLParams = {};
(function() {
...
var FCK = {
	...
	StartEditor: function() {
		...
		this.EditingArea = new FCKEditingArea(
			document.getElementById("xEditingArea")
		);
	...
	GetData: function(A) {
		...
		var D = FCK.DataProcessor.ConvertToDataFormat(
			C ? B.documentElement : B.body,
			!C,
	...
	SetData: function(A, B) {
		...
		if (FCKBrowserInfo.IsIE && FCK.EditorDocument) {
			FCK.EditorDocument.detachEvent(
				"onselectionchange",
	...
	InsertElement: function(A) {
		...
		if (FCKListsLib.BlockElements[B] != null) {
			...
			C.InsertNode(A);
			var D = FCKDomTools.GetNextSourceElement(
				A,
				false,
				null,
				["hr", "br", "param", "img", "area", "input"],
				true
			);
			if (!D && FCKConfig.EnterMode != "br") {
				D = this.EditorDocument.body.appendChild(
					this.EditorDocument.createElement(FCKConfig.EnterMode)
				);
...
function _FCK_PaddingNodeListener() {
	...
	if (!FCKBrowserInfo.IsIE && FCKDomTools.PaddingNode) {
		...
		if (A && A.rangeCount == 1) {
			...
			if (
				B.collapsed &&
				B.startContainer == FCK.EditorDocument.body &&
				B.startOffset == 0
			) {
		...
		if (C && C.nodeName.IEquals("body")) {
			if (
				...
				if (
					FCKSelection._GetSelectionDocument(
...
function _FCK_EditingArea_OnLoad() {
	...
	FCK.EnterKeyHandler = new FCKEnterKey(
		FCK.EditorWindow,
		FCKConfig.EnterMode,
...
function _FCK_GetEditorAreaStyleTags() {
	return (
		FCKTools.GetStyleHtml(FCKConfig.EditorAreaCSS) +
...
function _FCK_KeystrokeHandler_OnKeystroke(A, B) {
	...
	return (
		C.Execute.apply(C, FCKTools.ArgumentsToArray(arguments, 2)) !== false
	);
}
(function() {
...
var FCKTempBin = {
	...
	ToHtml: function() {
		for (var i = 0; i < this.Elements.length; i++) {
			this.Elements[i] =
				"<div>&nbsp;" + this.Elements[i].outerHTML + "</div>";
	...
	ToElements: function() {
		...
		for (var i = 0; i < this.Elements.length; i++) {
			if (this.Elements[i].isHtml) {
				...
				this.Elements[i] = A.firstChild.removeChild(
					A.firstChild.lastChild
				);
			}
		}
	}
...
var FCKFocusManager = (FCK.FocusManager = {
	...
	AddWindow: function(A, B) {
		...
		FCKTools.AddEventListener(
			C,
			"focus",
			B ? FCKFocusManager_Win_OnFocus_Area : FCKFocusManager_Win_OnFocus
		);
	},
...
function Doc_OnPaste() {
	...
	var B = FCK.Paste(
		!FCKConfig.ForcePasteAsPlainText && !FCKConfig.AutoDetectPasteFromWord
	);
...
FCK.InsertHtml = function(A) {
	...
	A =
		'<span id="__fakeFCKRemove__" style="display:none;">fakeFCKRemove</span>' +
		A;
...
FCK.CreateLink = function(A, B) {
	...
	if (!(D && !FCKTools.GetElementAscensor(E, "a")))
...
function Doc_OnMouseDown(A) {
	...
	if (
		e.nodeName &&
		e.nodeName.IEquals("input") &&
		e.type.IEquals(["radio", "checkbox"]) &&
		!e.disabled
	) {
		e.disabled = true;
...
if (document.location.protocol == "file:") {
	FCKConfig.BasePath = decodeURIComponent(
...
var FCKDomTools = {
	...
	EnforcePaddingNode: function(A, B) {
		...
		try {
			if (
				A.body.lastChild &&
				(A.body.lastChild.nodeType != 1 ||
					A.body.lastChild.tagName.toLowerCase() == B.toLowerCase())
			)
				return;
		} catch (e) {
			return;
		...
		this.PaddingNode = C;
		if (
			A.body.childNodes.length == 1 &&
			A.body.firstChild.nodeType == 1 &&
			A.body.firstChild.tagName.toLowerCase() == "br" &&
			(A.body.firstChild.getAttribute("_moz_dirty") != null ||
	...
	CheckAndRemovePaddingNode: function(A, B, C) {
		...
		try {
			if (
				...
				(D.firstChild &&
					D.firstChild.nodeValue != "\xa0" &&
					String(D.firstChild.tagName).toLowerCase() != "br")
			) {
				...
				if (I.nodeName.IEquals(FCKConfig.EnterMode) && I.firstChild) {
					...
					if (
						FCKListsLib.BlockBoundaries[
							I.firstChild.nodeName.toLowerCase()
						]
					) {
	...
	GetPositionedAncestor: function(A) {
		...
		while (B != FCKTools.GetElementDocument(B).documentElement) {
			if (this.GetCurrentElementStyle(B, "position") != "static")
				return B;
			if (
				B == FCKTools.GetElementDocument(B).documentElement &&
				currentWindow != w
			)
	...
	ScrollIntoView: function(A, B) {
		...
		if (B === false) {
			...
			E +=
				parseInt(
					this.GetCurrentElementStyle(A, "marginBottom") || 0,
					10
				) || 0;
		}
...
FCKTools.FixCssUrls = function(A, B) {
	...
	return B.replace(/url\s*\(([\s'"]*)(.*?)([\s"']*)\)/g, function(
		match,
		opener,
		path,
		closer
...
FCKTools.IsStrictMode = function(A) {
	return (
		"CSS1Compat" ==
		(A.compatMode || (FCKBrowserInfo.IsSafari ? "CSS1Compat" : null))
	);
};
...
FCKTools.AppendBogusBr = function(A) {
	...
	if (
		!B ||
		(B.getAttribute("type", 2) != "_moz" &&
			B.getAttribute("_moz_dirty") == null)
	) {
...
FCKTools.GetDocumentPosition = function(w, A) {
	...
	if (
		FCKDomTools.GetCurrentElementStyle(w.document.body, "position") !=
			"static" ||
...
FCKTools.GetVoidUrl = function() {
	if (FCK_IS_CUSTOM_DOMAIN)
		return (
			"javascript: void( function(){document.open();document.write('<html><head><title></title></head><body></body></html>');document.domain = '" +
			FCK_RUNTIME_DOMAIN +
			"';document.close();}() ) ;"
		);
	if (FCKBrowserInfo.IsIE) {
...
FCKTools.ResetStyles = function(A) {
	A.style.cssText =
		"margin:0;padding:0;border:0;background-color:transparent;background-image:none;";
};
...
function _AttachFormSubmitToAPI() {
	...
	if (A) {
		...
		if (
			!A._FCKOriginalSubmit &&
			(typeof A.submit == "function" ||
				(!A.submit.tagName && !A.submit.length))
		) {
...
function FCKeditorAPI_Cleanup() {
	if (
		window.FCKConfig &&
		FCKConfig.MsWebBrowserControlCompat &&
...
var FCKRegexLib = {
	...
	ToReplace: /___fcktoreplace:([\w]+)/gi,
...
var FCKLanguageManager = (FCK.Language = {
	...
	Initialize: function() {
		...
		this.ActiveLanguage.Name = this.AvailableLanguages[
...
FCKXHtml.GetXHTML = function(A, B, C) {
	FCKDomTools.CheckAndRemovePaddingNode(
		FCKTools.GetElementDocument(A),
		FCKConfig.EnterMode
	);
...
FCKXHtml._AppendNode = function(A, B) {
	...
	switch (B.nodeType) {
		case 1:
			...
			if (
				FCKBrowserInfo.IsGecko &&
				(B.hasAttribute("_moz_editor_bogus_node") ||
					B.getAttribute("type") == "_moz")
			) {
		...
		case 3:
			...
			return this._AppendTextNode(
				A,
				B.nodeValue.ReplaceNewLineChars(" ")
			);
		case 8:
		...
		default:
			A.appendChild(
				this.XML.createComment(
					"Element not supported - Type: " +
						B.nodeType +
						" Name: " +
						B.nodeName
				)
	...
	if (C)
		A.appendChild(
			this.XML.createTextNode(
				B.replace(FCKXHtmlEntities.EntitiesRegex, FCKXHtml_GetEntity)
			)
		);
	return C;
};
...
FCKXHtml.TagProcessors = {
	...
	iframe: function(A, B) {
		...
		A.appendChild(
			FCKXHtml.XML.createTextNode(FCKXHtml._AppendSpecialItem(C))
		);
		return A;
	},
	img: function(A, B) {
	...
	pre: function(A, B) {
		...
		if (C && C.nodeType == 3)
			A.appendChild(
				FCKXHtml.XML.createTextNode(FCKXHtml._AppendSpecialItem("\r\n"))
			);
	...
	script: function(A, B) {
		...
		A.appendChild(
			FCKXHtml.XML.createTextNode(FCKXHtml._AppendSpecialItem(B.text))
		);
		return A;
	},
	span: function(A, B) {
...
FCKXHtml._AppendAttributes = function(A, B, C, D) {
	...
	if (bHasStyle || B.style.cssText.length > 0) {
		...
		var J = B.style.cssText.replace(
			FCKRegexLib.StyleProperties,
...
FCKXHtml.TagProcessors["textarea"] = FCKXHtml.TagProcessors[
	"select"
] = function(A, B) {
	if (B.name) FCKXHtml._AppendAttribute(A, "name", B.name);
	A = FCKXHtml._AppendChildNodes(A, B);
...
FCKUndo._CheckIsBookmarksEqual = function(A, B) {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		return (
			this._CompareCursors(A.Start, B.Start) == 0 &&
...
FCKUndo.SaveUndoStep = function() {
	...
	if (
		...
		this._CheckIsBookmarksEqual(B, this.SavedData[this.CurrentIndex][1])
	)
		return;
	else if (
		this.CurrentIndex == 0 &&
	...
	if (this.Mode == 0) {
		...
		if (FCKBrowserInfo.IsIE)
			...
			if (E && F) {
				var G = A.substr(
					E[1].length,
					A.length - E[1].length - F[1].length
				);
				A = E[1] + "&nbsp;" + F[1];
				if (
					FCKBrowserInfo.IsGecko &&
...
FCKKeystrokeHandler.prototype.AttachToElement = function(A) {
	...
	if (
		...
		FCKTools.AddEventListenerEx(
			A,
			"keypress",
			_FCKKeystrokeHandler_OnKeyPress,
			this
		);
};
...
FCKStyle.prototype = {
		...
		if (A.CheckIsCollapsed()) {
			...
			for (var i = 0; i < I.Elements.length; i++) {
				...
				if (this.CheckElementRemovable(O)) {
					if (
						L &&
						!FCKDomTools.CheckIsEmptyElement(O, function(el) {
							return el != H;
						})
					) {
						M = O;
							...
							for (var Q in E) {
								if (FCKDomTools.HasAttribute(O, Q)) {
									switch (Q) {
										...
										case "class":
											if (
												FCKDomTools.GetAttributeValue(
													O,
													Q
												) !=
												this.GetFinalAttributeValue(Q)
											)
												continue;
										default:
				...
				L =
					L &&
					((K && !FCKDomTools.GetNextSibling(O)) ||
						(!K && !FCKDomTools.GetPreviousSibling(O)));
					...
					for (var Q in E) {
						if (FCKDomTools.HasAttribute(Z, Q)) {
							switch (Q) {
								...
								case "class":
									if (
										FCKDomTools.GetAttributeValue(Z, Q) !=
										this.GetFinalAttributeValue(Q)
									)
										continue;
								default:
	...
	CheckElementRemovable: function(A, B) {
		...
		if (C == this.Element) {
			...
			for (var F in D) {
				...
				if (
					this._CompareAttributeValues(
						F,
						FCKDomTools.GetAttributeValue(A, F),
						this.GetFinalAttributeValue(F) || ""
					)
				) {
					E = true;
					if (!B) break;
			...
			for (var i = 0; i < D.length; i++) {
				...
				if (FCKDomTools.HasAttribute(A, H)) {
					...
					if (
						I == null ||
						(typeof I == "string" &&
							FCKDomTools.GetAttributeValue(A, H) == I) ||
			...
			for (var F in B) {
				if (FCKDomTools.HasAttribute(E, F)) {
					switch (F) {
						...
						case "class":
							if (
								FCKDomTools.GetAttributeValue(E, F) !=
								this.GetFinalAttributeValue(F)
							)
								continue;
						default:
			...
			for (var i = 0; i < C.length; i++) {
				...
				if (FCKDomTools.HasAttribute(A, D)) {
					...
					if (
						E == null ||
						(E.test &&
							E.test(FCKDomTools.GetAttributeValue(A, D))) ||
						(typeof E == "string" &&
							FCKDomTools.GetAttributeValue(A, D) == E)
					)
	...
	GetFinalAttributeValue: function(A) {
		...
		if (B && this._Variables)
			B = B.Replace(
				FCKRegexLib.StyleVariableAttName,
				this._GetVariableReplace,
				this
			);
		return B;
	},
	...
	_CheckAndSplitPre: function(A) {
		...
		while (C) {
			...
			if (
				D &&
				D.nextSibling &&
				C.nodeName.IEquals("br") &&
				D.nodeName.IEquals("br")
			) {
				FCKDomTools.RemoveNode(C);
				C = D.nextSibling;
				FCKDomTools.RemoveNode(D);
				B = FCKDomTools.InsertAfterNode(
					B || A,
					FCKDomTools.CloneElement(A)
				);
				continue;
			}
			if (B) {
	...
	_ApplyInlineStyle: function(A, B, C) {
		...
		while (M) {
			...
			if (!P || G[P]) {
				if (
					(FCK.DTD[M.parentNode.nodeName.toLowerCase()] ||
						FCK.DTD.span)[F] ||
					!FCK.DTD[F]
				) {
	...
	_FixBookmarkStart: function(A) {
		...
		while ((B = A.nextSibling)) {
			if (
				B.nodeType == 1 &&
				FCKListsLib.InlineNonEmptyElements[B.nodeName.toLowerCase()]
			) {
	...
	_CreateElementAttribsForComparison: function(A) {
		...
		for (var i = 0; i < A.attributes.length; i++) {
			...
			if (D.specified) {
				B[D.nodeName.toLowerCase()] = FCKDomTools.GetAttributeValue(
					A,
					D
				).toLowerCase();
				C++;
...
var FCKStyles = (FCK.Styles = {
	...
	GetStyles: function() {
		...
		if (!A) {
			A = this._GetStyles = FCKTools.Merge(
...
FCKDomRange.prototype = {
	...
	CheckStartOfBlock: function() {
		...
		while (F) {
			switch (F.nodeType) {
				case 1:
					if (
						!FCKListsLib.InlineChildReqElements[
	...
	MoveToBookmark2: function(A) {
		var B = FCKDomTools.GetNodeFromAddress(
			this.Window.document,
			A.Start.slice(0, -1),
			true
		);
		var C = FCKDomTools.GetNodeFromAddress(
			this.Window.document,
			A.End.slice(0, -1),
			true
		);
	...
	FixBlock: function(A, B) {
		...
		if (
			FCKDomTools.CheckIsEmptyElement(D, function(element) {
...
FCKDomRange.prototype.SelectBookmark = function(A, B) {
	...
	if (G) {
		...
		D =
			B ||
			!F.previousSibling ||
			F.previousSibling.nodeName.toLowerCase() == "br";
	...
	GetNextParagraph: function() {
		...
		if (!A) {
			...
			if (
				...
				A = this.Range.Window.document.createElement(
					FCKConfig.EnterMode == "p" ? "p" : "div"
				);
		...
		if (!this._NextNode)
			this._NextNode =
				C || A == I
					? null
					: FCKDomTools.GetNextSourceNode(A, true, null, I);
		return A;
	}
};
	...
	_ExecContentsAction: function(A, B) {
		...
		for (var j = i; j < I.length; j++) {
			...
			if (K && levelStartNode != C)
				levelClone = K.appendChild(
					levelStartNode.cloneNode(levelStartNode == C)
				);
		...
		for (var k = i; k < J.length; k++) {
			...
			if (A > 0 && levelStartNode != D)
				levelClone = K.appendChild(
					levelStartNode.cloneNode(levelStartNode == D)
				);
		...
		if (A == 2) {
			...
			if (
				topStart &&
				topEnd &&
				(C.parentNode != topStart.parentNode ||
					D.parentNode != topEnd.parentNode)
			) {
...
FCKEnterKey.prototype.DoBackspace = function() {
	...
	if (B.StartBlockLimit == B.EndBlockLimit && H && I) {
		...
		if (B.CheckStartOfBlock()) {
			var K = B.StartBlock;
			var L = FCKDomTools.GetPreviousSourceElement(
				K,
				true,
				["BODY", B.StartBlockLimit.nodeName],
				["UL", "OL"]
			);
...
FCKEnterKey.prototype.DoDelete = function() {
	...
	if (B.CheckIsCollapsed() && B.CheckEndOfBlock(FCKBrowserInfo.IsGeckoLike)) {
		...
		var E = FCKDomTools.GetNextSourceElement(
			C,
			true,
			[B.StartBlockLimit.nodeName],
			["UL", "OL", "TR"],
			true
		);
		if (D) {
...
FCKEnterKey.prototype._ExecuteEnterBlock = function(A, B) {
	...
	if (D) {
		...
		if (!G && !H) {
			if (
				F.nodeName.IEquals("li") &&
				F.firstChild &&
				F.firstChild.nodeName.IEquals(["ul", "ol"])
			)
				F.insertBefore(
					FCKTools.GetElementDocument(F).createTextNode("\xa0"),
					F.firstChild
				);
...
FCKEnterKey.prototype._ExecuteEnterBr = function(A) {
	...
	if (B.StartBlockLimit == B.EndBlockLimit) {
		...
		if (!F && D && /^H[1-6]$/.test(E)) {
			FCKDomTools.InsertAfterNode(
				B.StartBlock,
				this.Window.document.createElement("br")
			);
			if (FCKBrowserInfo.IsGecko)
				FCKDomTools.InsertAfterNode(
					B.StartBlock,
			...
			G = E.IEquals("pre");
			if (G)
				H = this.Window.document.createTextNode(
					FCKBrowserInfo.IsIE ? "\r" : "\n"
				);
		...
		while (i >= 0 && (C = B[i--])) {
			if (C.name.length > 0) {
				if (C.innerHTML !== "") {
					...
					var D = FCKDocumentProcessor_CreateFakeImage(
						"FCK__Anchor",
						C.cloneNode(true)
					);
...
var FCKEmbedAndObjectProcessor = (function() {
	...
	var B = function(el) {
		...
		var D;
		var E = (D = FCKDocumentProcessor_CreateFakeImage(
			"FCK__UnknownObject",
			C
		));
	...
	return FCKTools.Merge(FCKDocumentProcessor.AppendNew(), {
		...
		RefreshView: function(placeHolder, original) {
			if (original.getAttribute("width") > 0)
				placeHolder.style.width = FCKTools.ConvertHtmlSizeToStyle(
					original.getAttribute("width")
				);
			if (original.getAttribute("height") > 0)
				placeHolder.style.height = FCKTools.ConvertHtmlSizeToStyle(
...
FCKSelection.Restore = function() {
	if (this.SelectionData) {
		...
		try {
			if (
				String(
					this._GetSelectionDocument(FCK.EditorDocument.selection)
	...
	for (var i = 0; i < F.height; i++) {
		...
		for (var j = 0; j < F.width; j++) {
			...
			while (K.childNodes.length > 0) {
				...
				if (
					L.nodeType != 1 ||
					(L.getAttribute("type", 2) != "_moz" &&
						L.getAttribute("_moz_dirty") != null)
				) {
					I.appendChild(L);
					J++;
				}
	...
	if (D && D.tagName.Equals("TD", "TH")) B[0] = D;
		...
		if (D) {
			for (var i = 0; i < D.cells.length; i++) {
				...
				if (
					C.inRange(E) ||
					(C.compareEndPoints("StartToStart", E) >= 0 &&
						C.compareEndPoints("StartToEnd", E) <= 0) ||
					(C.compareEndPoints("EndToStart", E) >= 0 &&
						C.compareEndPoints("EndToEnd", E) <= 0)
				) {
...
FCKSpellCheckCommand.prototype.Execute = function() {
	switch (FCKConfig.SpellChecker) {
		...
		case "SpellerPages":
			FCKDialog.OpenDialog(
				"FCKDialog_SpellCheck",
				"Spell Check",
				"dialog/fck_spellerpages.html",
				440,
				480
			);
			break;
...
FCKSpellCheckCommand.prototype._RunIeSpell = function() {
	try {
		...
		if (e.number == -2146827859) {
			...
			alert(
				"Error Loading ieSpell: " + e.message + " (" + e.number + ")"
			);
	}
};
...
FCKTextColorCommand.prototype.SetColor = function(A) {
	FCKUndo.SaveUndoStep();
	var B = FCKStyles.GetStyle(
		"_FCK_" + (this.Type == "ForeColor" ? "Color" : "BackColor")
	);
...
function FCKTextColorCommand_MoreOnClick(A, B) {
	...
	FCKDialog.OpenDialog(
		"FCKDialog_Color",
		FCKLang.DlgColorTitle,
		"dialog/fck_colorselector.html",
		410,
		320,
		FCKTools.Bind(B, B.SetColor)
	);
}
...
FCKTableCommand.prototype.GetState = function() {
	if (FCK.EditorDocument != null && FCKSelection.HasAncestorNode("TABLE")) {
		switch (this.Name) {
			...
			case "TableMergeCells":
				if (
					FCKTableHandler.CheckIsSelectionRectangular() &&
				...
				for (var i = L.Elements.length - 1; i >= 0; i--) {
					...
					if (P.nodeName.IEquals(["ol", "ul"])) {
						...
						if (Q) Q.contents.push(I);
							...
							Q = { root: P, contents: [I] };
							F.push(Q);
							FCKDomTools.SetElementMarker(
								G,
								P,
								"_FCK_ListGroupObject",
								Q
							);
						}
						N = true;
...
FCKJustifyCommand.prototype = {
	Execute: function() {
		...
		while ((F = E.GetNextParagraph())) {
			...
			if (D) {
				var G = F.className.replace(
					FCKJustifyCommand._GetClassNameRegex(),
					""
				);
				if (B == 0) {
					if (G.length > 0) G += " ";
...
FCKIndentCommand._InitIndentModeParameters = function() {
	if (FCKConfig.IndentClasses && FCKConfig.IndentClasses.length > 0) {
		...
		this._ClassNameRegex = new RegExp(
			"(?:^|\\s+)(" + FCKConfig.IndentClasses.join("|") + ")(?=$|\\s)"
		);
...
FCKIndentCommand.prototype = {
	Execute: function() {
		...
		var C = FCKDomTools.GetCommonParentNode(
			A.StartNode || A.StartContainer,
			A.EndNode || A.EndContainer,
			["ul", "ol"]
		);
	...
	_IndentList: function(A, B) {
		...
		for (var i = J._FCK_ListArray_Index; i <= K._FCK_ListArray_Index; i++)
			M[i].indent += I;
		for (
			var i = K._FCK_ListArray_Index + 1;
			i < M.length && M[i].indent > N;
			i++
		)
			M[i].indent += I;
...
FCKBlockQuoteCommand.prototype = {
	Execute: function() {
		...
		if (FCKBrowserInfo.IsIE) {
			...
			if (
				D &&
				D.parentNode.nodeName.IEquals("blockquote") &&
				!D.previousSibling
			) {
				F = D;
			...
			if (
				E &&
				E.parentNode.nodeName.IEquals("blockquote") &&
				!E.previousSibling
			) {
				F = E;
				while ((F = F.nextSibling)) {
		...
		if (A == 0) {
			...
			if (I.length < 1) {
				para = B.Window.document.createElement(
					FCKConfig.EnterMode.IEquals("p") ? "p" : "div"
				);
				B.InsertNode(para);
			...
			while (K.length > 0) {
				...
				if (H.nodeName.IEquals("blockquote")) {
					var M = FCKTools.GetElementDocument(
						H
			...
			var U = function(N) {
				for (var i = 0; i < N.childNodes.length; i++) {
					if (
						FCKListsLib.BlockElements[
							N.childNodes[i].nodeName.toLowerCase()
						]
					)
						return false;
				}
				...
				while (S.length) {
					...
					if (W.nodeName.IEquals("div")) {
						...
						var c =
							...
							W.previousSibling &&
							!FCKListsLib.BlockBoundaries[
								W.previousSibling.nodeName.toLowerCase()
							];
						if (a && c)
							M.appendChild(
								FCKTools.GetElementDocument(W).createElement(
									"br"
								)
							);
						var d =
							W.nextSibling &&
							!FCKListsLib.BlockBoundaries[
								W.nextSibling.nodeName.toLowerCase()
							];
						while (W.firstChild)
...
FCKCommands.GetCommand = function(A) {
	...
	switch (A) {
		...
		case "Templates":
			B = new FCKDialogCommand(
				"Templates",
				FCKLang.DlgTemplatesTitle,
				"dialog/fck_template.html",
				380,
				450
			);
			break;
		...
		case "Anchor":
			B = new FCKDialogCommand(
				"Anchor",
				FCKLang.DlgAnchorTitle,
				"dialog/fck_anchor.html",
				370,
				160
			);
			break;
		...
		case "BulletedList":
			B = new FCKDialogCommand(
				"BulletedList",
				FCKLang.BulletedListProp,
				"dialog/fck_listprop.html?UL",
				370,
				160
			);
			break;
		case "NumberedList":
			B = new FCKDialogCommand(
				"NumberedList",
				FCKLang.NumberedListProp,
				"dialog/fck_listprop.html?OL",
				370,
				160
			);
			break;
		...
		case "Find":
			B = new FCKDialogCommand(
				"Find",
				FCKLang.DlgFindAndReplaceTitle,
				"dialog/fck_replace.html",
				340,
				230,
				null,
				null,
		...
		case "SpecialChar":
			B = new FCKDialogCommand(
				"SpecialChar",
				FCKLang.DlgSpecialCharTitle,
				"dialog/fck_specialchar.html",
				400,
				290
			);
			break;
		...
		case "Select":
			B = new FCKDialogCommand(
				"Select",
				FCKLang.SelectionField,
				"dialog/fck_select.html",
				400,
				340
			);
			break;
		case "ImageButton":
			B = new FCKDialogCommand(
				"ImageButton",
				FCKLang.ImageButton,
				"dialog/fck_image.html?ImageButton",
				450,
				390
			);
			break;
...
FCKPanel.prototype.Show = function(x, y, A, B, C) {
	...
	if (this._Popup) {
		...
		if (FCKBrowserInfo.IsIE7) {
			if (
				this.ParentPanel &&
				this.ParentPanel.ResizeForSubpanel(this, D, E.offsetHeight)
			) {
		...
		if (
			FCK.ToolbarSet.CurrentInstance.GetInstanceObject("FCKPanel")
				._OpenedPanel &&
			FCK.ToolbarSet.CurrentInstance.GetInstanceObject("FCKPanel")
				._OpenedPanel != this
		)
			FCK.ToolbarSet.CurrentInstance.GetInstanceObject(
				"FCKPanel"
			)._OpenedPanel.Hide(false, true);
		FCKDomTools.SetElementStyles(E, {
			B: B ? B + "px" : "",
			C: C ? C + "px" : ""
		});
		...
		var F = FCKTools.GetDocumentPosition(
			this._Window,
			A.nodeType == 9
		...
		if (G) {
			var H = FCKTools.GetDocumentPosition(
				FCKTools.GetElementWindow(G),
				G
			);
			F.x -= H.x;
			F.y -= H.y;
...
FCKToolbarButtonUI.prototype.Create = function(A) {
	...
	FCKTools.AddEventListenerEx(
		C,
		"mouseover",
		FCKToolbarButtonUI_OnMouseOver,
		this
	);
	FCKTools.AddEventListenerEx(
		C,
		"mouseout",
		FCKToolbarButtonUI_OnMouseOut,
		this
	);
...
FCKToolbarButton.prototype.RefreshState = function() {
	...
	if (!A) return;
	var B = FCK.ToolbarSet.CurrentInstance.Commands.GetCommand(
		this.CommandName
	).GetState();
	if (B == A.State) return;
	A.ChangeState(B);
...
var FCKSpecialCombo = function(A, B, C, D, E) {
	...
	this._Panel.AppendStyleSheet(FCKConfig.SkinEditorCSS);
	this._PanelBox = this._Panel.MainNode.appendChild(
		this._Panel.Document.createElement("DIV")
	);
	this._PanelBox.className = "SC_Panel";
	this._PanelBox.style.width = this.PanelWidth + "px";
	this._PanelBox.innerHTML =
		'<table cellpadding="0" cellspacing="0" width="100%" style="TABLE-LAYOUT: fixed"><tr><td nowrap></td></tr></table>';
...
FCKSpecialCombo.prototype.AddItem = function(A, B, C, D) {
	var E = this._ItemsHolderEl.appendChild(
		this._Panel.Document.createElement("DIV")
	);
...
FCKSpecialCombo.prototype.Create = function(A) {
	...
	if (E) {
		...
		G.innerHTML =
			'<table width="100%" cellpadding="0" cellspacing="0" style="TABLE-LAYOUT: fixed;"><tbody><tr><td class="SC_FieldLabel"><label>&nbsp;</label></td><td class="SC_FieldButton">&nbsp;</td></tr></tbody></table>';
...
FCKToolbarStyleCombo.prototype.CreateItems = function(A) {
	...
	for (var D in C) {
		...
		var F =
			E.GetType() == 2
				? D
				: FCKToolbarStyleCombo_BuildPreview(E, E.Label || D);
		var G = A.AddItem(D, F);
		G.Style = E;
	}
...
FCKToolbarStyleCombo.prototype.RefreshActiveItems = function(A) {
	var B = FCK.ToolbarSet.CurrentInstance.Selection.GetBoundaryParentElement(
		true
	);
	if (B) {
		var C = new FCKElementPath(B);
...
FCKToolbarFontsCombo.prototype.GetStyles = function() {
	...
	if (!A) {
		alert(
			"The FCKConfig.CoreStyles['Size'] setting was not found. Please check the fckconfig.js file"
		);
		return {};
	}
	var B = {};
...
FCKToolbarFontsCombo.prototype.RefreshActiveItems =
	FCKToolbarStyleCombo.prototype.RefreshActiveItems;
...
FCKToolbarFontSizeCombo.prototype.GetStyles = function() {
	...
	if (!A) {
		alert(
			"The FCKConfig.CoreStyles['FontFace'] setting was not found. Please check the fckconfig.js file"
		);
		return {};
	}
	var B = {};
...
FCKToolbarFontSizeCombo.prototype.RefreshActiveItems =
	FCKToolbarStyleCombo.prototype.RefreshActiveItems;
FCKToolbarFontSizeCombo.prototype.StyleCombo_OnBeforeClick =
	FCKToolbarFontsCombo.prototype.StyleCombo_OnBeforeClick;
...
FCKToolbarPanelButton.prototype.RegisterPanel = function(A) {
	...
	B.style.top = "0px";
	var C = (A._FCKToolbarPanelButtonLineDiv = B.appendChild(
...
(function() {
	var A = [];
	var B =
		FCK && FCK.EditorWindow && FCK.EditorWindow.parent.parent.scayt
			? true
			: false;
	var C = false;
	var D = false;
	function ScaytEngineLoad(callback) {
		...
		if (FCK.Config.ScaytCustomUrl)
			FCK.Config.ScaytCustomUrl = new String(
				FCK.Config.ScaytCustomUrl
			).replace(new RegExp("^http[s]*://"), "");
		var H = document.location.protocol;
		var I =
			FCK.Config.ScaytCustomUrl ||
			"svc.spellchecker.net/spellcheck3/lf/scayt/scayt4.js";
		var J = H + "//" + I;
			...
			case "About":
				...
				if (B && D)
					FCKDialog.OpenDialog(
						"Scayt",
						"SCAYT Settings",
						"dialog/fck_scayt.html?" + c.toLowerCase(),
						343,
						343
					);
				break;
	...
	Z.prototype.GetLabel = function() {
		...
		return FCKBrowserInfo.IsIE
			? '<div class="TB_Button_Image"><img src="' +
					a +
					'" style="top:-192px"></div>'
			: '<img class="TB_Button_Image" src="' +
					FCK_SPACER_PATH +
					'" style="background-position: 0px -192px;background-image: url(' +
					a +
					');">';
	};
			...
			AddItems: function(menu) {
				...
				try {
					if (D && C)
						Q.fireOnContextMenu(
							null,
							FCK.ContextMenu._InnerContextMenu
						);
				} catch (err) {}
			}
		});
...
FCKToolbarItems.GetItem = function(A) {
	...
	switch (A) {
		...
		case "OrderedList":
			B = new FCKToolbarButton(
				"InsertOrderedList",
				FCKLang.NumberedListLbl,
				FCKLang.NumberedList,
				null,
				false,
				true,
				26
		...
		case "UnorderedList":
			B = new FCKToolbarButton(
				"InsertUnorderedList",
				FCKLang.BulletedListLbl,
				FCKLang.BulletedList,
				null,
				false,
				true,
				27
		...
		case "SpecialChar":
			B = new FCKToolbarButton(
				"SpecialChar",
				FCKLang.InsertSpecialCharLbl,
				FCKLang.InsertSpecialChar,
				null,
				false,
				true,
				42
...
FCKToolbar.prototype.Create = function(A) {
	...
	e.style.styleFloat = e.style.cssFloat =
		FCKLang.Dir == "ltr" ? "left" : "right";
	e.dir = FCKLang.Dir;
	e.cellPadding = 0;
	e.cellSpacing = 0;
	var C = e.insertRow(-1);
...
FCKToolbarSet.prototype.Load = function(A) {
	...
	FCKTools.DisableSelection(
		this._Document.getElementById("xCollapseHandle").parentNode
	);
	if (FCK.Status != 2)
...
var FCKDialog = (function() {
	...
	var G = function() {
		...
		FCKDomTools.SetElementStyles(C, {
			width:
				Math.max(H.scrollWidth, H.clientWidth, E.scrollWidth || 0) -
				1 +
				"px",
			height:
				Math.max(H.scrollHeight, H.clientHeight, E.scrollHeight || 0) -
				1 +
				"px"
		});
	};
	return {
		OpenDialog: function(
			...
			var L =
				FCKBrowserInfo.IsIE &&
				(!FCKBrowserInfo.IsIE7 || !FCKTools.IsStrictMode(D.document));
...
FCKMenuItem.prototype.Activate = function() {
	...
	if (this.HasSubMenu) {
		this.SubMenu.Show(
			this.MainElement.offsetWidth + 2,
			-2,
			this.MainElement
		);
	}
...
FCKMenuBlock.prototype.AddItem = function(A, B, C, D, E) {
	...
	F.OnActivate = FCKTools.CreateEventListener(
		FCKMenuBlock_Item_OnActivate,
		this
	);
	this._Items.push(F);
	return F;
...
var FCKContextMenu = function(A, B) {
	...
	D.Panel = C;
	D.OnClick = FCKTools.CreateEventListener(
...
function FCKContextMenu_AttachedElement_OnContextMenu(A, B, C) {
	if (
		(B.CtrlDisable && (A.ctrlKey || A.metaKey)) ||
...
function FCK_ContextMenu_Init() {
	var A = (FCK.ContextMenu._InnerContextMenu = new FCKContextMenu(
		FCKBrowserInfo.IsIE ? window : window.parent,
		FCKLang.Dir
	));
	...
	for (var i = 0; i < FCKConfig.ContextMenu.length; i++)
		B.RegisterListener(
			FCK_ContextMenu_GetListener(FCKConfig.ContextMenu[i])
		);
}
			...
			return {
				AddItems: function(menu, tag, tagName) {
					menu.AddItem(
						...
						FCKLang.Cut,
						7,
						FCKCommands.GetCommand("Cut").GetState() == -1
					);
					menu.AddItem(
						"Copy",
						FCKLang.Copy,
						8,
						FCKCommands.GetCommand("Copy").GetState() == -1
					);
					menu.AddItem(
						"Paste",
						FCKLang.Paste,
						9,
						FCKCommands.GetCommand("Paste").GetState() == -1
					);
				}
			};
		case "Table":
			return {
				AddItems: function(menu, tag, tagName) {
					...
					if (C) {
						...
						D.AddItem(
							"TableInsertCellBefore",
							FCKLang.InsertCellBefore,
							69
						);
						D.AddItem(
							"TableInsertCellAfter",
							FCKLang.InsertCellAfter,
							58
						);
						...
						D.AddItem(
							"TableVerticalSplitCell",
							FCKLang.VerticalSplitCell,
							61,
							FCKCommands.GetCommand(
								"TableVerticalSplitCell"
							).GetState() == -1
						...
						D.AddItem(
							...
							FCKLang.InsertRowBefore,
							70
						);
						D.AddItem(
							"TableInsertRowAfter",
							FCKLang.InsertRowAfter,
							62
						);
						...
						D.AddItem(
							...
							FCKLang.InsertColumnBefore,
							71
						);
						D.AddItem(
							"TableInsertColumnAfter",
							FCKLang.InsertColumnAfter,
							64
						);
						D.AddItem(
							"TableDeleteColumns",
							FCKLang.DeleteColumns,
							65
						);
					}
					if (B || C) {
		...
		case "HiddenField":
			return {
				AddItems: function(menu, tag, tagName) {
					if (
						tagName == "IMG" &&
						tag.getAttribute("_fckinputhidden")
					) {
						menu.AddSeparator();
						menu.AddItem(
							"HiddenField",
							FCKLang.HiddenFieldProp,
							56
						);
					}
				}
