...
FCK.InitializeBehaviors = function() {
			...
			if (
				!(
					...
					(!FCKBrowserInfo.IsOpera &&
						String(H.parentNode.contentEditable) == "false") ||
					(!(
						FCKListsLib.BlockElements[I] ||
						FCKListsLib.NonEmptyBlockElements[I]
					) &&
						B == 35)
				)
			)
			...
			if (J) {
				...
				while (
					...
					(!FCKListsLib.BlockElements[
						H.parentNode.tagName.toLowerCase()
					] &&
						!FCKListsLib.NonEmptyBlockElements[
							H.parentNode.tagName.toLowerCase()
						])
				)
					H = H.parentNode;
	...
	this.EditorDocument.addEventListener(
		"keyup",
		this.ExecOnSelectionChangeTimer,
		false
	);
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		this.EditorDocument.addEventListener(
			...
			function(ev) {
				if (
					ev.srcElement.nodeName.IEquals(
						"INPUT",
						"TEXTAREA",
						"SELECT"
					)
...
FCK.InsertHtml = function(A) {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		if (E && FCKListsLib.BlockElements[E.nodeName.toLowerCase()])
			range.SplitBlock();
...
FCK.CreateLink = function(A, B) {
	...
	if (A.length > 0) {
		...
		var E = this.EditorDocument.evaluate(
			"//a[@href='" + D + "']",
...
FCKDomRange.prototype.MoveToSelection = function() {
	...
	if (A && A.rangeCount > 0) {
		this._Range = FCKW3CRange.CreateFromRange(
			this.Window.document,
			A.getRangeAt(0)
		);
		this._UpdateElementInfo();
...
FCKSelection.GetType = function() {
	...
	if (B && B.rangeCount == 1) {
		...
		if (
			...
			FCKListsLib.StyleObjectElements[
				C.startContainer.childNodes[
					C.startOffset
...
FCKXml.prototype = {
	...
	SelectNodes: function(A, B) {
		...
		var C = [];
		var D = this.DOMDocument.evaluate(
			A,
			B ? B : this.DOMDocument,
	...
	SelectSingleNode: function(A, B) {
		...
		var C = this.DOMDocument.evaluate(
			A,
			B ? B : this.DOMDocument,
			this.DOMDocument.createNSResolver(this.DOMDocument.documentElement),
			9,
