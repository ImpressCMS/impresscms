...
* @version		$Id: edituser.php 11462 2011-12-11 01:07:52Z skenow $
*
* @package kernel 
* @subpackage users
*/
...
$allowedHTML = array('htmlcode');
	if(!empty($_POST)){ foreach($_POST as $k => $v){ if (!in_array($k,$allowedHTML)){${$k} = filter_var($v, FILTER_SANITIZE_STRING);}else{${$k} = $v;}}}
	if(!empty($_GET)){ foreach($_GET as $k => $v){ if (!in_array($k,$allowedHTML)){${$k} = filter_var($v, FILTER_SANITIZE_STRING);}else{${$k} = $v;}}}
$op = (isset($_GET['op']))?trim(filter_var($_GET['op'], FILTER_SANITIZE_STRING)):((isset($_POST['op']))?trim(filter_var($_POST['op'], FILTER_SANITIZE_STRING)):'editprofile');
...
$errors[] .= _US_NICKNAMENOSPACES."<br />";
	...
	else
	{
		...
		if($icmsConfigUser['allwshow_sig'] == 1)
		{
			...
			{
			$signature = $myts->displayTarea($_POST['user_sig'], 1, 1, 1, 1, 0);
			$edituser->setVar('user_sig', xoops_substr($signature, 0, intval($icmsConfigUser['sig_max_length'])));
			}
		}
