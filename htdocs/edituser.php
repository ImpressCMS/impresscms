...
if ($op == 'editprofile')
{
	...
	if ($icmsConfigUser['allwshow_sig'] == 1)
	{
		if ($icmsConfigUser['allow_htsig'] == 0)
		{
			...
			$sig_tarea = new icms_form_elements_Textarea('', 'user_sig', $icmsUser->getVar('user_sig', 'E'));
	...
	$bio_tarea = new icms_form_elements_Textarea(_US_EXTRAINFO, 'bio', $icmsUser->getVar('bio', 'E'));
	...
	$mailok_radio = new icms_form_elements_Radioyn(_US_MAILOK, 'user_mailok', (int) ($icmsUser->getVar('user_mailok')));
...
if ($op == 'avatarform')
{
	...
	$avatar_tray->addElement(new icms_form_elements_Label('', "<img src='".ICMS_UPLOAD_URL."/".$icmsUser->getVar("user_avatar", "E")."' name='avatar' id='avatar' alt='' /> <a href=\"javascript:openWithSelfMain('".ICMS_URL."/misc.php?action=showpopups&amp;type=avatars','avatars',600,400);\">"._LIST."</a>"));
	if ($icmsConfigUser['avatar_allow_upload'] == 1 && $icmsUser->getVar('posts') < $icmsConfigUser['avatar_minposts']) {
		...
		if ($uploader->fetchMedia($_POST['xoops_upload_file'][0]))
		{
			...
			if ($uploader->upload())
			{
				...
				if (!$avt_handler->insert($avatar))
				{
					...
					{
						$avatars =& $avt_handler->getObjects(new icms_criteria_Item('avatar_file', $oldavatar));
...
if ($op == 'avatarchoose')
{
	...
	if (0 === strpos($user_avatarpath, ICMS_UPLOAD_PATH) && is_file($user_avatarpath))
	{
		...
		{
			$avatars =& $avt_handler->getObjects(new icms_criteria_Item('avatar_file', $user_avatar));
