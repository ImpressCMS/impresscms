...
class IcmsAutoTasks extends IcmsPersistableObject {
	...
	function getType($part=null) {
		$type = explode('/',$this->getVar('sat_type'));
    ...
    function exec() {
		...
		$function =  create_function( '$param', $code );
...
class IcmsAutotasksHandler extends IcmsPersistableObjectHandler {

	public $virtual_config = array();

	function IcmsAutotasksHandler($db) {
		$this->IcmsPersistableObjectHandler ( $db, 'icmsautotasks', 'sat_id', 'sat_name', 'sat_code', '');
	}
	 ...
	 * @return string
	 */
	private function getSystemHandlerNameFromFileName(string $filename) {
	 ...
	 */
	function &getConfig() {
		if (is_array($this->virtual_config) && count($this->virtual_config) > 0) {
			return $this->virtual_config;
		}
	 ...
	 * @return	array
	 */
	function &getSystemHandlersList(bool $checkIfItIsAvaibleOnCurrentSystem) {
		static $ret = null;
		if ($ret == null) {
