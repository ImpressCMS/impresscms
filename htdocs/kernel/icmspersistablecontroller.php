...
* @version		$Id: icmspersistablecontroller.php 11498 2011-12-17 21:10:54Z skenow $
*/
...
class IcmsPersistableController {
	...
	function handleObjectDeletion($confirm_msg = false, $op='del', $userSide=false)
	{
		...
		if ($confirm) {
			...
			xoops_confirm($hiddens, xoops_getenv('SCRIPT_NAME'), sprintf($confirm_msg , $icmsObj->getVar($this->handler->identifierName)), _CO_ICMS_DELETE);
	...
	function handleObjectDeletionFromUserSide($confirm_msg = false, $op='del') {
		...
		if ($confirm) {
			...
			ob_start();
			xoops_confirm(array('op' => $op, $this->handler->keyName => $icmsObj->getVar($this->handler->keyName), 'confirm' => 1, 'redirect_page' => $impresscms->urls['previouspage']), xoops_getenv('SCRIPT_NAME'), sprintf($confirm_msg , $icmsObj->getVar($this->handler->identifierName)), _CO_ICMS_DELETE);
