...
* @version	$Id: resetpass.php 11462 2011-12-11 01:07:52Z skenow $
*/
/**
...
$c_password = (isset($_GET['c_password']))?trim(filter_var($_GET['c_password'], FILTER_SANITIZE_STRING)):((isset($_POST['c_password']))?trim(filter_var($_POST['c_password'], FILTER_SANITIZE_STRING)):$c_password);
$password = (isset($_GET['password']))?trim(filter_var($_GET['password'], FILTER_SANITIZE_STRING)):((isset($_POST['password']))?trim(filter_var($_POST['password'], FILTER_SANITIZE_STRING)):$password);
$password2 = (isset($_GET['password2']))?trim(filterv_var($_GET['password2'], FILTER_SANITIZE_STRING)):((isset($_POST['password2']))?trim(filter_var($_POST['password2'], FILTER_SANITIZE_STRING)):$password2);
...
else
{
    ...
    if($table->fieldExists('loginname')) {
		$userName = strtolower($getuser[0]->getVar('loginname'));
    }
	elseif($table->fieldExists('login_name')) {
		$userName = strtolower($getuser[0]->getVar('login_name'));
	} else {
		$userName = strtolower($getuser[0]->getVar('uname'));
	}

	if($userName !== strtolower($username)) {redirect_header('user.php',2,_US_SORRYUNAMENOTMATCHEMAIL);}
	else
	{
