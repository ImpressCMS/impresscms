...
// Prefer Composer-installed tcpdf package if present (trust path first, then webroot).
$vendor_tcpdf_candidates = [
	rtrim(ICMS_TRUST_PATH, "/\\") . "/vendor/tecnickcom/tcpdf",
	rtrim(ICMS_ROOT_PATH, "/\\") . "/vendor/tecnickcom/tcpdf",
];
$vendor_tcpdf_path = null;
foreach ($vendor_tcpdf_candidates as $_p) {
	if (file_exists($_p) && is_dir($_p)) {
		$vendor_tcpdf_path = $_p;
		break;
	}
}

if ($vendor_tcpdf_path !== null) {
	// Use vendor package path
	define("ICMS_PDF_LIB_PATH", $vendor_tcpdf_path);
	// If the vendor package is inside the webroot, try to produce a URL; otherwise keep legacy URL.
	$root_path_normalized = str_replace(
		"\\",
		"/",
		rtrim(ICMS_ROOT_PATH, "/\\"),
	);
	$vendor_normalized = str_replace("\\", "/", $vendor_tcpdf_path);
	if (strpos($vendor_normalized, $root_path_normalized) === 0) {
		$relative = substr($vendor_normalized, strlen($root_path_normalized));
		if ($relative === "" || $relative[0] !== "/") {
			$relative = "/" . $relative;
		}
		define("ICMS_PDF_LIB_URL", ICMS_URL . $relative);
	} else {
		define("ICMS_PDF_LIB_URL", ICMS_URL . "/libraries/tcpdf");
	}
} else {
	// Fallback to bundled libraries/tcpdf
