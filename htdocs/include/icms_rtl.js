...
function AdMenu(id) {
	...
	this.init = function() {
		if (!document.getElementById(this.id))
			alert(
				"Element '" +
					this.id +
					"' does not exist in this document. AdMenu cannot be initialized."
			);
		if (this.type != "horizontal" && this.type != "vertical") {
			return alert(
				"AdMenu.init() failed. Unknown menu type: '" + this.type + "'"
			);
		}
	...
	this.getMaxWidth = function(nodes) {
		...
		for (var i = 0; i < nodes.length; i++) {
			if (nodes[i].nodeType != 1 || nodes[i].className == "section") {
				continue;
			}
					...
					if (el.level == 1) {
						if (this.type == "horizontal") {
							box2.style.top =
								box1.offsetTop +
								box1.offsetHeight +
								this.position.level1.top +
								"px";
							if (this.browser.ie5) {
								box2.style.left =
									this.position.level1.left + "px";
							} else {
						...
						box2.style.top =
							box1.offsetTop + this.position.levelX.top + "px";
	...
	this.hide = function(id) {
		...
		document.getElementById(
			id + "-section"
		).style.zIndex = this.zIndex.hidden;
		if (this.visible.contains(id)) {
			if (this.visible.getLast() == id) {
				...
				throw "AdMenu.hide(" +
					id +
					") failed, trying to hide element that is not deepest visible element";
			}
		} else {
			throw "AdMenu.hide(" +
				id +
				") failed, cannot hide element that is not visible";
		}
	};
	this.show = function(id) {
		var el = new Element(id);
		document.getElementById(id).className =
			el.level == 1 ? "button-active" : "item-active";
		if (el.level > 1 && this.arrow2) {
		...
		document.getElementById(
			id + "-section"
		).style.zIndex = this.zIndex.visible;
	...
	function buttonOver() {
		...
		if (self.visible.contains(this.id)) {
			return;
		}
		self.hideAll();
	...
	function Element(id) {
		...
		if (!id) {
			throw "AdMenu.Element(id) failed, id cannot be empty";
		}
		this.menu = self;
		this.id = id;
