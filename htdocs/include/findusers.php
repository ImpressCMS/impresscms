...
$unsets = ["actkey", "pass", "theme", "umode", "uorder", "notify_mode"];
...
if (empty($_POST["user_submit"])) {
    ...
    if (FINDUSERS_MODE_QUERY == $mode) {
        ...
        if (FINDUSERS_MODE_ADVANCED == $mode) {
            ...
            $mailok_radio->addOptionArray(["mailok" =>_MA_USER_MAILOK, "mailng" =>_MA_USER_MAILNG, "both" =>_MA_USER_BOTH]);
            $avatar_radio = new icms_form_elements_Radio(_MA_USER_HASAVATAR, "user_avatar", empty($_POST["user_avatar"]) ? "both" : $_POST["user_avatar"]);
            $avatar_radio->addOptionArray(["y" =>_YES, "n" =>_NO, "both" =>_MA_USER_BOTH]);

            $level_radio = new icms_form_elements_Radio(_MA_USER_LEVEL, "level", @$_POST["level"]);
            $levels = [0 => _ALL, 1 => _MA_USER_LEVEL_ACTIVE, 2 => _MA_USER_LEVEL_INACTIVE, 3 => _MA_USER_LEVEL_DISABLED];
        ...
        $sort_select->addOptionArray(["uname" =>_MA_USER_UNAME, "last_login" =>_MA_USER_LASTLOGIN, "user_regdate" =>_MA_USER_REGDATE, "posts" =>_MA_USER_POSTS]);
        $order_select = new icms_form_elements_Select(_MA_USER_ORDER, "user_order", @$_POST["user_order"]);
        $order_select->addOptionArray(["ASC" =>_MA_USER_ASC, "DESC" =>_MA_USER_DESC]);
    ...
    $acttotal = $user_handler->getUserCountByGroupLink([], new icms_db_criteria_Item('level', 0, '>'));
    $inacttotal = $user_handler->getUserCountByGroupLink([], new icms_db_criteria_Item('level', 0, '<='));
    echo "</html><body>";
