...
function xoopsCheckAll(form, switchId) {
	...
	for (i = 0; i != eltForm.elements.length; i++) {
		if (
			eltForm.elements[i] != eltSwitch &&
			eltForm.elements[i].type == "checkbox"
		) {
...
function xoopsCodeImg(
	...
	if (text != null && text != "") {
		...
		if (imgPosRorLPhrase == null) {
			imgPosRorLPhrase =
				"'R' or 'r' for right, 'L' or 'l' for left, 'C' or 'c' for center, or leave it blank.";
		}
...
function xoopsCodeCode(
	...
	if (text != null && text != "") {
		...
		if (CodeLangTypePhrase == null) {
			CodeLangTypePhrase =
				"'P' or 'p' for PHP, 'C' or 'c' for CSS, 'J' or 'j' for JAVASCRIPT, 'H' or 'h' for HTML, or leave it blank.";
		}
...
function xoopsCodeText(id, hiddentext, enterTextboxPhrase) {
	...
	if (textDomValue == "") {
		...
		if (fontDomValue != "FONT") {
			textDomValue =
				"[font=" + fontDomValue + "]" + textDomValue + "[/font]";
			fontDom.options[0].selected = true;
		}
		if (colorDomValue != "COLOR") {
			textDomValue =
				"[color=" + colorDomValue + "]" + textDomValue + "[/color]";
			colorDom.options[0].selected = true;
		}
		if (sizeDomValue != "SIZE") {
			textDomValue =
				"[size=" + sizeDomValue + "]" + textDomValue + "[/size]";
			sizeDom.options[0].selected = true;
		}
		if (
			xoopsHiddenTextDomStyle.fontWeight == "bold" ||
			xoopsHiddenTextDomStyle.fontWeight == "700"
		) {
