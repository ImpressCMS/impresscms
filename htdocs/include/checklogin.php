...
$xoopsAuth =& icms_auth_Factory::getAuthConnection($myts->addSlashes($uname));
...
if (false != $user) {
	...
	if (!empty($_POST['rememberme'])) {
		$expire = time() + ( defined('XOOPS_AUTOLOGIN_LIFETIME')
	...
	redirect_header(
		...
		. urlencode(trim($_POST['xoops_redirect'])), 5, $xoopsAuth->getHtmlErrors(), false
	);
}
exit();
