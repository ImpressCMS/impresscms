...
if (! defined('ICMS_ROOT_PATH') || ! defined('ICMS_URL')) {
    die("You should patch just after define('ICMS_URL', ...) in mainfile.php");
}
...
if (! preg_match('?' . preg_quote(ICMS_ROOT_PATH, '?') . '(/common/)?', $_SERVER['SCRIPT_FILENAME'])) {
    ...
    if (! empty($_GET['lang']) && $_GET['lang'] == 'all') {
        // set by GET (all)
        $easiestml_lang = 'all';
    } elseif (! empty($_GET['lang']) && ($offset = array_search($_GET['lang'], $easiestml_langs)) !== false) {
    ...
    } elseif (! empty($_COOKIE['lang']) && ($offset = array_search($_COOKIE['lang'], $easiestml_langs)) !== false) {
...
function easiestml($s)
{
    ...
    } elseif (($pos = strpos($_SERVER['QUERY_STRING'], 'lang=')) === false) {
