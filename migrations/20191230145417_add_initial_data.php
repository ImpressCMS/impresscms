...
class AddInitialData extends AbstractMigration
{
	...
	{
		$language = env('INSTALL_LANGUAGE', 'english');
		$adminlogin_name = env('INSTALL_ADMIN_LOGIN');
		if (empty($adminlogin_name)) {
			throw new RuntimeException('INSTALL_ADMIN_LOGIN environment variable must be defined');
		}
		$adminpass = env('INSTALL_ADMIN_PASS');
		if (empty($adminpass)) {
			throw new RuntimeException('INSTALL_ADMIN_PASS environment variable must be defined');
		}
		$adminname = env('INSTALL_ADMIN_NAME');
		if (empty($adminname)) {
			throw new RuntimeException('INSTALL_ADMIN_NAME environment variable must be defined');
		}
		$adminmail = env('INSTALL_ADMIN_EMAIL');
		if (empty($adminmail)) {
		...
		$this->insertWithPrefix('groups', [
			[
				'name' => _MIGRATIONS_INSTALL_WEBMASTER,
				'description' => _MIGRATIONS_INSTALL_WEBMASTERD,
				'group_type' => 'Admin'
			]
		]);
		$this->insertWithPrefix('groups', [
			[
				'name' => _MIGRATIONS_INSTALL_REGUSERS,
				'description' => _MIGRATIONS_INSTALL_REGUSERSD,
				'group_type' => 'User'
			]
		]);
		$this->insertWithPrefix('groups', [
			[
				'name' => _MIGRATIONS_INSTALL_ANONUSERS,
				'description' => _MIGRATIONS_INSTALL_ANONUSERSD,
				'group_type' => 'Anonymous'
			]
		]);

		// data for table 'groups_users_link'
		$this->insertWithPrefix('groups_users_link', [
			[
				'groupid' => ICMS_GROUP_ADMIN,
				'uid' => 1
			],
			[
				'groupid' => ICMS_GROUP_USERS,
				'uid' => 1
			]
		]);
		...
		$this->insertWithPrefix('group_permission', [
			[
				'gperm_groupid' => ICMS_GROUP_ADMIN,
				'gperm_itemid' => 1,
				'gperm_modid' => 1,
			...
			[
				'gperm_groupid' => ICMS_GROUP_USERS,
				'gperm_itemid' => 1,
				'gperm_modid' => 1,
				'gperm_name' => 'module_read'
			],
			[
				'gperm_groupid' => ICMS_GROUP_ANONYMOUS,
				'gperm_itemid' => 1,
				'gperm_modid' => 1,
				'gperm_name' => 'module_read'
			],
		...
		$this->insertWithPrefix('imagecategory', [
			[
				'imgcat_id' => 1,
				'imgcat_pid' => 0,
				'imgcat_name' => 'Logos',
		...
		foreach ($modversion['blocks'] as $func_num => $newblock) {
			if (file_exists(ICMS_MODULES_PATH . '/system/templates/blocks/' . $newblock['template'])) {
				...
				if ($newblock['template'] == 'system_block_search.html') {
					...
					$canvaspos = 9;
				} elseif (in_array($newblock['template'], ['system_block_newusers.html', 'system_block_topusers.html'])) {
					$canvaspos = 10;
				} else {
					$canvaspos = 1;
				}
				...
				$this->insertWithPrefix('newblocks', [
					[
						'mid' => 1,
						'func_num' => $func_num,
						'options' => $options,
		...
		if (defined('_MIGRATION_WELCOME_WEBMASTER')) {
			$this->insertWithPrefix('newblocks', [
				[
					'mid' => 0,
					'func_num' => 0,
					...
					'title' => _MIGRATIONS_WELCOME_WEBMASTER,
					'content' => _MIGRATION_WELCOME_WEBMASTER,
					'side' => 4,
					'weight' => 0,
					'visible' => 1,
					'block_type' => 'C',
		...
		if (defined('_MIGRATION_WELCOME_ANONYMOUS')) {
			$this->insertWithPrefix('newblocks', [
				[
					'mid' => 0,
					'func_num' => 0,
					'options' => '',
					'name' => 'Custom Block (Auto Format + smilies)',
					'title' => _MIGRATIONS_WELCOME_ANONYMOUS,
					'content' => _MIGRATION_WELCOME_ANONYMOUS,
					'side' => 4,
					'weight' => 0,
					'visible' => 1,
					'block_type' => 'C',
		...
		$this->insertWithPrefix('config',
			[
				[
					...
					'conf_value' => _MIGRATIONS_LOCAOL_STNAME,
				...
				[
					...
					'conf_value' => _MIGRATIONS_LOCAL_SLOCGAN,
				...
				[
					...
					'conf_value' => _MIGRATIONS_EXT_DATE_FUNC,
				...
				[
					...
					'conf_value' => _MIGRATIONS_INSTALL_ANON,
		...
		$this->insertWithPrefix('configoption', [
			[
				'confop_id' => $ci++,
		...
		$this->insertWithPrefix('config',
			[
				...
				[
					...
					'conf_value' => _MIGRATIONS_INSTALL_L165,
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => serialize(['127.0.0.1']),
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => _MIGRATIONS_WELCOME_MSG_CONTENT,
		...
		$this->insertWithPrefix('config',
             ...
             */
				...
				[
					...
					'conf_value' => serialize(['webmaster', '^impresscms', '^admin']),
				...
				[
					...
					'conf_value' => serialize(['impresscms.org$']),
				...
				[
					...
					'conf_value' => _MIGRATIONS_INSTALL_DISCLMR,
				...
				[
					...
					'conf_value' => _MIGRATIONS_INSTALL_PRIVPOLICY,
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => _MIGRATIONS_LOCAL_FOOTER,
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => _MIGRATIONS_LOCAL_SENSORTXT,
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => _MIGRATIONS_DEF_LANG_TAGS,
			...
			[
				...
				'conf_value' => _MIGRATIONS_DEF_LANG_NAMES,
			...
			[
				...
				'conf_value' => _MIGRATIONS_LOCAL_LANG_NAMES,
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => _MIGRATIONS_AM_RSSLOCALLINK_DESC,
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => serialize(['o', '0', 'i', 'l', '1']),
		...
		$this->insertWithPrefix('config', [
			[
				...
				'conf_value' => serialize(['syntaxhighlightphp', 'hiddencontent']),
		...
		$host_domain = explode('.', parse_url(ICMS_URL, PHP_URL_HOST));
		if (count($host_domain) > 2) {
			$host_domain = array_slice($host_domain, -2);
		}
		$host_domain = implode('.', $host_domain);
		$host_base = parse_url(ICMS_URL, PHP_URL_HOST);
		...
		$this->insertWithPrefix('config', [
			...
			[
				...
				'conf_value' => serialize(['http', 'https', 'mailto', 'ftp', 'nntp', 'news']),
		...
		$this->insertWithPrefix('block_positions', [
			[
				'id' => 1,
				'pname' => 'canvas_left',
				'title' => '_AM_SBLEFT',
		...
		$this->insertWithPrefix('configcategory', [
			[
				'confcat_id' => 1,
		...
		$this->insertWithPrefix('imgset_tplset_link', [
			[
				'imgset_id' => 1,
				'tplset_name' => 'default'
			]
		...
		$this->insertWithPrefix('system_mimetype', [
			[
				'mimetypeid' => 1,
				'extension' => 'bin',
		...
		$this->insertWithPrefix('ranks', [
			[
				'rank_id' => 1,
				'rank_title' => _MIGRATION_RANK_1,
				'rank_min' => 0,
				'rank_max' => 20,
				'rank_special' => 0,
		...
		$this->insertWithPrefix('smiles', [
			[
				'id' => 1,
				'code' => ':-D',
				'smile_url' => 'smil3dbd4d4e4c4f2.gif',
				'emotion' => _MIGRATION_SMILE_1,
				'display' => 1
			],
			[
				'id' => 2,
			...
			[
				...
				'emotion' => _MIGRATION_SMILE_17,
				'display' => 0
			],
		]);

		$this->insertWithPrefix('icmspage', [
			[
				'page_id' => 2,
				'page_moduleid' => 1,
				'page_title' => _MIGRATION_PAGE_2,
				'page_url' => 'admin.php',
				'page_status' => 1
			],
			[
	...
	/**
	 * @param string $table Table without prefix where to insert data
	 * @param array $data Data to be inserted
	 *
	 * @return AbstractMigration
	 */
	private function insertWithPrefix($table, $data)
	{
		if (empty($data)) {
			return $this;
		}
