...
switch ($op) {
	...
	case 'delfile':
		...
		$src = '<img src="' . ICMS_MODULES_URL . "/system/admin/images/preview.php?file=" . $image->image_name . '" title="' . $image->image_nicename . '" /><br />';
...
function imanager_listimg($imgcat_id, $start = 0) {
	...
	$icmsAdminTpl->assign('cat_maxsize', icms_convert_size($imagecategory->imgcat_maxsize));
	$icmsAdminTpl->assign('cat_maxwidth', $imagecategory->imgcat_maxwidth);
	$icmsAdminTpl->assign('cat_maxheight', $imagecategory->imgcat_maxheight);
	$icmsAdminTpl->assign('cat_storetype', $imagecategory->imgcat_storetype);
	$icmsAdminTpl->assign('cat_display', $imagecategory->imgcat_display);
	$icmsAdminTpl->assign('cat_id', $imagecategory->imgcat_id);
	...
	foreach (array_keys($images) as $i) {
		...
		$preview_url = '<a class="cboxElement" href="' . $src_lightbox . '" title="' . $images[$i]->image_nicename . '"><img src="' . ICMS_IMAGES_SET_URL . '/actions/viewmag.png" alt="' . _PREVIEW . '" title="' . _PREVIEW . '" /></a>';
		...
		if (in_array($images[$i]->image_mimetype, $extra_perm)) {
			...
			if (file_exists(ICMS_LIBRARIES_PATH . '/image-editor/image-edit.php')) {
				$arrimg[$i]['editor_link'] = 'window.open(\'' . ICMS_LIBRARIES_URL . '/image-editor/image-edit.php?image_id=' . $images[$i]->image_id . '&uniq=' . $uniq . '\',\'icmsDHTMLImageEditor\',\'width=800,height=600,left=\'+parseInt(screen.availWidth/2-400)+\',top=\'+parseInt(screen.availHeight/2-350)+\',resizable=no,location=no,menubar=no,status=no,titlebar=no,scrollbars=no\'); return false;';
			} else {
...
function imanager_editcat($imgcat_id) {
	...
	$form->addElement(new icms_form_elements_Text(_MD_IMGCATNAME, 'imgcat_name', 50, 255, $imagecategory->imgcat_name), true);
	...
	$form->addElement(new icms_form_elements_Text(_IMGMAXSIZE, 'imgcat_maxsize', 10, 10, $imagecategory->imgcat_maxsize));
	$form->addElement(new icms_form_elements_Text(_IMGMAXWIDTH, 'imgcat_maxwidth', 3, 4, $imagecategory->imgcat_maxwidth));
	$form->addElement(new icms_form_elements_Text(_IMGMAXHEIGHT, 'imgcat_maxheight', 3, 4, $imagecategory->imgcat_maxheight));
	$form->addElement(new icms_form_elements_Text(_MD_IMGCATWEIGHT, 'imgcat_weight', 3, 4, $imagecategory->imgcat_weight));
	$form->addElement(new icms_form_elements_Radioyn(_MD_IMGCATDISPLAY, 'imgcat_display', $imagecategory->imgcat_display, _YES, _NO));
	...
	$form->addElement(new icms_form_elements_Label(_MD_IMGCATSTRTYPE, $storetype[$imagecategory->imgcat_storetype]));
...
function imanager_updatecat() {
	...
	foreach ($readgroup as $rgroup) {
		$imagecategoryperm =  $imagecategoryperm_handler->create();
		$imagecategoryperm->gperm_groupid = $rgroup;
		$imagecategoryperm->gperm_itemid = $imgcat_id;
		$imagecategoryperm->gperm_name = 'imgcat_read';
		$imagecategoryperm->gperm_modid = 1;
		$imagecategoryperm_handler->insert($imagecategoryperm);
		unset($imagecategoryperm);
...
function imanager_delcatok($imgcat_id) {
	...
	if ($imagecategory->imgcat_type != 'C') {
	...
	if ($imagecategory->imgcat_storetype == 'db') {
		...
		foreach (array_keys($images) as $i) {
			if (!$image_handler->delete($images[$i])) {
				...
				if (file_exists($categ_path . $images[$i]->image_name)) {
					if (!@unlink($categ_path . $images[$i]->image_name)) {
	...
	if (!$imgcat_handler->delete($imagecategory)) {
		$errors[] = sprintf(_MD_FAILDELCAT, $imagecategory->imgcat_name);
	} else {
		if ($imagecategory->imgcat_storetype != 'db') {
			if (!@rmdir($categ_path)) {
...
function imanager_delfileok($image_id, $redir = null) {
	...
	$imgcat_handler = icms::handler('icms_image_category');
	$imagecategory  =  $imgcat_handler->get((int) $image->imgcat_id);
	$categ_path = $imgcat_handler->getCategFolder($imagecategory);
