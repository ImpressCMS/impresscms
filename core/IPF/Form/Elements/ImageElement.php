...
class ImageElement extends \ImpressCMS\Core\Form\Elements\TrayElement {
	...
	public function __construct($object, $key) {
		...
		if ($object->getVar($key, 'e') != '' && (substr($object->getVar($key, 'e'), 0, 4) == 'http' || substr($object->getVar($key, 'e'), 0, 10) == '{ICMS_URL}')) {
			$this->addElement(new \ImpressCMS\Core\Form\Elements\LabelElement('', "<img src='" . str_replace('{ICMS_URL}', ICMS_URL, $object->getVar($key, 'e')) . "' alt='' /><br/><br/>"));
		} elseif ($object->getVar($key, 'e') != '') {
			$this->addElement(new \ImpressCMS\Core\Form\Elements\LabelElement('', "<a rel='lightbox' title='" . $object_imageurl . $object->getVar($key, 'e')
		...
		if (!isset($control['nourl']) || !$control['nourl']) {
			$this->addElement(new \ImpressCMS\Core\Form\Elements\LabelElement('<div style="padding-top: 8px; font-size: 80%;">' . _CO_ICMS_URL_FILE_DSC . '</div>', ''));
		...
		if (!$object->isNew()) {
			$this->addElement(new \ImpressCMS\Core\Form\Elements\LabelElement('', '<br /><br />'));
