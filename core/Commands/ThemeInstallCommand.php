...
class ThemeInstallCommand extends Command implements ContainerAwareInterface
{
	...
	protected function configure()
	{
		$this
			...
			->setDescription('Installs specific theme')
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$decoratedOutput->info('Installing theme from ' . $themeInfo->path . '...');
		$decoratedOutput->writeln(_VERSION . ': ' . icms_conv_nr2local($themeInfo->version));

		/**
		 * @var SetupStepInterface[] $steps
		 */
		$steps = (array)$this->container->get(SetupStepsProvider::SETUP_STEP_THEME_INSTALL);
		...
		foreach ($steps as $step) {
			...
			if (!$step->execute($themeInfo, $decoratedOutput)) {
				$output->fatal('Failed to install', $themeInfo->path);
				break;
			}
		}
