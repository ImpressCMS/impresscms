...
class Filesystem {
	 ...
	 */
	public static function combineFiles(array $files, $type, $minimize = false, $replace = false, $maxage = 0, $location = ICMS_CACHE_PATH) {
		$expired = false;
	 ...
	 * - if the file does not exist, you need write permissions for the path
	 *
	 * @param string $oldname File or directory to rename, with path
	 * @param string $newname New name, with full path
	 * @param bool $overwrite If TRUE, overwrite an existing file with the same name
	 * @return bool
	 */
	public static function rename($oldname, $newname, $overwrite) {
		if ($oldname === $newname) {
			return true;
		}
	 ...
	 * is being completed
	 */
	public static function getImgList($dirname, $prefix = '', $extension = ['gif', 'jpg', 'png']) {
		return self::getFileList($dirname, $prefix, $extension);
	}

	public static function getFontList($dirname, $prefix = '', $extension = ['ttf']) {
		return self::getFileList($dirname, $prefix, $extension);
	}

	public static function getPhpFiles($dirname, $prefix = '', $extension = ['php']) {
		return self::getFileList($dirname, $prefix, $extension);
	}

	public static function getHtmlFiles($dirname, $prefix = '', $extension = ['htm', 'html', 'xhtml']) {
		return self::getFileList($dirname, $prefix, $extension);
	}
}
